...
Public Class FrmVoucherTypePrintSettings
    ...
    Private Sub FrmVoucher_Type_Print_SettingsMaster_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1 " & AgL.RetDivisionCondition(AgL, "H.Div_Code") & " And H.Site_Code = '" & AgL.PubSiteCode & "' AND IfNull(H.IsDeleted,0) = 0"

        AgL.PubFindQry = " SELECT H.Code AS SearchCode, H.Query , Vt.Description As Voucher_Type, H.Report_Name, H.Report_Heading,  H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], " &
                        " H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date], H.Status  " &
                        " FROM  Voucher_Type_Print_Settings H " &
                        " LEFT JOIN Voucher_Type Vt ON Vt.V_Type = H.V_Type " & mConStr &
    ...
    Private Sub FrmVoucher_Type_Print_SettingsMaster_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SQLiteConnection, ByVal Cmd As SQLiteCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = " Update Voucher_Type_Print_Settings " &
                "   SET  " &
                "	V_Type = " & AgL.Chk_Text(TxtVoucherType.AgSelectedValue) & " ," &
                "	Site_Code = " & AgL.Chk_Text(AgL.PubSiteCode) & " ," &
                "	Query = " & AgL.Chk_Text(TxtQuery.Text) & ", " &
                "	Report_Name = " & AgL.Chk_Text(TxtReportName.Text) & ", " &
                "	Report_Heading = " & AgL.Chk_Text(TxtReportHeading.Text) & " " &
    ...
    Private Sub FrmVoucher_Type_Print_SettingsMaster_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        ...
        mQry = "Select Code As SearchCode " &
            " From Voucher_Type_Print_Settings " & mConStr &
            " And IfNull(IsDeleted,0)=0 Order By V_Type "
    ...
    Private Sub FrmVoucher_Type_Print_SettingsMaster_BaseFunction_FIniMastLog(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMastLog
        ...
        mQry = "Select UID As SearchCode " &
               " From Voucher_Type_Print_Settings_log " & mConStr &
    ...
    Private Sub FrmVoucher_Type_Print_SettingsMaster_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = " Select T.*  " &
            " From Voucher_Type_Print_Settings T " &
