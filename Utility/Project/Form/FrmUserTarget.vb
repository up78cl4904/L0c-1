...
Public Class FrmUserTarget
    ...
    Private Sub IniGrid()
        ...
        With AgCL
            ...
            Col1EntryPoint_Qry = "Select Distinct MnuText As Code, MnuText As Name " &
    ...
    Private Sub FIniMaster(Optional ByVal BytDel As Byte = 0, Optional ByVal BytRefresh As Byte = 1)
        mQry = "Select User_Target.Code As SearchCode " &
        " From User_Target "
        ...
        Try
            ...
            If DTMaster.Rows.Count > 0 Then
                If TxtDate_To.Text.Trim <> "" And DTMaster.Rows.Count > 1 Then
                    ...
                    mQry = "Select IfNull(Count(*),0) Cnt From User_Target " &
    ...
    Private Sub Topctrl1_tbSave() Handles Topctrl1.tbSave
        ...
        Try
            ...
            If Topctrl1.Mode = "Add" Then
                mQry = "Insert Into User_Target (Code, UserName, Date_From, Date_To, " &
                        " Div_Code, Site_Code, U_EntDt, PreparedBy, U_AE) Values(" &
                        " '" & mSearchCode & "', " & AgL.Chk_Text(CboUserName.SelectedValue) & "," &
                        " " & AgL.ConvertDate(TxtDate_From.Text) & ", " & AgL.ConvertDate(TxtDate_To.Text) & "," &
                        " '" & AgL.PubDivCode & "', '" & AgL.PubSiteCode & "', '" & Format(AgL.PubLoginDate, "Short Date") & "', '" & AgL.PubUserName & "', 'A') "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
            Else
                mQry = "Update User_Target Set UserName = " & AgL.Chk_Text(CboUserName.SelectedValue) & "," &
                        " Date_From = " & AgL.ConvertDate(TxtDate_From.Text) & ", Date_To = " & AgL.ConvertDate(TxtDate_To.Text) & ", " &
            ...
            With DGL1
                For I = 0 To .Rows.Count - 1
                    If .Item(Col1EntryPoint, I).Value IsNot Nothing Then
                        If .Item(Col1EntryPoint, I).Value <> "" And
                            (Val(.Item(Col1Add_Target, I).Value) <> 0 Or Val(.Item(Col1Edit_Target, I).Value) <> 0 Or Val(.Item(Col1Print_Target, I).Value) <> 0 Or
                            Val(.Item(Col1Sms_Target, I).Value) <> 0 Or Val(.Item(Col1Fax_Target, I).Value) <> 0 Or Val(.Item(Col1Email_Target, I).Value) <> 0) Then

                            Sr = Sr + 1
                            mQry = "Insert Into User_Target_Detail ( Code, Sr, EntryPoint, Add_Target, Edit_Target, Print_Target," &
                                    " Sms_Target, Email_Target, Fax_Target) Values(" &
                                    " '" & mSearchCode & "', " & Sr & "," & AgL.Chk_Text(.Item(Col1EntryPoint, I).Value) & ", " &
                                    " " & Val(.Item(Col1Add_Target, I).Value) & ", " & Val(.Item(Col1Edit_Target, I).Value) & ", " &
                                    " " & Val(.Item(Col1Print_Target, I).Value) & ", " & Val(.Item(Col1Sms_Target, I).Value) & ", " &
                                    " " & Val(.Item(Col1Email_Target, I).Value) & ", " & Val(.Item(Col1Fax_Target, I).Value) & " " &
                                    " ) "
                            AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
                        End If
                    End If
        ...
        Try
            ...
            If DTMaster.Rows.Count > 0 Then
                ...
                With DsTemp.Tables(0)
                    ...
                    End If
                End With
                mQry = "Select User_Target_Detail.* " &
                    " From User_Target_Detail " &
                    " Where Code='" & mSearchCode & "'" &
                    " Order By Sr"
                DsTemp = AgL.FillData(mQry, AgL.GCn)
                With DsTemp.Tables(0)
            ...
            For I = 0 To .Rows.Count - 1
                If .Item(Col1EntryPoint, I).Value IsNot Nothing Then
                    If .Item(Col1EntryPoint, I).Value <> "" Then
                        ...
                        If mDataExists = False And (Val(.Item(Col1Add_Target, I).Value) <> 0 Or Val(.Item(Col1Edit_Target, I).Value) <> 0 Or Val(.Item(Col1Print_Target, I).Value) <> 0 Or
                            Val(.Item(Col1Sms_Target, I).Value) <> 0 Or Val(.Item(Col1Fax_Target, I).Value) <> 0 Or Val(.Item(Col1Email_Target, I).Value) <> 0) Then
                            mDataExists = True
                        End If
                    End If
