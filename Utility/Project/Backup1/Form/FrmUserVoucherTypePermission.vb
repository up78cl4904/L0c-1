...
Public Class FrmUserVoucherTypePermission
    ...
    Private Sub Topctrl1_tbSave() Handles Topctrl1.tbSave
        ...
        Try
            ...
            mQry = "Delete From User_VoucherType_Permission " & _
            ...
            With DGL1
                For I = 0 To .Rows.Count - 1
                    If AgL.XNull(.Item(Col1Code, I).Value).ToString.Trim <> "" And AgL.StrCmp(.Item(Col1Permission, I).Value, AgLibrary.ClsConstant.StrCheckedValue) Then
                        mQry = " INSERT INTO User_VoucherType_Permission (UserName, V_Type,	SNo, Active, SiteList, DivisionList ) " & _
                                " Values( '" & mSearchCode & "', " & AgL.Chk_Text(.Item(Col1Code, I).Value) & "," & Val(.Item(ColSNo, I).Value) & ", 'Y', " & _
    ...
    Private Sub Fill_Grid(ByVal UserName As String)
        Dim I As Integer
        Try
            mQry = " SELECT V.V_Type AS Code, Vt.Short_Name , Vt.Description, " & _
                    " convert(BIT, CASE isnull(U.Active,'N') WHEN 'Y' THEN 1 ELSE 0 END ) AS Permission, U.DivisionList, U.SiteList " & _
                    " FROM " & _
                    " ( " & _
                    " SELECT V_Type, Active, SNo  " & _
                    " FROM User_VoucherType_Permission " & _
                    " WHERE UserName ='SA' And IsNull(Active,'Y') = 'Y' " & _
                    " ) AS V  " & _
                    " LEFT JOIN User_VoucherType_Permission U ON U.V_Type = V.V_Type AND U.UserName ='" & UserName & "' " & _
                    " LEFT JOIN Voucher_Type Vt ON Vt.V_Type = V.V_Type  " & _
