...
Public Class FrmUserControlPermission
            ...
            With DGL1
                For I = 0 To .Rows.Count - 1
                    If AgL.XNull(.Item(Col1_MnuName, I).Value).ToString.Trim <> "" Then
                        ...
                        mQry = "Delete From User_Control_Permission " & _
                                " Where UserName='" & mSearchCode & "' And " & _
                                "       MnuModule=" & AgL.Chk_Text(.Item(Col1_MnuModule, I).Value) & " And " & _
                                "       MnuName=" & AgL.Chk_Text(.Item(Col1_MnuName, I).Value) & " And " & _
                                "       GroupText=" & AgL.Chk_Text(.Item(Col1_ControlName, I).Value) & " "
                        AgL.Dman_ExecuteNonQry(mQry, AgL.GcnMain, AgL.ECmd)

                        If AgL.XNull(.Item(Col1_GroupText, I).Value).ToString.Trim <> "" Then
                            mQry = "Insert Into User_Control_Permission (UserName, MnuModule, MnuName, MnuText, GroupText, GroupName) Values(" & _
                                    " '" & mSearchCode & "'," & AgL.Chk_Text(.Item(Col1_MnuModule, I).Value) & "," & AgL.Chk_Text(.Item(Col1_MnuName, I).Value) & "," & _
                                    " " & AgL.Chk_Text(.Item(Col1_MnuText, I).Value) & ", " & AgL.Chk_Text(.Item(Col1_GroupText, I).Value) & ", " & _
    ...
    Private Sub User_Control_Permission(ByVal mUser As String)
        ...
        Try
            ...
            '        " Where IsNull(U.MnuModule,'')<>'' And U.UserName='" & mUser & "' " & _
            '        " " & IIf(bStrModuleList.Trim <> "", " AND U.MnuModule IN (" & bStrModuleList.Replace("|", "'") & ") ", "") & " " & _
            '        " ORDER BY U.MnuModule"

            mQry = "SELECT Distinct U.MnuModule " & _
                    " FROM User_Control_Permission U" & _
                    " Where IsNull(U.MnuModule,'')<>'' And U.UserName='SA' " & _
                    " " & IIf(bStrModuleList.Trim <> "", " AND U.MnuModule IN (" & bStrModuleList.Replace("|", "'") & ") ", "") & " " & _
                    " ORDER BY U.MnuModule"
            ...
            If DTblModule.Rows.Count > 0 Then
                For J = 0 To DTblModule.Rows.Count - 1
                    ...
                    '        " FROM User_Control_Permission U" & _
                    '        " Where IsNull(U.MnuModule,'')='" & DTblModule.Rows(J)("MnuModule") & "' And U.UserName='" & mUser & "' " & _
                    '        " ORDER BY U.MnuName"
                    mQry = "SELECT Distinct U.MnuName,U.MnuText " & _
                            " FROM User_Control_Permission U" & _
                            " Where IsNull(U.MnuModule,'')='" & DTblModule.Rows(J)("MnuModule") & "' " & _
                            " And U.UserName='SA' " & _
                            " ORDER BY U.MnuText "
    ...
    Private Sub Fill_Grid(ByVal mMnuModule As String, ByVal mParent As String)
        ...
        Try
            ...
            '        " CONVERT(BIT,CASE WHEN Len(IsNull(U.GroupText,'')) > 0 THEN 1 ELSE 0 END) AS [Permission], " & _
            ...
            mSubQry = "SELECT '" & CboUserName.Text & "' AS UserName, U1.MnuName, U1.MnuModule, U1.MnuText, U1.GroupName, U1.GroupText " & _
                        " FROM User_Control_Permission U1 " & _
                        " WHERE U1.UserName='SA' "

            mQry = "SELECT U.MnuName As [Menu Name], U.GroupText As [Control Name], " & _
                    " CONVERT(BIT,CASE WHEN Len(IsNull(U2.GroupText,'')) > 0 THEN 1 ELSE 0 END) AS [Permission], " & _
                    " U.MnuModule, U.MnuText, U.GroupName, U.GroupText " & _
                    " FROM (" & mSubQry & ") As U " & _
                    " Left Join User_Control_Permission U2 On U.UserName = U2.UserName And U.MnuName = U2.MnuName And U.MnuModule = U2.MnuModule And U.GroupName = U2.GroupName And U.GroupText = U2.GroupText " & _
                    " WHERE U.UserName='" & CboUserName.Text & "' And U.MnuModule ='" & mMnuModule & "' And" & _
                    " U.MnuName ='" & mParent & "' " & _
                    " " & IIf(bStrModuleList.Trim <> "", " AND U.MnuModule IN (" & bStrModuleList.Replace("|", "'") & ") ", "") & " " & _
                    " ORDER BY U.GroupText"
