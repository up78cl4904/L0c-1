Public Class ClsMain
    ...
    Public Const ModuleName As String = "Utility"

    Sub New(ByVal AgLibVar As AgLibrary.ClsMain)
        AgL = AgLibVar
        AgIniVar = New AgLibrary.ClsIniVariables(AgL)
        ClsMain_AgTemplate = New AgTemplate.ClsMain(AgL)
    End Sub
    ...
    Public Sub UpdateTableStructure()
        ...
        If AgL.Dman_Execute("Select IfNull(Count(*),0) Cnt FROM Division D Where IfNull(D.SitewiseV_No,0)<>0 And D.Div_Code='" & AgL.PubDivCode & "' ", AgL.GcnMain).ExecuteScalar > 0 Then
            If AgL.Dman_Execute("Select IfNull(Count(*),0) Cnt FROM Company C Where IfNull(C.Site_Code,'')<>'' And C.Div_Code='" & AgL.PubDivCode & "' ", AgL.GcnMain).ExecuteScalar > 0 Then
                AgL.Dman_ExecuteNonQry("Update Company Set Site_Code=Null Where Div_Code='" & AgL.PubDivCode & "' ", AgL.GcnMain)
            End If

            AgL.Dman_ExecuteNonQry("Update Voucher_Type Set SiteWise=1 Where IfNull(SiteWise,0)=0 ", AgL.GCn)

            'Inserting Record in Voucher Prefix Table----------------------------------
            mQry = "Select IfNull(Count(*),0) Cnt " &
            ...
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar = 0 Then
                mQry = "Insert Into Voucher_Prefix(Site_Code,V_Type,Date_From,Date_To,Prefix,Start_Srl_No,Div_Code)" &
            ...
            Else
                mQry = "Insert Into Voucher_Prefix(Site_Code,V_Type,Date_From,Date_To,Prefix,Start_Srl_No,Div_Code)" &
                        " (Select '" & AgL.PubSiteCode & "' As Site_Code, V1.V_Type, V1.Date_From, V1.Date_To, V1.Prefix, 0 As Start_Srl_No,'" & AgL.PubDivCode & "' As Div_Code " &
                        "   From Voucher_Prefix V1 " &
                        "   Where V1.Site_Code='1' And V1.Div_Code='" & AgL.PubDivCode & "' And V1.V_Type Not In " &
            ...
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar = 0 Then
                mQry = "Insert Into Enviro(Id,Site_Code,Div_Code, " &
                        ...
                        " '" & mEnviroId & "', '" & AgL.PubSiteCode & "', '" & AgL.PubDivCode & "', " &
    ...
    Private Sub AddNewTable()
        ...
        Try
            If Not AgL.IsTableExist("Synchronise_Error", AgL.GCn) Then
                mQry = "CREATE TABLE dbo.Synchronise_Error " &
                        "	( " &
                        "	RowId   BIGINT NOT NULL, " &
                        "	Message NVARCHAR (max) COLLATE SQL_Latin1_General_CP1_CI_AS NULL, " &
                        "	CONSTRAINT PK_Synchronise_Error PRIMARY KEY (RowId) " &
                        "	) "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn)
            End If
        ...
        Try
            If Not AgL.IsTableExist("Log_TableRecords", AgL.GCn) Then
                mQry = "CREATE TABLE [dbo].[Log_TableRecords]( " &
                    "	[RowId] [bigint] IDENTITY(1,1) NOT NULL, " &
                    "	[SearchKey] [nvarchar](21) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                    "	[AED] [nvarchar](1) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                    "	[UpdateDate] [datetime] NOT NULL, " &
                    "	[TableName] [nvarchar](50) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                    "	[Site] [nvarchar](max) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                    "	[UploadDate] [smalldatetime] NULL, " &
                    " CONSTRAINT [PK_Log_TableRecords] PRIMARY KEY CLUSTERED  " &
                    "( " &
                    "	[SearchKey] ASC, " &
                    "	[AED] ASC, " &
                    "	[UpdateDate] ASC " &
        ...
        Try
            If Not AgL.IsTableExist("Table_SearchField", AgL.GCn) Then
                mQry = "CREATE TABLE [dbo].[Table_SearchField]( " &
                        "	[RowId] [bigint] IDENTITY(1,1)  NOT NULL, " &
                        "	[TABLE_NAME] [nvarchar](128) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                        "	[SEARCH_FIELD] [nvarchar](128) COLLATE SQL_Latin1_General_CP1_CI_AS NULL " &
                        ") ON [PRIMARY] "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn)
            End If
        Catch ex As Exception
        ...
        Try
            If Not AgL.IsTableExist("Log_TablePermission", AgL.GCn) Then
                mQry = "CREATE TABLE [dbo].[Log_TablePermission]( " &
        ...
        Try
            mQry = "CREATE TABLE dbo.EntryPointPermission " &
                    " ( " &
                    " MnuModule      NVARCHAR (50) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                    " MnuName        NVARCHAR (100) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL, " &
                    " IsOnLineEntry  BIT CONSTRAINT DF_EntryPointPermission_IsOnLineEntry DEFAULT ((0)) NOT NULL, " &
                    " IsOffLineEntry BIT CONSTRAINT DF_EntryPointPermission_IsOffLineEntry DEFAULT ((1)) NOT NULL, " &
                    " CONSTRAINT PK_EntryPointPermission PRIMARY KEY (MnuModule,MnuName) " &
                    " )"
    ...
    Private Sub AddNewField()
        ...
        '============================< Company >==================================================
        AgL.AddNewField(AgL.GcnMain, "Company", "ImageDbName", "nVarChar(50)")
        '============================< ********** >==================================================
    ...
    Sub CreateSqlProcedures()
        ...
        Try
            ...
            mQry = "CREATE PROCEDURE dbo.Create_LogTableRecords 	@TableName NVARCHAR(128), 	@AED NVARCHAR(1), 	@SearchKey NVARCHAR(21), @UniqueKey NVARCHAR(MAX) AS  " &
            "	BEGIN 	 " &
            "		SET NOCOUNT ON; 	 " &
            ...
            "		SET @mCreateLogYn=(SELECT  CreateLogYn FROM Log_TablePermission Limit 1) " &
            "		IF (@mCreateLogYn=1) " &
            "		Begin " &
            "			SET @SearchField=(SELECT Search_Field FROM Table_SearchField WHERE TABLE_NAME =@TableName) 	 " &
            "			SET @UniqueField=(SELECT UniqueField FROM Table_SearchField WHERE TABLE_NAME =@TableName) 	 " &
            "			SET @UpdateDate=(SELECT GetDate())	     	 	 " &
            "			INSERT INTO Log_TableRecords 		    " &
            "			(SearchKey,AED,UpdateDate,TableName,Site,UploadDate,SearchField, UniqueKey, UniqueField) 	VALUES (@searchkey,@aed,@updatedate,@tablename,'',Null,@searchfield, @UniqueKey, @UniqueField)	  " &
            "		end " &
            "	END  "
    ...
    Sub CreateSqlTriggers()
        ...
        mQry = "SELECT * FROM Table_SearchField WHERE IfNull(Search_Field,'')<>''"
        ...
        With DtTemp
            For I = 0 To DtTemp.Rows.Count - 1
                ...
                mQry = "CREATE TRIGGER Tr_" & AgL.XNull(.Rows(I)("Table_Name")) & "_A ON " & AgL.XNull(.Rows(I)("Table_Name")) & "  " &
                        "FOR INSERT " &
                        "AS " &
                        "DECLARE @Code NVARCHAR(21) " &
                        "DECLARE @UniqueKey NVARCHAR(Max) " &
                        "DECLARE @UDate DateTime " &
                        "IF EXISTS (SELECT * FROM Inserted) 	 " &
                        "	BEGIN  " &
                        "		SET @Code=(SELECT  " & AgL.XNull(.Rows(I)("Search_Field")) & " FROM Inserted i Limit 1) " &
                        "		SET @UniqueKey=(SELECT  " & AgL.XNull(.Rows(I)("UniqueField")) & " FROM Inserted i Limit 1) " &
                        "       Set @UDate = (SELECT GetDate()) " &
                        "       IF NOT EXISTS(SELECT * FROM Log_TableRecords WHERE SearchKey = @Code And UniqueKey=@UniqueKey AND TableName = '" & AgL.XNull(.Rows(I)("Table_Name")) & "' And AED='A' AND UpdateDate =@UDate) " &
                        "           BEGIN " &
                        "		        EXEC Create_LogTableRecords  '" & AgL.XNull(.Rows(I)("Table_Name")) & "','A',@Code,@UniqueKey " &
                        "           End " &
                        "	END "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn)

                mQry = "CREATE TRIGGER Tr_" & AgL.XNull(.Rows(I)("Table_Name")) & "_E ON " & AgL.XNull(.Rows(I)("Table_Name")) & "  " &
                        "FOR Update " &
                        "AS " &
                        "DECLARE @Code NVARCHAR(21) " &
                        "DECLARE @UniqueKey NVARCHAR(Max) " &
                        ...
                        "		SET @UniqueKey=(SELECT " & AgL.XNull(.Rows(I)("UniqueField")) & " FROM Inserted i Limit 1) " &
                        "       Set @UDate = (SELECT GetDate()) " &
                        "       IF NOT EXISTS(SELECT * FROM Log_TableRecords WHERE SearchKey = @Code AND TableName = '" & AgL.XNull(.Rows(I)("Table_Name")) & "' And AED='E' AND UpdateDate =@UDate) " &
                        "           BEGIN " &
                        "		        EXEC Create_LogTableRecords  '" & AgL.XNull(.Rows(I)("Table_Name")) & "','E',@Code,@UniqueKey " &
                        "           End " &
                        "	END "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn)

                mQry = "CREATE TRIGGER Tr_" & AgL.XNull(.Rows(I)("Table_Name")) & "_D ON " & AgL.XNull(.Rows(I)("Table_Name")) & "  " &
                        "FOR Delete " &
                        "AS " &
                        "DECLARE @Code NVARCHAR(21) " &
                        "DECLARE @UniqueKey NVARCHAR(Max) " &
                        ...
                        "	BEGIN  " &
                        "		SET @Code=(SELECT " & AgL.XNull(.Rows(I)("Search_Field")) & " FROM Deleted i Limit 1) " &
                        "		SET @UniqueKey=(SELECT " & AgL.XNull(.Rows(I)("UniqueField")) & " FROM Inserted i Limit 1) " &
                        "       Set @UDate = (SELECT GetDate()) " &
                        "       IF NOT EXISTS(SELECT * FROM Log_TableRecords WHERE SearchKey = @Code AND TableName = '" & AgL.XNull(.Rows(I)("Table_Name")) & "' And AED='D' AND UpdateDate =@UDate) " &
                        "           BEGIN " &
                        "		        EXEC Create_LogTableRecords  '" & AgL.XNull(.Rows(I)("Table_Name")) & "','D',@Code,@UniqueKey " &
                        "           End " &
                        "	END "
                AgL.Dman_ExecuteNonQry(mQry, AgL.GCn)
