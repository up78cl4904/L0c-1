Public Class FrmVoucherEntry
    ...
    Private Sub FIniMaster(Optional ByVal BytDel As Byte = 0, Optional ByVal BytRefresh As Byte = 1)
        Topctrl1.FIniForm(DTMaster, AgL.GCn, "Select DocId As SearchCode From LedgerM Where Site_Code='" & AgL.PubSiteCode & "' And V_Prefix='" & AgL.PubCompVPrefix & "' Order By V_Date,V_Type,Cast((Case When IsNumeric(RecId)=1 Then RecId Else 0 End) As BigInt)", , , , , BytDel, BytRefresh)
    End Sub
    ...
    Public Sub MoveRec()
        ...
        If DTMaster.Rows.Count > 0 Then
            ...
            ADTemp = New SqlClient.SqlDataAdapter("Select LM.DocId,LM.V_Type,LM.v_Prefix,LM.Site_Code,LM.V_No,LM.V_Date,LM.SubCode,  " & _
                    ...
                    "From LedgerM LM Left Join SubGroup SG On LM.SubCode=SG.SubCode Left Join " & _
            ...
            If DTTemp.Rows.Count > 0 Then
                ...
                ADTemp = New SqlClient.SqlDataAdapter("Select LG.SubCode,SG.Name As AcName,SG.ManualCode,LG.V_SNo,CCM.Name As CCName,LG.CostCenter, " & _
                    ...
                    "From Ledger LG Left Join SubGroup SG On LG.SubCode=SG.SubCode Left Join " & _
    ...
    Private Sub FGenerateNo(Optional ByVal BlnGenerateOnlyNo As Boolean = False)
        ...
        If RFNumberSystem <> ClsMain.RecIdFormat.DD_MM And RFNumberSystem <> ClsMain.RecIdFormat.MM Then
            TxtRecId.Text = CMain.FGetMaxNo("Select IsNull(Max(Convert(Bigint,LM.RecId)),0)+1 As Mx From LedgerM LM Where IsNumeric(LM.RecId)<>0 And LM.V_Prefix='" & TxtVNo.Tag & "' And LM.V_Type='" & TxtType.Tag & "' And LM.Site_Code='" & AgL.PubSiteCode & "' ", AgL.GCn)
        End If
        ...
        Try
            If DTMaster.Rows.Count > 0 Then
                If MsgBox(" Are You Sure To Delete ? ", MsgBoxStyle.YesNo + MsgBoxStyle.Question) = vbYes Then
                    ...
                    If Not CMain.FGetMaxNo("Select Count(DocId) From LedgerM Where DocId='" & StrDocID & "' And IsNull(PostedBy,'')='' ", AgL.GCn) > 0 Then MsgBox("Corresponding Records Exist") : Exit Sub
    ...
    Private Sub Topctrl1_tbEdit() Handles Topctrl1.tbEdit
        ...
        If DTMaster.Rows.Count > 0 Then
            ...
            If Not CMain.FGetMaxNo("Select Count(DocId) From LedgerM Where DocId='" & AgL.XNull(DTMaster.Rows(BMBMaster.Position).Item("SearchCode")) & "' And IsNull(PostedBy,'')='' ", AgL.GCn) > 0 Then MsgBox(ClsMain.MsgEditChk) : Topctrl1.FButtonClick(99) : Exit Sub
                        ...
                        If UBound(SVTMain.LAdjVar) > 0 Then
                            For J = 0 To UBound(SVTMain.LAdjVar)
                                If Val(SVTMain.LAdjVar(J).DblAdjustment) > 0 Then
                                    GCnCmd.CommandText = "Insert Into " & StrLedgerAdj & "(Adj_Type, Vr_DocId,Vr_V_SNo,Adj_DocId,Adj_V_SNo,Amount,Site_Code) Values " & _
                                                 " ('" & (SVTMain.LAdjVar(J).StrAdj_Type) & "','" & (StrDocId) & "', " & _
                                                 " " & Int_Prv_V_SNo & ",'" & (SVTMain.LAdjVar(J).StrDocId) & "', " & _
                                                 " " & Val(SVTMain.LAdjVar(J).StrV_SNo) & ", " & _
                                                 " " & IIf(AgL.StrCmp(SVTMain.LAdjVar(J).StrAdjustmentDrCr, "Cr"), -SVTMain.LAdjVar(J).DblAdjustment, SVTMain.LAdjVar(J).DblAdjustment) & ", " & _
                                                 " '" & AgL.PubSiteCode & "')"
                                    GCnCmd.ExecuteNonQuery()
                                End If
                            Next
...
LblRecursive:
        ...
        Select Case sender.name
            ...
            Case BtnPostedBy.Name
                If DTMaster.Rows.Count > 0 Then
                    ...
                    If Not CMain.FGetMaxNo("Select Count(DocId) From LedgerM Where DocId='" & StrDocID & "' And IsNull(PostedBy,'')=''", AgL.GCn) > 0 Then
    ...
    '    StrSQL = StrSQL + "LG.Narration, IsNull(" & StrFieldContra & ",0) As BillAmt,0 As Adjusted,LA.Amount As Adjustment	"
