...
Public Class FrmDisplayHierarchy
    ...
    Private Sub FBalanceSheet_Disp()
        ...
        Try
            ...
            StrCondition1 = " Where LG.V_Date <= '" & CDate(DHSMain.StrToDate).ToString("u") & "' And LG.Site_Code In (" & DHSMain.StrSiteCode & ")"
    ...
    Private Function FGetTRDDataTable() As DataTable
        ...
        Try
            StrCondition1 = " Where LG.V_Date <= '" & CDate(DHSMain.StrToDate).ToString("u") & "' And LG.Site_Code In (" & DHSMain.StrSiteCode & ") "
            StrCondition1 += " And LG.V_Date >= (Case When Ag.GroupNature in ('R','E') Then '" & CDate(AgL.PubStartDate).ToString("u") & "' Else '1900/Jan/01' End) "
    ...
    Private Sub FTrailBalance_Disp()
        ...
        Try
            ...
            LblDisplayDate.Text = "As On : " & DHSMain.StrToDate
            LblDisplaySite.Text = DHSMain.StrSiteName
            If UCase(DHSMain.StrZeroBalace) = "N" Then StrConditionZeroBal = "Having (Round(IfNull(Sum(LG.AmtDr),0),2)-Round(IfNull(Sum(LG.AmtCr),0),2)) <> 0 "
            StrCondition1 = " Where LG.V_Date <= '" & CDate(DHSMain.StrToDate).ToString("u") & "' And LG.Site_Code In (" & DHSMain.StrSiteCode & ")"
            StrCondition1 += " And LG.V_Date >= (Case When Ag.GroupNature in ('R','E') Then '" & CDate(AgL.PubStartDate).ToString("u") & "' Else '1900/Jan/01' End) "
            '========== For Detail Section =======
            ...
            StrSQLQuery = StrSQLQuery + "(IfNull(Sum(Cast(LG.AmtDr as Float)),0.0)-IfNull(Sum(LG.AmtCr),0.0)) Else 0 End) As AmtDr, "
            StrSQLQuery = StrSQLQuery + "(Case When (IfNull(Sum(LG.AmtCr),0)-IfNull(Sum(LG.AmtDr),0))>0 Then "
            StrSQLQuery = StrSQLQuery + "(IfNull(Sum(LG.AmtCr),0)-IfNull(Sum(LG.AmtDr),0)) Else 0 End) As AmtCr "
            StrSQLQuery = StrSQLQuery + "From Ledger LG Left Join SubGroup SG On LG.SubCode=SG.SubCode  Left Join "
            StrSQLQuery = StrSQLQuery + "AcGroup AG On AG.GroupCode=SG.GroupCode Left Join "
    ...
    Private Sub FDTrailBalance_Disp()
        ...
        Try
            ...
            If UCase(DHSMain.StrZeroBalace) = "N" Then StrConditionZeroBal = "Having (IfNull(Sum(LG.AmtDr),0)-IfNull(Sum(LG.AmtCr),0)) <> 0 "
            ...
            StrSQLQuery += "From ( "
            StrSQLQuery += "Select IfNull(SG.SubCode,'') As SubCode, "
    ...
    Private Sub FDisplay_Level_Group(ByVal StrForCode As String, ByVal StrForName As String)
        ...
        Try
            ...
            StrSQLQuery += "When IfNull(AG.GroupUnder,'')='" & StrForCode & "' Then 'A+' || IfNull(AG.GroupCode,'') "
            ...
            StrSQLQuery += "When IfNull(AG.GroupUnder,'')='" & StrForCode & "' Then IfNull(AG.GroupName,'') "
    ...
    Public Sub FForward(Optional ByVal IntRow As Integer = 0, Optional ByVal IntCol As Integer = 0, Optional ByVal DspType As ClsStructure.DisplayType = ClsStructure.DisplayType.BalanceSheet)
        ...
        ElseIf IntLevel > 0 And UCase(FGMain(GGR_SG, IntRow).Value) <> "T" And UCase(FGMain(GGR_SG, IntRow).Value) <> UCase("") And UCase(FGMain(GGR_SG, IntRow).Value) <> Nothing Then
            If LblDisplay.Tag = CnsBalanceSheet Or LblDisplay.Tag = CnsProfitAndLoss Then
                ...
                End Select
            Else
                StrForCode = FGMain(GGRCode, IntRow).Value.ToString()
                StrForName = FGMain(GGRName, IntRow).Value.ToString()
            End If
