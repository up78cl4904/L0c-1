Public Class FrmVoucherEntryBackup
    ...
    '' ''            Topctrl1.FIniForm(DTMaster, AgL.GCn, "Select DocId As SearchCode From LedgerM_Temp Where Site_Code='" & agl.PubSiteCode & "' And IfNull(PostedBy,'')='' And V_Prefix='" & Agl.PubCompVPrefix & "' Order By V_Date,V_Type,Cast((Case When IsNumeric(RecId)=1 Then RecId Else 0 End) As BigInt)", , , , , BytDel, BytRefresh)
    '' ''        End If
    '' ''    End Sub
    ...
    '' ''            ADTemp = New SqliteDataAdapter("Select LM.DocId,LM.V_Type,LM.v_Prefix,LM.Site_Code,LM.V_No,LM.V_Date,LM.SubCode,  " & _
    ...
    '' ''                End If
    '' ''                DTTemp.Clear()
    '' ''                ADTemp = New SqliteDataAdapter("Select LG.SubCode,SG.Name As AcName,SG.ManualCode,LG.V_SNo,CCM.Name As CCName,LG.CostCenter, " & _
    ...
    '' ''                        'For TDS
    '' ''                        DTTemp1.Clear()
    '' ''                        ADTemp = New SqliteDataAdapter("Select LG.FormulaString,LG.SubCode,SG.Name As AcName,SG.ManualCode, " & _
    ...
    '' ''            TxtRecId.Text = CMain.FGetMaxNo("Select IfNull(Max(Convert(Bigint,LM.RecId)),0)+1 As Mx From LedgerM_Temp LM Where IsNumeric(LM.RecId)<>0 And LM.V_Prefix='" & TxtVNo.Tag & "' And LM.V_Type='" & TxtType.Tag & "' And LM.Site_Code='" & agl.PubSiteCode & "' ", Agl.Gcn)
    '' ''        End If
    ...
    '' ''                    If Not CMain.FGetMaxNo("Select Count(DocId) From LedgerM_Temp Where DocId='" & StrDocID & "' And IfNull(PostedBy,'')='' ", Agl.Gcn) > 0 Then MsgBox("Corresponding Records Exist") : Exit Sub
    ...
    '' ''            If Not CMain.FGetMaxNo("Select Count(DocId) From LedgerM_Temp Where DocId='" & Agl.Xnull(DTMaster.Rows(BMBMaster.Position).Item("SearchCode")) & "' And IfNull(PostedBy,'')='' ", Agl.Gcn) > 0 Then MsgBox(ClsMain.MsgEditChk) : Topctrl1.FButtonClick(99) : Exit Sub
    ...
    '' ''        Try
    '' ''            If FormWorkAs = ClsStructure.EntryType.ForEntry Then
    '' ''                agl.PubFindQry = "Select LM.DocId,RTrim(LM.RecId) As VNo,LM.V_Prefix As Prefix,LM.V_Type,VT.Description,convert(nvarchar(12),LM.V_Date,103) As VDate,IfNull(SG.Name,'') As Account,IfNull(LM.PostedBy,'') As PostedBy " & _
    ...
    '' ''            ElseIf FormWorkAs = ClsStructure.EntryType.ForPosting Then
    '' ''                agl.PubFindQry = "Select LM.DocId,RTrim(LM.RecId) As VNo,LM.V_Prefix As Prefix,LM.V_Type,VT.Description,convert(nvarchar(12),LM.V_Date,103) As VDate,IfNull(SG.Name,'') As Account,IfNull(LM.PostedBy,'') As PostedBy " & _
    '' ''                                      "From LedgerM_Temp LM Left Join Voucher_Type VT On LM.V_Type=VT.V_Type Left Join SubGroup SG On LM.SubCode=SG.SubCode Where LM.Site_Code='" & agl.PubSiteCode & "' And IfNull(PostedBy,'')='' And LM.V_Prefix='" & Agl.PubCompVPrefix & "' "
    '' ''            End If
    ...
    '' ''        AgL.ADMain = New SqliteDataAdapter("Select SG.SubCode As SearchCode,SG.Name,SG.ManualCode," & _
    '' ''        "IfNull(CT.CityName,''),SG.Nature,CCM.Name As CCName,SG.CostCenter,AG.GroupName " & _
    ...
    '' ''                    If Not CMain.FGetMaxNo("Select Count(DocId) From LedgerM_Temp Where DocId='" & StrDocID & "' And IfNull(PostedBy,'')=''", Agl.Gcn) > 0 Then
