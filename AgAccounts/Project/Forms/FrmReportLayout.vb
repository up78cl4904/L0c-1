...
#End Region
    ...
    Private Sub FIntCalForDebtors()
        ...
        StrSQLQuery += "From "
        StrSQLQuery += "( "
        StrSQLQuery += "Select	LG.DocId As Adj_DocId,LG.V_SNo As Adj_V_SNo,LG.V_Type,Cast(LG.V_No as Varchar) as V_No,LG.V_Date, "
    ...
    Private Sub FINI_BillWiseAdj()
        ...
        FRH_Multiple(2) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable(
                          ...
                          "IfNull(SG.Zone,'') From SubGroup SG Left Join " &
                          "AcGroup AG On AG.GroupCode=SG.GroupCode Left Join " &
                          "City CT On SG.CityCode=CT.CityCode Left Join " &
                          "ZoneMast ZM On ZM.Code=SG.Zone Where " & AgL.PubSiteListCharIndex & " " &
                          "Order By SG.Name",
    ...
    Private Sub FINI_GTAReg()
        ...
        FRH_Multiple(2) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable("Select  'o'  As Tick,max(ST.Consignor) As Code, S.Name,max(S.ManualCode) as ManualCode FROM STaxTrn ST " &
                          ...
                          "Where " & AgL.PubSiteListCharIndex & "" &
    ...
    Private Sub FIni_DailyTransBook()
        ...
        FRH_Multiple(3) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable(
                          "Select 'o' As Tick,SG.SubCode,SG.Name From SubGroup SG  Where SG.SiteList Like '%|" & AgL.PubSiteCode & "|%' Order by SG.Name",
    ...
    Private Sub FINI_MonthlyLedgerSummaryFull()
        ...
        FSetValue(2, "Account Name", FGDataType.DT_Selection_Multiple, FilterCodeType.DTString, "All", False)
        FRH_Multiple(2) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable(
                          "Select 'o' As Tick,SG.SubCode,SG.Name,Sg.ManualCode,IfNull(CT.CityName,'') From SubGroup SG Left Join City CT On SG.CityCode=CT.CityCode where " & AgL.PubSiteListCharIndex & " Order By SG.Name",
    ...
    Private Sub FMonthlyLedgerSummaryFull()
        ...
        StrSQLQuery += StrCondition1 + StrConditionsite
        StrSQLQuery += "Group By IfNull(SG.SubCode,''),(datename(Month ,LG.V_date) +' ' || (datename(year ,LG.V_date))) "
        StrSQLQuery += " ) As Tmp "
    ...
    Private Sub FINI_InterestLedger()
        ...
        FRH_Multiple(5) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable(
                          "Select 'o' As Tick,SG.SubCode,SG.Name,Sg.ManualCode From SubGroup SG Where " & AgL.PubSiteListCharIndex & " Order By SG.Name",
                          AgL.GCn)), "", 600, 660, , , False)
        FRH_Multiple(5).FFormatColumn(0, "Tick", 40, DataGridViewContentAlignment.MiddleCenter, True)
        FRH_Multiple(5).FFormatColumn(1, , 0, , False)
        FRH_Multiple(5).FFormatColumn(2, "Name", 440, DataGridViewContentAlignment.MiddleLeft)
    ...
    Private Sub FINI_FBTReport()
        ...
        FRH_Multiple(2) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable("Select 'o' As Tick,SG.SubCode,SG.Name,SG.ManualCode From SubGroup  SG " &
                          "Where Nature='Expenses' and " & AgL.PubSiteListCharIndex & "" &
    ...
    Private Sub FINI_BillWsOS(ByVal StrReportFor As String)
        ...
        FRH_Multiple(2) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable("Select 'o' As Tick,SG.SubCode,SG.Name,SG.ManualCode,AG.GroupName From SubGroup  SG Left Join  " &
                          ...
                          "AGP.GroupUnder='" & StrReportForCode & "') Or SG.GroupCode='" & StrReportForCode & "' and " & AgL.PubSiteListCharIndex & "" &
        ...
        FRH_Multiple(3) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable(
                          "Select 'o' As Tick,Zm.Code,Zm.Name From ZoneMast Zm  Order By Zm.Name",
    ...
    Private Sub FIni_Ledger()
        ...
        FRH_Multiple(3) = New DMHelpGrid.FrmHelpGrid_Multi(New DataView(CMain.FGetDatTable(
                          ...
                          "AcGroup AG On AG.GroupCode=SG.GroupCode Left Join " &
                          "City CT On SG.CityCode=CT.CityCode where " & AgL.PubSiteListCharIndex & " " &
                          "Order By SG.Name",
    ...
    Private Sub FIni_Bank_CashBook(ByVal StrTypeIn As String)
        ...
        FRH_Single(3) = New DMHelpGrid.FrmHelpGrid(New DataView(CMain.FGetDatTable(
                          "Select SG.SubCode,SG.Name From SubGroup SG  Where SG.SiteList Like '%|" & AgL.PubSiteCode & "|%' And " &
...
LblForLastRecord:
    ...
    Private Sub FBank_CashBookSingle()
        ...
        SQL = "Select LG.DocId,LG.AmtDr,LG.AmtCr,LG.V_Date,(Cast(LG.RecID as Varchar)+'-'+LG.Site_Code) As RecId,LG.V_Type, "
        SQL += "LG.Chq_No,LG.Chq_Date,LG.SubCode,LG.ContraSub,LG.Narration, "
...
LblForLastRecord:
    ...
    Private Sub FAgeing()
        ...
        StrSQLQuery = StrSQLQuery + " max(Sg.Name) As PName,Max(SM.Name) As Division,0 As AmtPR_Contra,Max(AG.GroupName) as GroupName, IfNull(Max(SG.CreditLimit),0) as CreditLimit ,IfNull(Max(SG.DueDays),0) as DueDays,IfNull(Max(CT.CityName),'') AS CityName    From Ledger As Lg "
        ...
        StrSQLQuery = StrSQLQuery + " IfNull(Max(SG.CreditLimit),0) as CreditLimit ,IfNull(Max(SG.DueDays),0) as DueDays,IfNull(Max(CT.CityName),'') AS CityName   From Ledger As Lg "
        StrSQLQuery = StrSQLQuery + " Left Join SubGroup As SG On Lg.SubCode=Sg.SubCode"
        StrSQLQuery = StrSQLQuery + " Left Join AcGroup As AG On ag.GroupCode=Sg.GroupCode "
        StrSQLQuery = StrSQLQuery + " Left Join Sitemast As SM On SM.Code=Lg.site_Code "
        StrSQLQuery = StrSQLQuery + " LEFT JOIN City CT ON CT.CityCode =SG.CityCode "
    ...
    Private Sub FFixedAssetRegister()
        ...
        StrCondition2 = "WHERE AR.V_Date Between  '" & CDate(AgL.PubStartDate).ToString("u") & "' And  '" & CDate(AgL.PubEndDate).ToString("u") & "' "
        StrCondition3 = "And AT.V_Date Between  '" & CDate(AgL.PubStartDate).ToString("u") & "' And  '" & CDate(AgL.PubEndDate).ToString("u") & "' "
    ...
    Private Sub FSalesTaxClubbing()
        ...
        mTotalVatPayble = AgL.VNull(AgL.Dman_Execute("SELECT Sum(L.AmtDr) - Sum(L.AmtCr)  AS Balance " &
                ...
                " And L.V_Date Between '" & DateValue(CDate(FGMain(GFilter, 0).Value).ToString("u")) & "' And '" & DateValue(CDate(FGMain(GFilter, 1).Value).ToString("u")) & "' ", AgL.GCn).ExecuteScalar)
        ...
        mQry = "SELECT L.SubCode, Max(Sg.Name) As AcName, Max(Sg.ManualCode) As ManualCode, " &
                ...
                " LEFT JOIN AcGroup G ON Sg.GroupCode = G.GroupCode " &
                " WHERE G.GroupName = 'Vat' " &
                " And L.V_Date Between '" & DateValue(CDate(FGMain(GFilter, 0).Value).ToString("u")) & "' And '" & DateValue(CDate(FGMain(GFilter, 1).Value).ToString("u")) & "' " &
                " GROUP BY L.SubCode "
        mQry += " UNION ALL "
