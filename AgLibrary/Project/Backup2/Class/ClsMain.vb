...
#End Region
    ...
    Public GcnImage As SqlClient.SqlConnection
...
#End Region
    ...
    Public Function FunIsVTypeAffect_FA(ByVal StrV_Type As String, ByVal StrConnectionString As String) As Boolean
        ...
        Try
            ...
            mQry = "SELECT IsNull(Vt.Affect_FA,1) AS Affect_FA " & _
                    ...
                    For I = 0 To UBound(RecAry)
                        If RecAry(I).AmtCr + RecAry(I).AmtDr <> 0 Then
                            ...
                            GSQL = "insert into ledger(" _
                                ...
                                ", " & Chk_Text(XNull(TblTemp.Rows(0)("GroupCode"))) & ", " & Chk_Text(XNull(TblTemp.Rows(0)("GroupNature"))) & ", " & Chk_Text(mRecId) & ", " & _
                                " " & Chk_Text(mU_Name) & "," & ConvertDate(mU_EntDt) & ",'" & EMode & "')"
...
err1:
    ...
    Public Function GetMaxId(ByVal mTableName As String, ByVal mPrimaryField As String, ByVal mConn As System.Data.SqlClient.SqlConnection, _
        ...
        mCmd = Dman_Execute("Select Case When IsNull(Max(Convert(BigInt, SubString(" & mPrimaryField & ",3,Len(" & mPrimaryField & ")))) ,0) > 0 Then IsNull(Max(Convert(BigInt, SubString(" & mPrimaryField & ",3,Len(" & mPrimaryField & ")))) ,0) Else 0 End  +1 As MaxId From " & mTableName & " With (NoLock)  " & _
    ...
    Public Function Check_Entry(ByVal TableName As String, ByVal FieldName As String, ByVal FieldValue As String, ByVal FieldDataType As FieldType, ByVal Message As String, ByVal mConn As System.Data.SqlClient.SqlConnection, Optional ByVal ShowDetail As Boolean = False) As Boolean
        ...
        Select Case FieldDataType
            Case FieldType.StringType
                ECmdTm = Dman_Execute("Select Count(" & FieldName & ") from " & TableName & "  With (NoLock) where " & FieldName & "='" & FieldValue & "'", mConn)
            Case FieldType.NumType
                ECmdTm = Dman_Execute("Select Count(" & FieldName & ") from " & TableName & "  With (NoLock) where " & FieldName & "=" & FieldValue & "", mConn)
            Case FieldType.DateType
                ECmdTm = Dman_Execute("Select Count(" & FieldName & ") from " & TableName & "  With (NoLock) where " & FieldName & "=" & ConvertDate(FieldValue) & "", mConn)
            Case Else
                ECmdTm = Nothing
        End Select
    ...
    Public Function IsFunctionExist(ByVal FunctionName As String, ByVal mConn As SqlClient.SqlConnection, Optional ByVal DropProcedure As Boolean = False) As Boolean
        Dim mQry As String
        Dim DtTemp As DataTable = Nothing
        Dim I As Integer
    ...
    Public Function FunCreateSubGroup_Log(ByVal SqlConn As SqlClient.SqlConnection, ByVal SqlCmd As SqlClient.SqlCommand, ByVal StrSubCode As String) As String
        ...
        Try
            bQry = "SELECT Convert(NVARCHAR(36),Sg.UID) AS Uid " & _
            ...
            If bStrUID.Trim <> "" Then
                ...
                bQry = "SELECT IsNull(Count(*),0) AS Cnt FROM SubGroup_Log Sg WITH (NoLock) " & _
            ...
            If bBlnIsSubGroupLogExists = False Then
                bQry = "INSERT INTO SubGroup_Log (SubCode, Site_Code, Div_Code, Name, DispName, GroupCode, GroupNature, ManualCode, Nature, Party_Type, U_Name, U_EntDt, U_AE, Edit_Date, ModifiedBy, CommonAc, UpLoadDate, ChequeReport, EntryBy, EntryDate, EntryType, EntryStatus, ApproveBy, ApproveDate, Status, UID, " & _
                        " SalesTaxPostingGroup, ExcisePostingGroup, EntryTaxPostingGroup, " & _
                        " ApprovedBy, StCategory, SiteStr, STRegNo, ECCNo, EXREGNO, CEXRANGE, CEXDIV, COMMRATE, VATNo, " & _
                        " SiteList, NamePrefix, Add1, Add2, Add3, CityCode, CountryCode, PIN, Phone, Mobile, FAX, EMail, CSTNo, LSTNo, TINNo, PAN, TDS_Catg, ActiveYN, CreditLimit, CreditDays, DueDays, ContactPerson, " & _
                        " PAdd1, PAdd2, PAdd3, PCityCode, PCountryCode, PPin, PPhone, PMobile, PFax, Curr_Bal, OpBal_DocId, FatherName, FatherNamePrefix, HusbandName, HusbandNamePrefix, DOB, Remark, Location ) " & _
                        " (SELECT SubCode, Site_Code, Div_Code, Name, DispName, GroupCode, GroupNature, ManualCode, Nature, Party_Type, U_Name, U_EntDt, U_AE, Edit_Date, ModifiedBy, CommonAc, UpLoadDate, ChequeReport, EntryBy, EntryDate, EntryType, 'Open' As EntryStatus, ApproveBy, ApproveDate, 'Active' AS Status, UID, " & _
                        " SalesTaxPostingGroup, ExcisePostingGroup, EntryTaxPostingGroup, " & _
                        " ApprovedBy, StCategory, SiteStr, STRegNo, ECCNo, EXREGNO, CEXRANGE, CEXDIV, COMMRATE, VATNo, " & _
                        " SiteList, NamePrefix, Add1, Add2, Add3, CityCode, CountryCode, PIN, Phone, Mobile, FAX, EMail, CSTNo, LSTNo, TINNo, PAN, TDS_Catg, ActiveYN, CreditLimit, CreditDays, DueDays, ContactPerson, " & _
                        " PAdd1, PAdd2, PAdd3, PCityCode, PCountryCode, PPin, PPhone, PMobile, PFax, Curr_Bal, OpBal_DocId, FatherName, FatherNamePrefix, HusbandName, HusbandNamePrefix, DOB, Remark, Location " & _
                        " FROM dbo.SubGroup WITH (Nolock) " & _
                        " WHERE SubCode = " & AglObj.Chk_Text(StrSubCode) & " " & _
                        " )"
                AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
            End If
            ...
            bQry = "SELECT IsNull(Count(*),0) AS Cnt " & _
                    " FROM dbo.SubGroup_Image WITH (NoLock) " & _
            ...
            bQry = "SELECT IsNull(Count(*),0) AS Cnt " & _
                    " FROM dbo.SubGroup_Image_Log WITH (NoLock) " & _
                    " WHERE SubCode = " & AglObj.Chk_Text(StrSubCode) & " "
            ...
            If bBlnIsImageExists Then
                bQry = "UPDATE SubGroup_Image " & _
                        " SET UID = " & AglObj.Chk_Text(bStrUID) & " " & _
                        " WHERE SubCode  = " & AglObj.Chk_Text(StrSubCode) & " " & _
                        " AND IsNull(Convert(NVARCHAR(36),UID),'') <> " & AglObj.Chk_Text(bStrUID) & " "
                AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
            End If

            If bBlnIsImageLogExists Then
                bQry = "UPDATE SubGroup_Image_Log " & _
                        " SET UID = " & AglObj.Chk_Text(bStrUID) & " " & _
                        " WHERE SubCode  = " & AglObj.Chk_Text(StrSubCode) & " " & _
                        " AND IsNull(Convert(NVARCHAR(36),UID),'') <> " & AglObj.Chk_Text(bStrUID) & " "
                AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
            ...
            Else
                If bBlnIsImageExists Then
                    bQry = "INSERT INTO SubGroup_Image_Log (SubCode, Photo, Signature, UID) " & _
                            " (SELECT SubCode, Photo, Signature, UID FROM dbo.SubGroup_Image WITH (NoLock) " & _
                            " WHERE SubCode = " & AglObj.Chk_Text(StrSubCode) & " " & _
                            " )"
                    AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
                End If
...
#Region "Update Table Structure Functions"
    ...
    Public Function FExecuteDBScript(ByVal ModuleTable() As LITable, ByVal Conn As SqlConnection) As LIException()
    ...
    'Note:- Adding/Altering Tables In Database
    Public Sub FSynchronizeTables(ByRef ModuleTable() As LITable, ByRef LIExcpAry() As LIException, ByVal Conn As SqlConnection)
        Dim I As Integer
        Dim J As Integer
    ...
    Public Sub FSetPK_FK(ByRef StrSQL As String, ByRef LIExcpAry() As LIException, ByVal Gcn As SqlConnection)
        Dim I As Integer
        Dim DTTemp As DataTable
        Dim FrmPGB As New FrmProgressBar
    ...
    'Note:- Droping Constraints PK/FK/Default Values
    Public Sub FDropKeys(ByRef StrSQL As String, ByRef LIExcpAry() As LIException, ByVal Gcn As SqlConnection)
        Dim I As Integer
        Dim DTTemp As DataTable
        Dim FrmPGB As New FrmProgressBar
    ...
    Private Function FCreateTable(ByVal ModuleTable As LITable, ByVal Gcn As SqlConnection) As LIException
        Dim DTTemp As DataTable
        Dim SQLCmd As New SqlClient.SqlCommand
        Dim LIExpRtn As New LIException
    ...
    Private Function FCreateColumn(ByVal StrTableName As String, ByVal ModuleTableCol As LIColumn, ByVal Gcn As SqlConnection) As LIException
        Dim DTTemp As DataTable
        Dim SQLCmd As New SqlClient.SqlCommand
    ...
    Private Function FDropColumn(ByVal StrTableName As String, ByVal StrColName As String, ByVal Gcn As SqlConnection) As LIException
        Dim SQLCmd As New SqlClient.SqlCommand
