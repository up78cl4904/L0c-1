...
#Region "Public Declaration"
    ...
    Public GCn As SQLiteConnection
    Public GcnRead As SQLiteConnection
...
#End Region
    ...
    Public GcnMain As SQLiteConnection  ' SqliteConnection
    Public ECompConn As SQLiteConnection 'SqliteConnection
    'Public GCnAdo As ADODB.Connection
    'Public GcnMainAdo As ADODB.Connection
    Public GCnRep As SQLiteConnection ' SqliteConnection
    Public GcnSiteComp As SQLiteConnection 'SqliteConnection
    Public GcnSite As SQLiteConnection 'SqliteConnection
    Public GcnSiteRead As SQLiteConnection 'SqliteConnection
    Public GcnLibrary As SQLiteConnection 'SqliteConnection
    Public GcnImage As SQLiteConnection 'SqliteConnection

    Public ECmd As SQLiteCommand  'SqliteCommand
    Public ECompCmd As SQLiteCommand  'SqliteCommand
    Public ECmdSite As SQLiteCommand 'SqliteCommand
    Public ECompCmdSite As SQLiteCommand 'SqliteCommand
    Public EAdptr As SQLiteDataAdapter  'SqliteDataAdapter
    Public EAdptrSite As SQLiteDataAdapter 'SqliteDataAdapter
    Public ETrans As SQLiteTransaction  ' SqlClient.SqlTransaction
    Public ETransSite As SQLiteTransaction 'SqlClient.SqlTransaction
...
#Region "SubRoutine Synchronise Site Database"
    ...
    '' ''                    Dman_ExecuteNonQry(mQry, GcnSite)
    '' ''                ElseIf UTrim(XNull(.Rows(I)("AED"))) = "A" Then
    '' ''                    mQry = "Select IfNull(Count(*),0) Cnt " & _
...
#End Region
    ...
    Public Sub LogTableEntry(ByVal DocId As String, ByVal EntryPoint As String, ByVal EntryMode As String,
                                ...
                                ByVal mConn As Object, Optional ByVal mCmd As Object = Nothing,
        ...
        mQry = "Insert Into LogTable(DocId, EntryPoint, MachineName, U_Name, U_EntDt, U_AE, Remark, V_Date, SubCode, PartyDetail, Amount, Site_Code, Div_Code) " &
            " Values('" & DocId & "','" & EntryPoint & "', '" & MachineName & "' ,'" & U_Name & "'," &
            " '" & mU_EntDt & "','" & EntryMode & "', " & Chk_Text(mRemark) & ", " & Chk_Text(V_Date) & ", " & Chk_Text(SubCode) & ", " & Chk_Text(PartyDetail) & ", " & Val(Amount) & ", " & Chk_Text(Site_code) & ", " & Chk_Text(Div_Code) & ")"
        Dman_ExecuteNonQry(mQry, mConn, mCmd)
    End Sub
...
err1:
    ...
    Public Function CondStrFinancialYear(ByVal FieldName As String, ByVal PubStartDate As String,
        ...
        If FieldName.Trim <> "" Then
            CondStrFinancialYear = " And " & FieldName & " >= '" & CDate(PubStartDate).ToString("u") & "' And  " & FieldName & " <= '" & CDate(PubEndDate).ToString("u") & "' "
        End If
    End Function
    ...
    Public Function GetDocId(ByVal V_Type As String, ByRef mVno As String,
        ...
        Try
            ...
            mQry = "Select  VT.V_Type,VT.DivisionWise,VT.SiteWise, VT.Number_Method, VP.Prefix,VP.Start_Srl_No " &
                ...
                " Where VP.V_Type='" & V_Type & "' And VP.Date_From <= '" & CDate(VDate).ToString("u") & "' And Vp.Date_To >= '" & CDate(VDate).ToString("u") & "' " & mCondStr &
            ...
            mQry = "Select V_Type from Voucher_Prefix VP  " &
                    " Where VP.V_Type='" & V_Type & "' And VP.Date_From <= '" & (CDate(Format(VDate, "dd/MMM/yyyy")).ToString("u")) & "' AND VP.Date_To >= '" & (CDate(Format(VDate, "dd/MMM/yyyy")).ToString("u")) & "' " &
            ...
            If DsTemp.Tables(0).Rows.Count > 0 Then
                mQry = "Select  VP.V_Type,VP.Date_From,VP.Prefix,(Case When '" & mNumberMethod & "'='Manual' And " & Val(mVno) & "<>0 Then " & Val(mVno) & " Else  IfNull(VP.Start_Srl_No,0)+1 End) as Start_Srl_No " &
                    " From Voucher_Type VT  " &
                    " Left Join Voucher_Prefix VP  on VT.V_Type=VP.V_Type " &
                    " Where VP.V_Type='" & V_Type & "' And VP.Date_From<='" & (CDate(Format(VDate, "dd/MMM/yyyy")).ToString("u")) & "'  And VP.Date_To>='" & (CDate(Format(VDate, "dd/MMM/yyyy")).ToString("u")) & "' " & mCondStr
    ...
    Public Function AddForeignKey(ByVal mConn As SQLiteConnection, ByVal Constraint_Name As String,
        ...
        Dim ECmd As SQLiteCommand
        Try
            mQry = "SELECT IfNull(count(*),0) Cnt FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_TYPE='FOREIGN KEY' AND TABLE_NAME = '" & ForeignKeyTable & "' AND CONSTRAINT_NAME = '" & Constraint_Name & "' "
            ECmd = Dman_Execute(mQry, mConn)
    ...
    Public Sub CreateNCat(ByVal mConn As Object, ByVal NCat As String, ByVal Category As String, ByVal NCatDescription As String, ByVal SiteCode As String)
        Dim mQry$ = ""
        mQry = "Select IfNull(Count(*),0) Cnt From VoucherCat Where NCat='" & NCat & "' And Category='" & Category & "' And NCatDescription='" & NCatDescription & "' "
    ...
    Public Sub CreateVType(ByVal mConn As Object, ByVal NCat As String, ByVal Category As String, ByVal V_Type As String, ByVal Description As String, ByVal Short_Name As String,
        ...
        If mComp_Code.Trim <> "" Then mCondStr += " And IfNull(Comp_Code,'" & AglObj.PubCompCode & "') = '" & mComp_Code & "' "
    ...
    Public Sub AddNewSubGroupType(ByVal mConn As Object, ByVal PartyTypeDescription As String, Optional ByVal PartyTypeCode As Integer = 0,
        ...
        If PartyTypeCode = 0 Then
            mQry = "Select IfNull(Max(Party_Type),0) + 1 As MaxId From SubGroupType "
            PartyTypeCode = Dman_Execute(mQry, mConn).ExecuteScalar()
        Else
            mQry = "Select IfNull(Count(*),0) As Cnt From SubGroupType Where Party_Type=" & PartyTypeCode & " "
    ...
    Public Function AddNewField(ByVal mConn As SQLiteConnection, ByVal mTable As String, ByVal mColumn As String, ByVal mDataType As String, Optional ByVal mDefault_Value As String = "", Optional ByVal AllowNull As Boolean = True) As Boolean
        ...
        Try
            ...
            mQry = "select IfNull(count(*),0) from sysColumns where id = object_id('" & mTable & "') and name in ('" & mColumn & "')"
            ECmd = Dman_Execute(mQry, mConn)
    ...
    Public Function FunGetDataFilePath(ByVal mConn As SQLiteConnection) As String
    ...
    Public Function FunGetLogFilePath(ByVal mConn As SQLiteConnection) As String
        Dim mQry As String, bReturnStr$ = ""
        Try
            mQry = "SELECT Reverse(Substring(Reverse(F.physical_name), charindex('\',Reverse(F.physical_name)), Len(F.physical_name))) ReturnStr " &
                    " FROM sys.Database_Files F " &
    ...
    Public Function FunGetLogicalDataFileName(ByVal mConn As SQLiteConnection) As String
        Dim bReturnStr$ = ""
        Try
    ...
    Public Function FunGetLogicalLogFileName(ByVal mConn As SQLiteConnection) As String
        Dim bReturnStr$ = ""
        Try
    ...
    Public Function FunCreateSubGroup_Log(ByVal SqlConn As SQLiteConnection, ByVal SqlCmd As SQLiteCommand, ByVal StrSubCode As String) As String
        ...
        Try
            ...
            If bStrUID.Trim <> "" Then
                ...
                bQry = "SELECT IfNull(Count(*),0) AS Cnt FROM SubGroup_Log Sg  " &
            ...
            bQry = "SELECT IfNull(Count(*),0) AS Cnt " &
            ...
            If bBlnIsImageExists Then
                bQry = "UPDATE SubGroup_Image " &
                        " SET UID = " & AglObj.Chk_Text(bStrUID) & " " &
                        " WHERE SubCode  = " & AglObj.Chk_Text(StrSubCode) & " " &
                        " AND IfNull(Convert(NVARCHAR(36),UID),'') <> " & AglObj.Chk_Text(bStrUID) & " "
                AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
            End If

            If bBlnIsImageLogExists Then
                bQry = "UPDATE SubGroup_Image_Log " &
                        " SET UID = " & AglObj.Chk_Text(bStrUID) & " " &
                        " WHERE SubCode  = " & AglObj.Chk_Text(StrSubCode) & " " &
                        " AND IfNull(Convert(NVARCHAR(36),UID),'') <> " & AglObj.Chk_Text(bStrUID) & " "
                AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
            ...
            Else
                If bBlnIsImageExists Then
                    bQry = "INSERT INTO SubGroup_Image_Log (SubCode, Photo, Signature, UID) " &
                            " (SELECT SubCode, Photo, Signature, UID FROM dbo.SubGroup_Image  " &
                            " WHERE SubCode = " & AglObj.Chk_Text(StrSubCode) & " " &
                            " )"
                    AglObj.Dman_ExecuteNonQry(bQry, SqlConn, SqlCmd)
                End If
...
#End Region
    ...
    Public Function PortfolioItem_Save(ByVal mConn As SQLiteConnection,
        ...
        With ObjPortfolio_ItemDetail
            If .Sr > 0 Then
                ...
                Dman_ExecuteNonQry(mQry, mConn, mCmd)
            Else
                .Sr = FillData("Select IfNull(Max(Sr),0)+1 From Portfolio_ItemDetail Where Code = '" & Portfolio & "'", mConnRead).tables(0).rows(0)(0)
            End If
    ...
    Public Sub SendSms(ByVal Agl As AgLibrary.ClsMain)
        ...
        Try
            ...
            If Agl.PubLongSmsLimit > 0 Then
                mQry = "Select IfNull(Count(*),0) As Cnt " &
                        " From Sms_Trans  " &
                        " Where Status = 'Send' "
    ...
    Public Function RetDivisionCondition(ByVal Agl As AgLibrary.ClsMain, ByVal Div_Fld As String)
        ...
        If Agl.PubDivWiseBrowsing Then
            mCond = " And IfNull(" & Div_Fld & ",'" & Agl.PubDivCode & "') = '" & Agl.PubDivCode & "' "
        ElseIf Not StrCmp(Agl.PubUserName, "Sa") Then
            mCond = " And IfNull(" & Div_Fld & ",'" & Agl.PubDivCode & "') In " & Agl.PubDivisionList & " "
        End If
    ...
    Public Function BackupDataBase(ByVal mBackupFilePath As String, ByVal Agl As AgLibrary.ClsMain, ByVal mConn As SQLiteConnection, Optional ByVal mOverWrite As Boolean = False) As Boolean
    ...
    Public Function AddFieldSqlite(tableName As String, columnName As String, dataType As String, DefaultValue As String, Optional isNullable As Boolean = True, Optional referentialKey As String = "")
        Dim mQry As String
        Try
