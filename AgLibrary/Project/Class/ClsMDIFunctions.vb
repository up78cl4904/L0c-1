...
Public Class ClsMDIFunctions
    ...
    Public Sub FGenerate_UP_Control(ByVal ObjFor As Object, ByVal StrModule As String,
    ByVal GCnCmd As SQLiteCommand)
        Dim FrmObj As Form
        Dim ADMain As OleDb.OleDbDataAdapter = Nothing
        Dim DTMain As New DataTable
        ...
        Try
            mQry = "Select MnuName,MnuText,ControlPermissionGroups From User_Permission Where UserName='SA' And IfNull(Active,'Y') = 'Y' And MnuModule='" & StrModule & "' And ReportFor Is Null"
            DTMain = Agl.FillData(mQry, Agl.ECompConn).tables(0)
            For I = 0 To DTMain.Rows.Count - 1
                StrName = Agl.XNull(DTMain.Rows(I).Item("MnuName"))
                StrText = Agl.XNull(DTMain.Rows(I).Item("MnuText"))
        ...
        Catch Ex As Exception
            MsgBox(Ex.Message & " in FGenerate_UP_Control of AgLibrary.ClsMDIFunctions ")
        End Try
    End Sub

    Private Sub FGenerate_UP_ContainerControl(ByRef ObjContainer As Object, ByVal StrModule As String,
                                                ByVal StrMnuName As String, ByVal StrMnuText As String, ByVal StrControlPermissionGroups As String,
                                                ByVal GCnCmd As SQLiteCommand)
            ...
            For Each ObjCtrl In ObjContainer.Controls
                If ObjCtrl.GetType.ToString = "System.Windows.Forms.GroupBox" Then
                    If UCase(StrControlPermissionGroups).Contains(Trim(UCase(ObjCtrl.Text))) And ObjCtrl.Text <> "" Then
                        GCnCmd.CommandText = "Insert Into User_Control_Permission(UserName,MnuModule,MnuName,MnuText,GroupText,GroupName) Values" &
    ...
    Public Sub FGenerate_Apply_GlobalParameter(ByVal ObjFor As Object, ByVal StrModule As String,
    ByVal GCnCmd As SQLiteCommand)
        Dim FrmObj As Form
        Dim ObjCtrl As Object
        Dim ADMain As OleDb.OleDbDataAdapter = Nothing
        ...
        mQry = "Select MnuName,MnuText From User_Permission Where UserName='SA' And IfNull(Active,'Y') = 'Y' And MnuModule='" & StrModule & "'"
        DTMain = Agl.FillData(mQry, Agl.ECompConn).tables(0)
        For I = 0 To DTMain.Rows.Count - 1
            StrName = Agl.XNull(DTMain.Rows(I).Item("MnuName"))
            StrText = Agl.XNull(DTMain.Rows(I).Item("MnuText"))
    ...
    ByVal IntSno As Integer, ByVal StrMnuPath As String, ByVal GCnCmd As SQLiteCommand)
        Dim Mnu As Object
    ...
    ByVal StrMnuModule As String, ByVal IntSNo As Integer, ByVal IntLevel As String, ByVal ReportFor As String, ByVal ControlPermissionGroups As String)
        ...
        GCnCmd.CommandText = "Insert Into User_Permission(UserName,Parent,MnuText,MnuName,Permission,SNo,MnuModule,MnuLevel,ReportFor, ControlPermissionGroups,MainStreamCode, GroupLevel, Active) Values " &
    ...
    ByVal IntSno As Integer, ByVal GCnCmd As SQLiteCommand) As Integer
    ...
    Public Function FRotateAllMenuItems(ByRef Menus As ToolStripItemCollection, ByVal StrMnuMain As String, ByVal StrModule As String, ByVal StrParent As String,
    ByVal IntSno As Integer, ByVal GCnCmd As SQLiteCommand) As Integer
        Dim TSI_Main As ToolStripItem
        Dim TSMI_Main As ToolStripMenuItem
        Dim ReportFor As String
    ...
    Public Sub FManageEntryPointPermission(ByVal mConn As SQLiteConnection, ByVal mCmd As SQLiteCommand)
        ...
        If Agl.IsTableExist("EntryPointPermission", mConn) Then
            bQry = "INSERT INTO EntryPointPermission " &
                    " 	( " &
                    " 	MnuModule, " &
                    "   MnuName " &
                    " 	) " &
                    ...
                    " LEFT JOIN EntryPointPermission E ON E.MnuModule = U.MnuModule AND E.MnuName = U.MnuName " &
                    " WHERE U.UserName = 'SA' AND E.MnuModule IS NULL "
            Agl.Dman_ExecuteNonQry(bQry, mConn, mCmd)

            bQry = "Delete From EntryPointPermission " &
                    " Where MnuModule + MnuName Not In " &
                    " (SELECT " &
                    " 	U.MnuModule + U.MnuName " &
                    " FROM User_Permission U " &
                    " WHERE U.UserName = 'SA') "
            Agl.Dman_ExecuteNonQry(bQry, mConn, mCmd)
        End If
