...
#End Region
    ...
    Private Sub TempJobReceive_BaseEvent_Approve_PreTrans(ByVal SearchCode As String) Handles Me.BaseEvent_Approve_PreTrans
        mQry = " SELECT H.DocId,L.Item,L.Qty,L.JobOrder,L.TotalMeasure,L.ProdOrder  " &
                " FROM JobIssRec H  " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Type AS [Receive Type], H.V_Prefix AS Prefix, H.V_Date AS Date, H.V_No AS [Receive No], " &
                    ...
                    " D.Div_Name AS Division, SM.Name AS [Site Name], SGJ.DispName AS [Job Worker Name], G.Description AS Godown, JO.ManualRefNo AS [Job ORDER No], SF.*  " &
                    ...
                    " LEFT JOIN (" & AgStructure.AgCalcGrid.AgStructureSubQueryFooter(AgL, EntryNCat, FrmType) & ") As SF On H.UID = SF.UId " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Receive Type], H.V_Prefix AS Prefix, H.V_Date AS Date, H.V_No AS [Receive No], " &
                            " H.ManualRefNo AS [Manual No], H.Process, H.DueDate AS [Due Date], H.IssQty AS [Issue Qty], H.IssMeasure AS [Issue Measure],  " &
                            " H.RecQty AS [Rec Qty], H.RecMeasure AS [Rec Measure], H.JobReceiveFor AS [Job Receive For], H.Remarks, H.Structure, H.EntryBy AS [Entry By],  " &
                            " H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date],  " &
                            " H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, H.BillingType AS [Billing Type], H.OrderBy AS [ORDER By],  " &
                            ...
                            " LEFT JOIN Godown G ON G.Code = H.Godown   " &
                            " LEFT JOIN JobOrder  JO ON H.JobOrder   =JO.DocID  " &
                            " LEFT JOIN (" & AgStructure.AgCalcGrid.AgStructureSubQueryFooter(AgL, EntryNCat, FrmType) & ") As SF On H.DocId = SF.DocId " &
                            " Where 1=1  " & mCondStr
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE JobIssRec_Log " &
                ...
                " JobReceiveFor = " & AgL.Chk_Text(mJobReceiveFor) & ", " &
                " TotalConsumptionQty = " & Val(LblCTotalQty.Text) & ", " &
                " TotalConsumptionMeasure = " & Val(LblCTotalMeasure.Text) & ", " &
                " TotalByProductQty = " & Val(LblTotalByProductQty.Text) & ", " &
                " TotalByProductMeasure = " & Val(LblTotalByProductMeasure.Text) & " " &
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1Item, I).Value <> "" Then
                mSr += 1
                mQry = "INSERT INTO JobReceiveDetail_Log(UID, DocId, Sr, Item,DocQty, RetQty, Qty, BillQty, Unit, MeasurePerPcs,DocMeasure, RetMeasure, TotalMeasure, BillMeasure, " &
                        " MeasureUnit, JobOrder, Rate, Amount, Netamount, Remark, JobIssueDocId, " &
                        " ProdOrder, LossPer, LossQty, LotNo, BOM) " &
                        " Values (" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(mInternalCode) & "," &
                        " " & mSr & ", " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1Item, I)) & ", " &
                        " " & Val(Dgl1.Item(Col1DocQty, I).Value) & ", " & Val(Dgl1.Item(Col1RetQty, I).Value) & ", " & Val(Dgl1.Item(Col1Qty, I).Value) & ", " & Val(Dgl1.Item(Col1BillQty, I).Value) & ", " & AgL.Chk_Text(Dgl1.Item(Col1Unit, I).Value) & ", " &
                        ...
                        " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1JobOrderDocID, I)) & ", " &
                        " " & Val(Dgl1.Item(Col1Rate, I).Value) & ", " &
                        " " & Val(Dgl1.Item(Col1Amount, I).Value) & ", " &
                        " " & Val(AgCalcGrid1.AgChargesValue(AgTemplate.ClsMain.Charges.NETAMOUNT, I, AgStructure.AgCalcGrid.LineColumnType.Amount)) & ", " &
                        " " & AgL.Chk_Text(Dgl1.Item(Col1Remark, I).Value) & ", " &
                        " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1JobIssueDocId, I)) & ", " &
                        " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1ProdOrder, I)) & ", " &
                        " " & Val(Dgl1.Item(Col1LossPer, I).Value) & ", " &
                        " " & Val(Dgl1.Item(Col1LossQty, I).Value) & ", " &
                        " " & AgL.Chk_Text(Dgl1.Item(Col1LotNo, I).Value) & ", " &
                        " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1BOM, I)) & " " &
                        " ) "
                AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    ...
    Private Sub Save_BomDetail(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand)
        Dim I As Integer, mSr As Integer
        ...
        With Dgl2
            For I = 0 To .Rows.Count - 2
                If .Item(Col2Item, I).Value <> "" Then
                    mSr += 2
                    mQry = " INSERT INTO JobReceiveBOM_Log(DocId, " &
                            " Sr, " &
                            " Item, " &
                            " BOMItem, " &
                            " StockItem, " &
                            ...
                            " " & Val(mSr) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col2Item, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col2BOMItem, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col2StockItem, I)) & ", " &
                            " " & Val(.Item(Col2Qty, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col2LotNo, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col2Unit, I).Value) & ", " &
                            " " & Val(.Item(Col2MeasurePerPcs, I).Value) & ", " &
                            " " & Val(.Item(Col2TotalMeasure, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col2MeasureUnit, I).Value) & ", " &
                            " " & AgL.Chk_Text(mPrevProcess) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col2ProdOrder, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col2JobOrder, I)) & ", " &
                            " '" & mSearchCode & "')"
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                End If
            Next
    ...
    Private Sub Save_ByProductDetail(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand)
        Dim I As Integer, mSr As Integer
        ...
        With Dgl3
            For I = 0 To .Rows.Count - 1
                If .Item(Col3Item, I).Value <> "" Then
                    mSr += 3
                    mQry = " INSERT INTO JobReceiveByProduct_Log(DocId, " &
                            " Sr, " &
                            " Item, " &
                            " StockItem, " &
                            " Qty, " &
                            ...
                            " VALUES (" & AgL.Chk_Text(mInternalCode) & ", " &
                            " " & Val(mSr) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col3Item, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col3StockItem, I)) & ", " &
                            " " & Val(.Item(Col3Qty, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col3LotNo, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col3Unit, I).Value) & ", " &
                            " " & Val(.Item(Col3MeasurePerPcs, I).Value) & ", " &
                            " " & Val(.Item(Col3TotalMeasure, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col3MeasureUnit, I).Value) & ", " &
                            " " & AgL.Chk_Text(TxtProcess.AgSelectedValue) & ", " &
                            " " & Val(.Item(Col3Rate, I).Value) & ", " &
                            " " & Val(.Item(Col3Amount, I).Value) & ", " &
                            " " & IIf(AgL.StrCmp(.Item(Col3IsDelivered, I).Value, "Yes"), 1, 0) & ", " &
                            " '" & mSearchCode & "')"
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                End If
            Next
        ...
        Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
            Case Col1Item
                If AgL.StrCmp(Topctrl1.Mode, "Add") Then
                    Dgl1.AgRowFilter(Dgl1.Columns(Col1Item).Index) = " IsDeleted = 0  " &
    ...
    Private Function FCheckDuplicateRefNo() As Boolean
        ...
        If Topctrl1.Mode = "Add" Then
            ...
            mQry = " SELECT COUNT(*) FROM JobIssRec_Log WHERE ManualRefNo = '" & TxtManualRefNo.Text & "'   " &
    ...
    Private Sub Txt_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtGodown.Enter, TxtJobWorker.Enter, TxtJobOrderNo.Enter
        Try
            Select Case sender.name
                Case TxtGodown.Name
                    TxtGodown.AgRowFilter = " Div_Code = '" & AgL.PubDivCode & "' " &
        ...
        With DsMain.Tables(0)
            If .Rows.Count > 0 Then
                For I = 0 To DsMain.Tables(0).Rows.Count - 1
                    mQry = " UPDATE " & bTableName & " " &
                            " SET ReceiveQty = (SELECT IfNull(Sum(L.Qty),0)  " &
                            " 				   FROM JobReceiveDetail L " &
                            "                  LEFT JOIN JobIssRec H On L.DocId  = H.DocId " &
                            "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                            " 				   WHERE Vt.NCat = '" & EntryNCat & "' " &
                            "                  And L.JobOrder = '" & AgL.XNull(.Rows(I)("JobOrder")) & "' " &
                            " 				   And L.Item = '" & AgL.XNull(.Rows(I)("Item")) & "' And IfNull(L.ProdOrder,'') = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' And IfNull(H.IsDeleted,0)=0), " &
                            " ReceiveMeasure = (SELECT IfNull(Sum(L.TotalMeasure),0)  " &
                            " 				   FROM JobReceiveDetail L " &
                            "                  LEFT JOIN JobIssRec H On L.DocId  = H.DocId " &
                            "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                            " 				   WHERE Vt.NCat = '" & EntryNCat & "' " &
                            "                  And L.JobOrder = '" & AgL.XNull(.Rows(I)("JobOrder")) & "' " &
                            " 				   And L.Item = '" & AgL.XNull(.Rows(I)("Item")) & "' And IfNull(L.ProdOrder,'') = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' And IfNull(H.IsDeleted,0)=0), " &
                            " ReceiveLoss = (SELECT IfNull(Sum(L.LossQty),0)  " &
                            " 				   FROM JobReceiveDetail L " &
                            "                  LEFT JOIN JobIssRec H On L.DocId  = H.DocId " &
                            "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                            " 				   WHERE Vt.NCat = '" & EntryNCat & "' " &
                            "                  And L.JobOrder = '" & AgL.XNull(.Rows(I)("JobOrder")) & "' " &
                            " 				   And L.Item = '" & AgL.XNull(.Rows(I)("Item")) & "' And IfNull(L.ProdOrder,'') = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' And IfNull(H.IsDeleted,0)=0) " &
                            " Where DocId = '" & AgL.XNull(.Rows(I)("JobOrder")) & "' " &
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mQry = " UPDATE " & bTableName & " " &
                            ...
                            "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                            " 				   WHERE Vt.NCat = '" & EntryNCat & "' " &
                            "                  And L.JobOrder = '" & .AgSelectedValue(Col1JobOrderDocID, I) & "' " &
                            " 				   And L.Item = '" & .AgSelectedValue(Col1Item, I) & "' And IfNull(L.ProdOrder,'') = '" & .AgSelectedValue(Col1ProdOrder, I) & "'  And IfNull(H.IsDeleted,0)=0), " &
                            " ReceiveMeasure = (SELECT IfNull(Sum(L.TotalMeasure),0)  " &
                            " 				   FROM JobReceiveDetail L " &
                            "                  LEFT JOIN JobIssRec H On L.DocId  = H.DocId " &
                            "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                            ...
                            " And Item = '" & .AgSelectedValue(Col1Item, I) & "'   And IfNull(ProdOrder,'') = '" & .AgSelectedValue(Col1ProdOrder, I) & "' "
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)

                    mQry = "UPDATE JobOrder " &
                            " SET " &
                            " LastReceiveDate = (SELECT TOP 1 H.V_Date  " &
                            "                  FROM JobReceiveDetail L " &
                            "                  LEFT JOIN JobIssRec H ON H.DocID = L.DocId " &
                            "                  WHERE L.JobOrder = '" & .AgSelectedValue(Col1JobOrderDocID, I) & "'  " &
                            "                  ORDER BY H.V_Date DESC) " &
                            " Where DocId = '" & .AgSelectedValue(Col1JobOrderDocID, I) & "' "
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)

                    mQry = " UPDATE MaterialPlanDetail " &
                            " SET JobRecQty  = ( " &
                            ...
                            " 	    LEFT JOIN Voucher_Type Vt    On H.V_Type = Vt.V_Type   " &
                            " 	    WHERE Vt.NCat = '" & EntryNCat & "'    " &
                            " 	    And L.ProdOrder = '" & .AgSelectedValue(Col1ProdOrder, I) & "' " &
                            " 	    And IfNull(H.IsDeleted,0) = 0  " &
                            " 	    And L.Item = '" & .AgSelectedValue(Col1Item, I) & "' " &
                            "       ) " &
                            " WHERE ProdOrder = '" & .AgSelectedValue(Col1ProdOrder, I) & "' " &
    ...
    Private Sub TempJobReceive_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = "SELECT G.Code, G.Description, Sm.ManualCode As Site, G.Site_Code, G.Div_Code, IfNull(G.IsDeleted,0) as IsDeleted, " &
                " IfNull(G.Status,'" & ClsMain.EntryStatus.Active & "') AS Status " &
                " FROM Godown G " &
        ...
        mQry = "SELECT Po.DocId AS Code, " &
                " ManualRefNo As [Prod.Order No. Manual], PO.V_Type + '-' + Convert(nVarchar,PO.V_No) as [Prod.Order No.]  " &
                " FROM ProdOrder Po "
        ...
        mQry = " SELECT I.Code AS Code, I.Description AS Item, H.ManualRefNo As JobOrderNo, " &
                " H.V_Date As JobOrderDate, H.DueDate, " &
                " IfNull(L.Qty,0) - (IfNull(L.ReceiveQty,0)+IfNull(L.CancelQty,0)+IfNull(L.ReceiveLoss,0)) AS BalanceQty, H.Remarks, " &
                " '' As JobIssueDocId, L.DocId AS JobOrder, I.ItemType, I.SalesTaxPostingGroup,  " &
                " L.Unit, L.MeasurePerPcs AS Measure, L.MeasureUnit, L.Rate, I.BOM,  " &
                " IfNull(H.IsDeleted,0) AS IsDeleted, " &
                " IfNull(H.Status,'" & ClsMain.EntryStatus.Active & "') AS Status, " &
                " H.Div_Code, Vt.NCat, H.JobWorker, L.ProdOrder, " &
                " L.Sr As LineSr, IfNull(H.JobWithMaterialYN,0) As JobWithMaterialYN, H.Remarks " &
        ...
        mQry = "SELECT I.Code, I.Description AS Item, H.ManualRefNo As JobIssueNo, " &
                " H.V_Date As JobIssueDate, H.DueDate, " &
                " IfNull(L.Qty,0) - IfNull(L.ReceiveQty,0)  AS BalanceQty, H.Remarks, " &
                " L.DocId AS JobIssueDocId, '' As JobOrder, I.ItemType, " &
                " I.SalesTaxPostingGroup, L.Unit, L.MeasurePerPcs As Measure, I.BOM,    " &
                " L.MeasureUnit,  IfNull(H.IsDeleted ,0) AS IsDeleted,   " &
                " IfNull(H.Status,'" & ClsMain.EntryStatus.Active & "') AS Status,  " &
                " H.Div_Code, Vt.NCat, H.JobWorker, Null as ProdOrder, I.Rate, " &
                " L.Sr as LineSr, 0 As JobWithMaterialYN" &
        ...
        mQry = " SELECT  H.Code, H.Description, H.Unit, H.ItemType, IfNull(H.IsDeleted,0) As IsDeleted, " &
                " H.UpcCode, H.Bom, H.Status, '' AS JobIssueDocId, '' As JobOrder, " &
                " H.Div_Code, H.SalesTaxPostingGroup, H.Measure, H.MeasureUnit, " &
                " H.ItemGroup, H.Rate " &
                " FROM Item H "
        ...
        mQry = "SELECT H.DocID AS Code, H.ManualRefNo AS JobOrderNo , " &
                " IfNull(H.IsDeleted,0) AS IsDeleted, " &
                " IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status, " &
                " H.Div_Code, Vt.NCat As Process, H.JobWorker " &
                " FROM JobOrder H " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type "
        HelpDataSet.JobOrder = AgL.FillData(mQry, AgL.GCn)
    End Sub
    ...
    Private Sub ProcFillQcParameterDetail(ByVal RowIndex As Integer)
        ...
        Try
            ...
            If Dgl1.AgSelectedValue(Col1JobOrderDocID, RowIndex) <> "" Then
                mQry = " SELECT H.Parameter, H.StdValue " &
                        " FROM JobOrderQCInstruction H " &
                        " WHERE H.DocId = '" & Dgl1.AgSelectedValue(Col1JobOrderDocID, RowIndex) & "'  "
            Else
                mQry = "SELECT Qd.Parameter, Qd.StdValue " &
                        " FROM Process P " &
                        " LEFT JOIN QcGroup Q ON P.QcGroup  = Q.Code " &
                        " LEFT JOIN QcGroupDetail Qd ON Q.Code = Qd.Code " &
    ...
    Private Sub TempJobOrder_BaseFunction_ConsumptionGridFill() Handles Me.BaseFunction_ConsumptionGridFill
        ...
        Try
            ...
            mQry = "CREATE TABLE [#" & bTempTable & "] " &
                    " (ProdOrder nVarChar(21), JobOrder nVarChar(21), Item NVARCHAR(10), " &
            ...
            With Dgl1
                For I = 0 To .Rows.Count - 1
                    If .Item(Col1Item, I).Value <> "" Then
                        mQry = "INSERT INTO [#" & bTempTable & "](ProdOrder, JobOrder, " &
                                " Item, BOMItem, TotalConsumptionQty) " &
                                " SELECT " & AgL.Chk_Text(.AgSelectedValue(Col1ProdOrder, I)) & " As ProdOrder, " &
                                " " & AgL.Chk_Text(.AgSelectedValue(Col1JobOrderDocID, I)) & " As JobOrder, " &
                                " " & AgL.Chk_Text(.AgSelectedValue(Col1Item, I)) & " As Item, " &
                                " Bd.Item As BOMItem, Bd.Qty * " & Val(.Item(Col1DocQty, I).Value) & " " &
                                " FROM Bom B  " &
                                " LEFT JOIN BomDetail Bd ON B.Code = Bd.Code " &
            ...
            mQry = "SELECT T.ProdOrder, T.JobOrder, T.Item, T.BOMItem, Sum(IfNull(TotalConsumptionQty,0)) As BomQty " &
                    " From [#" & bTempTable & "] T " &
                    " Group By T.ProdOrder, T.JobOrder, T.Item, T.BOMItem " &
    ...
    Private Sub ProcFillJobOrderDetails(ByVal JobOrderNo As String)
        ...
        Dim I As Integer = 0
        Try
            mQry = "  SELECT I.Code AS Code, L.DocId AS JobOrder , " &
                        " IfNull(L.Qty,0) - (IfNull(L.ReceiveQty,0)+IfNull(L.CancelQty,0)+IfNull(L.ReceiveLoss,0)) AS BalanceQty,   " &
                        " IfNull(L.TotalMeasure,0) - (IfNull(L.ReceiveMeasure,0)+IfNull(L.CancelMeasure,0)+IfNull(L.ReceiveMeasure,0)) AS BalanceMeasure,   " &
                        " I.ItemType, I.SalesTaxPostingGroup, L.LossPer, L.Loss As LossQty,    " &
                        " L.Unit, L.MeasurePerPcs , L.MeasureUnit, " &
                        " L.Rate, I.BOM, H.JobWorker, L.ProdOrder, L.Amount, L.Remark   " &
                        " FROM JobOrderDetail L    " &
                        " LEFT JOIN Item I ON L.Item = I.Code   " &
                        " LEFT JOIN JobOrder H ON L.DocId = H.DocID   " &
                        " Where L.DocId = '" & TxtJobOrderNo.AgSelectedValue & "' " &
                        " And IfNull(L.Qty,0) - (IfNull(L.ReceiveQty,0)+IfNull(L.CancelQty,0)+IfNull(L.ReceiveLoss,0))  > 0"
