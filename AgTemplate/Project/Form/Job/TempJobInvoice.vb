...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Type AS [Invoice Type], H.V_Prefix AS Prefix, H.V_Date AS [Invoice Date], H.V_No AS [Invoice No], " &
                " H.ManualRefNo AS [Manual No], H.JobWorkerDocNo AS [Job Worker Doc No], H.BillingType AS [Billing Type],  H.TotalDocQty AS [Total Doc Qty],  " &
                " H.TotalQty AS [Total Qty], H.TotalBillQty AS [Total Bill Qty], H.TotalLoss AS [Total Loss], H.TotalDocMeasure AS [Total Doc Measure], H.TotalMeasure AS [Total Measure],  " &
                " H.TotalBillMeasure AS [Total Bill Measure], H.TotalLossMeasure AS [Total Loss Measure], H.Amount, H.NetAmount AS [Net Amount], H.Remarks, H.Structure,  " &
                " H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By],  " &
                " H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, H.Process, H.TotalBomAmount AS [Total Bom Amount], H.NetPaybleAmount AS [Net Payble Amount], " &
                " D.Div_Name AS Division,SM.Name AS [Site Name],SGJ.DispName AS [Worker Name] " &
                " FROM  JobInvoice_Log H " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Invoice Type], H.V_Prefix AS Prefix, H.V_Date AS [Invoice Date], H.V_No AS [Invoice No], " &
                        " H.ManualRefNo AS [Manual No], H.JobWorkerDocNo AS [Job Worker Doc No], H.BillingType AS [Billing Type],  H.TotalDocQty AS [Total Doc Qty],  " &
                        " H.TotalQty AS [Total Qty], H.TotalBillQty AS [Total Bill Qty], H.TotalLoss AS [Total Loss], H.TotalDocMeasure AS [Total Doc Measure], H.TotalMeasure AS [Total Measure],  " &
                        " H.TotalBillMeasure AS [Total Bill Measure], H.TotalLossMeasure AS [Total Loss Measure], H.Amount, H.NetAmount AS [Net Amount], H.Remarks, H.Structure,  " &
                        " H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By],  " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = " UPDATE JobInvoice_Log " &
                " SET " &
                " ManualRefNo = " & AgL.Chk_Text(TxtManualRefNo.Text) & ", " &
                " JobWorker =" & AgL.Chk_Text(TxtJobWorker.AgSelectedValue) & ", " &
                " JobWorkerDocNo = " & AgL.Chk_Text(TxtJobWorkerDocNo.Text) & ", " &
                " BillingType = " & AgL.Chk_Text(TxtBillingType.Text) & ", " &
                " TotalDocQty = " & Val(LblTotalDocQty.Text) & ", " &
                " TotalQty = " & Val(LblTotalQty.Text) & ", " &
                " TotalBillQty = " & Val(LblTotalBillQty.Text) & ", " &
                " TotalLoss = " & Val(LblTotalLossQty.Text) & ", " &
                " TotalDocMeasure = " & Val(LblTotalDocMeasure.Text) & ", " &
                " TotalMeasure = " & Val(LblTotalMeasure.Text) & ", " &
                " TotalBillMeasure = " & Val(LblTotalBillMeasure.Text) & ", " &
                " TotalLossMeasure = " & Val(LblTotalLossMeasure.Text) & ", " &
                " TotalBomAmount = " & Val(LblTotalBomAmount.Text) & ", " &
                " Amount = " & Val(AgCalcGrid1.AgChargesValue(AgTemplate.ClsMain.Charges.GROSSAMOUNT, AgStructure.AgCalcGrid.AgCalcGridColumn.Col_Amount)) & ", " &
                " NetAmount = " & Val(AgCalcGrid1.AgChargesValue(AgTemplate.ClsMain.Charges.NETAMOUNT, AgStructure.AgCalcGrid.AgCalcGridColumn.Col_Amount)) & ", " &
                " NetPaybleAmount = " & Val(LblNetPaybleAmount.Text) & ", " &
        ...
        With Dgl1
            For I = 0 To Dgl1.RowCount - 1
                If Dgl1.Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = " INSERT INTO JobInvoiceDetail_Log (DocId, Sr, JobReceive, Item, " &
                            " DocQty, Qty, BillQty, LossPer, LossQty, " &
                            " Unit, MeasurePerPcs, DocMeasure, TotalMeasure, LossMeasure, " &
                            " BillMeasure, MeasureUnit, JobOrder, Rate, " &
                            " Amount, NetAmount, JobIssueDocId, ProdOrder, " &
                            " Remark, LotNo, UID) " &
                            " VALUES ('" & mInternalCode & "', " &
                            " " & Val(mSr) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1JobReceive, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1Item, I)) & ", " &
                            " " & Val(.Item(Col1DocQty, I).Value) & ", " &
                            " " & Val(.Item(Col1Qty, I).Value) & ", " &
                            " " & Val(.Item(Col1BillQty, I).Value) & ", " &
                            " " & Val(.Item(Col1LossPer, I).Value) & ", " &
                            " " & Val(.Item(Col1LossQty, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1Unit, I).Value) & ", " &
                            " " & Val(.Item(Col1MeasurePerPcs, I).Value) & ", " &
                            " " & Val(.Item(Col1DocMeasure, I).Value) & ", " &
                            " " & Val(.Item(Col1TotalMeasure, I).Value) & ", " &
                            " " & Val(.Item(Col1LossMeasure, I).Value) & ", " &
                            " " & Val(.Item(Col1BillMeasure, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1MeasureUnit, I).Value) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1JobOrder, I)) & ", " &
                            ...
                            " " & AgL.Chk_Text(.Item(Col1JobIssueDocId, I).Value) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1ProdOrder, I)) & ", " &
        ...
        With Dgl3
            For I = 0 To Dgl3.RowCount - 1
                If Dgl3.Item(Col3Item, I).Value <> "" Then
                    mSr += 3
                    mQry = " INSERT INTO JobInvoiceBom_Log (DocId, " &
                            " Sr, " &
                            " Item, " &
                            " Qty, " &
                            " Unit, " &
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                With DsTemp.Tables(0)
                    ...
                    If FrmType = ClsMain.EntryPointType.Main Then
                        mQry = "Select Distinct L.JobReceive, R.ManualRefNo, R.V_Date As JobReceiveDate " &
                                " from JobInvoiceDetail L " &
                                " LEFT JOIN JobIssRec R On L.JobReceive = R.DocId " &
                                " where L.DocId = '" & SearchCode & "' "
                    Else
                        mQry = "Select Distinct L.JobReceive, R.ManualRefNo, R.V_Date As JobReceiveDate " &
                                " from JobInvoiceDetail_Log L " &
                                " LEFT JOIN JobIssRec R On L.JobReceive = R.DocId " &
                    ...
                    If FrmType = ClsMain.EntryPointType.Main Then
                        mQry = "Select L.* " &
                                " from JobInvoiceBom L " &
                                " Where L.DocId = '" & SearchCode & "' "
                    Else
                        mQry = "Select L.* " &
                                " from JobInvoiceBom_Log L " &
                                " Where L.UID = '" & SearchCode & "' "
                    End If
                    DsTemp = AgL.FillData(mQry, AgL.GCn)
                    With DsTemp.Tables(0)
    ...
    Private Sub TempJobReceive_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        ...
        mQry = " SELECT J.SubCode AS Code, Sg.DispName AS JobWorker, H.Process, " &
        ...
        mQry = " SELECT H.DocID AS Code, H.ManualRefNo, H.V_Type + '-' + Convert(NVARCHAR,H.V_No) AS JobReceiveNo, " &
                " IfNull(H.IsDeleted,0) AS IsDeleted, H.Div_Code, " &
                " IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status " &
                " FROM JobIssRec H "
        ...
        mQry = "SELECT Po.DocId AS Code, Po.ManualRefNo As [Prod.Order No.]  " &
                " FROM ProdOrder Po "
    ...
    Private Sub ProcFill(ByVal bJobReceiveNoStr As String)
        ...
        Try
            ...
            mQry = " SELECT L.DocId AS JobReceive, L.Sr, L.Item, L.DocQty, L.Qty, " &
                      " L.BillQty, L.LossPer, L.LossQty, L.Unit, " &
                      " L.MeasurePerPcs, L.DocMeasure, L.TotalMeasure, 0 As LossMeasure, L.BillMeasure, L.MeasureUnit, " &
                      " L.JobOrder, L.Rate, L.Amount, L.NetAmount, L.JobIssueDocId, L.ProdOrder, L.Remark, L.LotNo " &
    ...
    Private Sub ProcFillPendingJobReceive(ByVal bJobWorker As String, ByVal bV_Date As String)
        ...
        Dim I As Integer = 0
        Try
            mQry = "SELECT H.DocId As JobReceive, Max(H.ManualRefNo) As ManualRefNo, " &
                    " Max(H.V_Date) As JobReceiveDate " &
                    " FROM JobReceiveDetail L " &
                    " LEFT JOIN JobIssRec H On L.DocId = H.DocId  " &
                    " LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                    " WHERE H.JobWorker = '" & bJobWorker & "'  " &
                    " And H.V_Date <= '" & bV_Date & "' " &
                    " And H.Process = '" & TxtProcess.AgSelectedValue & "' " &
                    ...
                    " GROUP BY H.DocId " &
                    " HAVING IfNull(Sum(L.InvoiceQty), 0) = 0 "
            DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
    ...
    '                            " And IfNull(H.JobWithMaterialYN,0) <> 0 "
    '                    DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
        ...
        Try
            ...
            If Dgl1.Rows.Count > 0 Then
                For I = 0 To Dgl1.Rows.Count - 1
                    If Dgl1.Item(Col1JobOrder, I).Value <> "" Then
                        If bQry = "" Then
                            bQry = " SELECT L.Item, L.Qty, L.Unit, L.MeasurePerPcs, L.TotalMeasure, L.MeasureUnit " &
                                      " FROM JobIssueDetail  L " &
                                      " LEFT JOIN JobOrder H On L.JobOrder = H.DocId " &
                                      " Where L.JobOrder = '" & Dgl1.AgSelectedValue(Col1JobOrder, I) & "' " &
                                      " And IfNull(H.JobWithMaterialYN,0) <> 0 "
                        Else
                            bQry += " UNION ALL "
                            bQry += " SELECT L.Item, L.Qty, L.Unit, L.MeasurePerPcs, L.TotalMeasure, L.MeasureUnit " &
            ...
            mQry = " Select V1.Item, Sum(V1.Qty) As Qty, Max(V1.Unit) As Unit, " &
                    " Max(V1.MeasurePerPcs) As MeasurePerPcs, Sum(V1.TotalMeasure) As TotalMeasure, " &
                    " Max(V1.MeasureUnit) As MeasureUnit " &
                    " From (" & bQry & ") As V1 " &
                    " Group By V1.Item "
            DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
            With DtTemp
    ...
    Private Sub FrmProductionOrder_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Approve_InTrans
        ...
        '                    " SET InvoiceQty = (SELECT IfNull(Sum(L.Qty),0)  " & _
        ...
        '                    "                  And IfNull(L.ProdOrder,'') = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' " & _
        '                    "                  And IfNull(H.IsDeleted,0)=0), " & _
        '                    " InvoiceMeasure = (SELECT IfNull(Sum(L.TotalMeasure),0)  " & _
        '                    " 				   FROM JobInvoiceDetail L " & _
        '                    "                  LEFT JOIN JobInvoice H On L.DocId  = H.DocId " & _
        '                    "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " & _
        '                    " 				   WHERE Vt.NCat = '" & EntryNCat & "' " & _
        ...
        '                    "                  And IfNull(L.ProdOrder,'') = '" & AgL.XNull(.Rows(I)("ProdOrder")) & "' " & _
        '                    "                  And IfNull(H.IsDeleted,0)=0), " & _
        '                    " InvoiceAmount = (SELECT IfNull(Sum(L.LossQty),0)  " & _
        '                    " 				   FROM JobInvoiceDetail L " & _
        '                    "                  LEFT JOIN JobInvoice H On L.DocId  = H.DocId " & _
        '                    "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " & _
        '                    " 				   WHERE Vt.NCat = '" & EntryNCat & "' " & _
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mQry = " UPDATE JobReceiveDetail " &
                             " SET InvoiceQty = (SELECT IfNull(Sum(L.Qty),0)  " &
                             " 				    FROM JobInvoiceDetail L " &
                             "                  LEFT JOIN JobInvoice H On L.DocId  = H.DocId " &
                             "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                             " 				    WHERE Vt.NCat = '" & EntryNCat & "' " &
                             "                  And L.JobReceive = '" & .AgSelectedValue(Col1JobReceive, I) & "' " &
                             "                  And L.JobOrder = '" & .AgSelectedValue(Col1JobOrder, I) & "' " &
                             " 				    And L.Item = '" & .AgSelectedValue(Col1Item, I) & "' " &
                             "                  And IfNull(L.ProdOrder,'') = '" & .AgSelectedValue(Col1ProdOrder, I) & "' " &
                             "                  And IfNull(H.IsDeleted,0)=0), " &
                             " InvoiceMeasure = (SELECT IfNull(Sum(L.TotalMeasure),0)  " &
                             " 				    FROM JobInvoiceDetail L " &
                             "                  LEFT JOIN JobInvoice H On L.DocId  = H.DocId " &
                             "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                             " 				    WHERE Vt.NCat = '" & EntryNCat & "' " &
                             ...
                             "                  And IfNull(L.ProdOrder,'') = '" & .AgSelectedValue(Col1ProdOrder, I) & "' " &
                             "                  And IfNull(H.IsDeleted,0)=0), " &
                             " InvoiceAmount = (SELECT IfNull(Sum(L.LossQty),0)  " &
                             " 				    FROM JobInvoiceDetail L " &
                             "                  LEFT JOIN JobInvoice H On L.DocId  = H.DocId " &
                             "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
                             " 				    WHERE Vt.NCat = '" & EntryNCat & "' " &
                             ...
                             "                  And IfNull(L.ProdOrder,'') = '" & .AgSelectedValue(Col1ProdOrder, I) & "' " &
                             "                  And IfNull(H.IsDeleted,0)=0) " &
                             " Where DocId = '" & .AgSelectedValue(Col1JobReceive, I) & "' " &
                             " And JobOrder = '" & .AgSelectedValue(Col1JobOrder, I) & "' " &
                             " And Item = '" & .AgSelectedValue(Col1Item, I) & "'  " &
                             " And IfNull(ProdOrder,'') = '" & .AgSelectedValue(Col1ProdOrder, I) & "' "
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)

                    mQry = " UPDATE JobOrderDetail " &
                             " SET InvoiceQty = (SELECT IfNull(Sum(L.Qty),0)  " &
                             " 				    FROM JobInvoiceDetail L " &
                             "                  LEFT JOIN JobInvoice H On L.DocId  = H.DocId " &
                             "                  LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " &
