...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        With AgCL
            ...
            .AddAgTextColumn(Dgl1, Col1Status, 70, 0, Col1Status, False, False, False)
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1Item, I).Value <> "" Then
                ...
                mQry = "INSERT INTO Stock_Log (UID, DocId, Sr, V_Type, V_Prefix, " &
                        ...
                        " Item, Process, Godown, Qty_Iss, Unit, LotNo, " &
                        " MeasurePerPcs, Measure_Iss, MeasureUnit, Status, Rate, Amount, Cost) " &
                        ...
                        " " & AgL.Chk_Text(Dgl1.Item(Col1Status, I).Value) & ", " &
                ...
                mQry = "INSERT INTO StockProcess_Log (UID, DocId, Sr, V_Type, V_Prefix, " &
                        " V_Date, V_No, Div_Code, Site_Code, " &
                        " Item, Process, Qty_Rec, Unit, LotNo, " &
                        " MeasurePerPcs, Measure_Rec, " &
                        " MeasureUnit, Status, Rate, Amount, Cost) " &
                        " VALUES (" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(mInternalCode) & ", " &
                        " " & mSr & ", " & AgL.Chk_Text(TxtV_Type.AgSelectedValue) & ", " & AgL.Chk_Text(LblPrefix.Text) & ", " &
                        " " & AgL.Chk_Text(TxtV_Date.Text) & ", " & Val(TxtV_No.Text) & ", " &
                        " " & AgL.Chk_Text(TxtDivision.AgSelectedValue) & ", " & AgL.Chk_Text(TxtSite_Code.AgSelectedValue) & ",  " &
    ...
    Private Sub Txt_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TxtV_Type.Validating, TxtManualRefNo.Validating, TxtFromGodown.Validating
        Select Case sender.NAME
            ...
            Case TxtFromGodown.Name
                TxtManualRefNo.Text = FGetManualRefNo("ManualRefNo", "StockHead_Log", TxtV_Type.AgSelectedValue, TxtV_Date.Text, TxtDivision.AgSelectedValue, TxtSite_Code.AgSelectedValue, TxtFromGodown.AgSelectedValue, ClsMain.ManualRefType.Max)
    ...
    Public Function FGetManualRefNo(ByVal FieldName As String, ByVal TableName As String, ByVal V_Type As String, ByVal V_Date As String, ByVal Div_Code As String, ByVal Site_Code As String, ByVal Godown_Code As String, Optional ByVal RefType As ClsMain.ManualRefType = ClsMain.ManualRefType.Max) As String
        ...
        Select Case RefType
            Case Else
                mQry = "Select IfNull(Max(Cast(Replace(Replace(" & FieldName & ",'-',''),'.','') as integer)),0)+1 From " & TableName & "  WHERE ABS(" & FieldName & ")>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "' and FromGodown = '" & Godown_Code & "' And V_Date Between '" & CDate(mStartDate).ToString("u") & "' and  '" & CDate(mEndDate).ToString("u") & "'  "
        ...
        Try
            ...
            Else
                If Dgl1.AgHelpDataSet(Col1Item) IsNot Nothing Then
                    ...
                    Dgl1.Item(Col1CurrentStock, mRow).Value = ClsMain.FunRetStock(Dgl1.AgSelectedValue(Col1Item, mRow), mInternalCode, , TxtFromGodown.AgSelectedValue, , , TxtV_Date.Text, Dgl1.Item(Col1LotNo, mRow).Value)
        ...
        With Dgl1
            For I = 0 To .Rows.Count - 1
                If .Item(Col1Item, I).Value <> "" Then
                    ...
                    '        " And IfNull(Process,'') = '" & .Item(Col1Process, I).Value & "' " & _
                    '        " And IfNull(Status,'Standard') = '" & .Item(Col1Status, I).Value & "' " & _
                    ...
                    If Val(.Item(Col1CurrentStock, I).Value) < Val(.Item(Col1Qty, I).Value) Then
                        If MsgBox("Qty of " & .Item(Col1Item, I).Value & " In  Lot No " & .Item(Col1LotNo, I).Value & " is less than " & Dgl1.Item(Col1Qty, I).Value & "", MsgBoxStyle.YesNo) = MsgBoxResult.No Then
                            .CurrentCell = .Item(Col1Qty, I) : Dgl1.Focus()
                            passed = False : Exit Sub
                        End If
                    End If
    ...
    Private Function FCheckDuplicateRefNo() As Boolean
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = " SELECT COUNT(*) FROM StockHead WHERE ManualRefNo = '" & TxtManualRefNo.Text & "'   " &
                    " AND V_Type ='" & TxtV_Type.AgSelectedValue & "'  And Div_Code = '" & TxtDivision.AgSelectedValue & "' And Site_Code = '" & TxtSite_Code.AgSelectedValue & "' And FromGodown = '" & TxtFromGodown.AgSelectedValue & "' And IfNull(IsDeleted,0) = 0  "
        ...
        Else
            mQry = " SELECT COUNT(*) FROM StockHead WHERE ManualRefNo = '" & TxtManualRefNo.Text & "'  " &
                    " AND V_Type ='" & TxtV_Type.AgSelectedValue & "'  And Div_Code = '" & TxtDivision.AgSelectedValue & "' And FromGodown = '" & TxtFromGodown.AgSelectedValue & "' And Site_Code = '" & TxtSite_Code.AgSelectedValue & "' And IfNull(IsDeleted,0) = 0 AND DocID <>'" & mInternalCode & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then FCheckDuplicateRefNo = False : MsgBox("Reference No. Already Exists") : TxtManualRefNo.Focus()
        End If
    End Function
    ...
    Private Sub Txt_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtFromGodown.Enter, TxtToGodown.Enter
        Select Case sender.name
            Case TxtFromGodown.Name
                TxtFromGodown.AgRowFilter = " Div_Code = '" & AgL.PubDivCode & "' " &
                                            " And Site_Code = '" & TxtSite_Code.AgSelectedValue & "'  " &
                                            " And Status = '" & AgTemplate.ClsMain.EntryStatus.Active & "' " &
                                            " And IsDeleted = 0 "

            Case TxtToGodown.Name
                TxtToGodown.AgRowFilter = " Site_Code = '" & TxtSite_Code.AgSelectedValue & "' " &
                                          " And Status = '" & AgTemplate.ClsMain.EntryStatus.Active & "' " &
                                          " And IsDeleted = 0 "
