...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Adjustment Type], H.V_Prefix AS Prefix, H.V_Date AS Date, H.V_No AS [Entry No], " &
                ...
                " H.MoveToLogDate AS [Move To Log Date], H.Status, H.ReferenceDocID AS [Reference No], H.Structure, H.OrderBy AS [ORDER By], H.ManualRefNo AS [Manual No], " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE StockHead_Log " &
                " SET " &
                " TotalQty = " & Val(LblTotalPhysicalQty.Text) & ", " &
                " Amount = " & Val(LblTotalAmount.Text) & ", " &
                " FromGodown = " & AgL.Chk_Text(TxtGodown.AgSelectedValue) & ", " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = "INSERT INTO PhysicalStockDetail_Log(UID, DocId, Sr, Item, Process, " &
                            " Status, CurrentStock, PhysicalStock, Unit, " &
                            " Rate, Amount, Differnce) " &
                            " VALUES('" & mSearchCode & "', '" & mInternalCode & "', " & Val(mSr) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1Item, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1Process, I)) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1Status, I).Value) & ", " &
                            " " & Val(.Item(Col1CurrentStock, I).Value) & ", " &
                            " " & Val(.Item(Col1PhysicalStock, I).Value) & ", " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = "SELECT I.Code, I.Description, I.Unit, I.ItemType, I.SalesTaxPostingGroup, Measure, MeasureUnit , " &
                " IfNull(I.IsDeleted ,0) AS IsDeleted, IfNull(I.Status,'" & ClsMain.EntryStatus.Active & "') AS Status, " &
                " I.Div_Code, I.Unit FROM Item I"
        Dgl1.AgHelpDataSet(Col1Item, 8) = AgL.FillData(mQry, AgL.GCn)

        mQry = "SELECT Code, Description, Site_Code, IfNull(IsDeleted,0) as IsDeleted, IfNull(Status,'" & ClsMain.EntryStatus.Active & "') AS Status FROM Godown Order By Description"
