...
#End Region
    ...
    Private Sub TempGr_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Approve_InTrans
        Dim I As Integer

        '------------------------------------------------------------------------
        'Updating Purchase Quotation Selection Detail In Purchase Quotation As Null
        '-------------------------------------------------------------------------
        mQry = " UPDATE PurchQuotationDetail " &
                " SET QuotSelection = NULL, " &
                " QuotSelectionIndex = NULL, " &
                " QuotSelectionV_Type = NULL, " &
                " QuotSelectionV_No = NULL, " &
                " QuotSelectionV_Date = NULL " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If AgL.StrCmp(.Item(Col1Select, I).Value, AgLibrary.ClsConstant.StrCheckedValue) Then
                    mQry = " UPDATE PurchQuotationDetail " &
                            " SET QuotSelection = " & AgL.Chk_Text(mInternalCode) & ", " &
                            " QuotSelectionIndex = " & Val(.Item(Col1SelectionIndex, I).Value) & ", " &
                            " QuotSelectionV_Type = " & AgL.Chk_Text(TxtV_Type.AgSelectedValue) & ", " &
                            " QuotSelectionV_No = " & Val(TxtV_No.Text) & ", " &
                            " QuotSelectionV_Date = " & AgL.Chk_Text(TxtV_Date.Text) & " " &
                            " WHERE DocId = '" & .AgSelectedValue(Col1PurchQuot, I) & "' " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Selection Type], H.V_Prefix AS Prefix, H.V_Date AS [Selection Date], H.V_No AS [Selection No], " &
                            " H.Structure, H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status],  " &
                            " H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, " &
                            " D.Div_Name AS Division,SM.Name AS [Site Name], SGE.DispName AS [Employee Name], I.Description AS  Item " &
                            " FROM  PurchQuotSelection H " &
                            " LEFT JOIN Division D ON D.Div_Code=H.Div_Code  " &
                            " LEFT JOIN SiteMast SM ON SM.Code=H.Site_Code  " &
                            " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                            " LEFT JOIN SubGroup SGE ON SGE.SubCode  = H.Employee  " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Type AS [Selection Type], H.V_Prefix AS Prefix, H.V_Date AS [Selection Date], H.V_No AS [Selection No], " &
                    " H.Structure, H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status],  " &
                    " H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, " &
                    " D.Div_Name AS Division,SM.Name AS [Site Name], SGE.DispName AS [Employee Name], I.Description AS  Item " &
                    " FROM  PurchQuotSelection_Log H " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = " UPDATE PurchQuotSelection_Log " &
                " SET Employee = " & AgL.Chk_Text(TxtEmployee.AgSelectedValue) & ", " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If AgL.StrCmp(.Item(Col1Select, I).Value, AgLibrary.ClsConstant.StrCheckedValue) Then
                    mSr += 1
                    mQry = " INSERT INTO PurchQuotSelectionDetail_Log(UID, DocId, Sr, SelectionIndex, " &
                            " Remarks, PurchIndent, " &
                            " PurchQuot, PurchQuotV_Type, PurchQuotV_No, PurchQuotV_Date, Vendor, VendorName, " &
                            " Currency, BillingType, VendorQuoteNo, VendorQuoteDate, TermsAndConditions, " &
                            " TotalQty, TotalMeasure, TotalAmount, NetAmount, PriceMode) " &
                            " VALUES (" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(mInternalCode) & ", " & mSr & ", " &
                            " " & Val(.Item(Col1SelectionIndex, I).Value) & ", " & AgL.Chk_Text(.Item(Col1Remarks, I).Value) & ",  " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1PurchIndent, I)) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1PurchQuot, I)) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1PurchQuotV_Type, I).Value) & ", " &
                            " " & Val(.Item(Col1PurchQuotV_No, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1PurchQuotV_Date, I).Value) & ", " &
                            " " & AgL.Chk_Text(.AgSelectedValue(Col1Vendor, I)) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1Vendor, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1Currency, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1BillingType, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1VendorQuoteNo, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1VendorQuoteDate, I).Value) & ", " &
                            " " & AgL.Chk_Text(.Item(Col1TermsAndConditions, I).Value) & ", " &
                            " " & Val(.Item(Col1TotalQty, I).Value) & ", " &
                            " " & Val(.Item(Col1TotalMeasure, I).Value) & ", " &
                            " " & Val(.Item(Col1TotalAmount, I).Value) & ", " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        If FrmType = ClsMain.EntryPointType.Main Then
            mQry = "Select P.* " &
                    " From PurchQuotSelection P " &
                    " Where P.DocID = '" & SearchCode & "'"
        Else
            mQry = "Select P.* " &
                    " From PurchQuotSelection_Log P " &
    ...
    Private Sub ProcFillPurchIndentDetail()
        ...
        Dim I As Integer = 0
        Try
            mQry = " SELECT H.PurchIndent, H.DocID AS PurchQuot, H.V_Type AS PurchQuotV_Type, " &
                        " H.V_No AS PurchQuotV_No, H.V_Date AS PurchQuotV_Date, H.Vendor, H.Currency, " &
                        " H.BillingType, H.VendorQuoteNo, H.VendorQuoteDate, H.TermsAndConditions, H.TotalQty, " &
                        " H.TotalMeasure, H.TotalAmount, H.PriceMode, " &
                        " Sh.Amount As NetAmount " &
                        " FROM PurchQuotation H " &
                        " LEFT JOIN PurchQuotationDetail  L ON H.DocID = L.DocId " &
                        " LEFT JOIN Structure_TransFooter Sh On H.DocId = Sh.DocId And Sh.Charges = '" & ClsMain.Charges.NETAMOUNT & "'" &
                        " WHERE L.Item = '" & TxtItem.AgSelectedValue & "' " &
    ...
    Private Sub TempPurchQuotSelection_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = "Select H.SubCode As Code, Sg.DispName As Employee, Sg.ManualCode , " &
        ...
        mQry = " SELECT H.Code AS Code, H.Description AS Item, " &
                " IfNull(H.IsDeleted,0) As IsDeleted, " &
                " IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) As Status, " &
                " H.Div_Code, H.ItemType " &
                " FROM Item H  "
        HelpDataSet.Item = AgL.FillData(mQry, AgL.GCn)

        mQry = " SELECT H.DocID AS Code,H.V_Type + '-' + Convert(NVARCHAR(5),H.V_No) AS IndentNo , " &
                  " IfNull(H.IsDeleted,0) AS IsDeleted, H.Div_Code , " &
                  " IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, " &
                  " H.V_Date As IndentDate, Vt.NCat, H.MoveToLog " &
                  " FROM PurchIndent  H " &
                  " LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type "
        HelpDataSet.PurchIndent = AgL.FillData(mQry, AgL.GCn)

        mQry = " SELECT H.DocID AS Code,H.V_Type + '-' + Convert(NVARCHAR(5),H.V_No) AS QuatationNo , " &
                " IfNull(H.IsDeleted,0) AS IsDeleted, H.Div_Code , " &
                " IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status , " &
                " H.V_Date As QuotationDate, Vt.NCat, H.MoveToLog  " &
                " FROM PurchQuotation  H " &
                " LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type "
        HelpDataSet.PurchQuotation = AgL.FillData(mQry, AgL.GCn)

        mQry = "SELECT Sg.SubCode, Sg.DispName AS [Name], " &
               " IfNull(Sg.IsDeleted,0) AS IsDeleted, " &
               " IfNull(SG.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, " &
               " Sg.Div_Code " &
               " FROM Vendor H " &
    ...
    Private Sub TempPurchQuotSelectionSelection_BaseEvent_ApproveDeletion_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_ApproveDeletion_InTrans
        mQry = " UPDATE PurchQuotationDetail " &
                " SET QuotSelection = NULL, " &
                " QuotSelectionIndex = NULL, " &
                " QuotSelectionV_Type = NULL, " &
