...
#End Region
    ...
    Private Sub FrmShade_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            ...
            mQry = "Select count(*) From QcGroup_log Where Description='" & TxtLQCGroup.Text & "' And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IfNull(MoveToLog,'')=''  "
        ...
        Else
            ...
            mQry = "Select count(*) From QcGroup_log Where Description='" & TxtLQCGroup.Text & "' And UID <>'" & mSearchCode & "' And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IfNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then MsgBox("QC Group Already Exists in Log File") : passed = False : Exit Sub
        End If
    End Sub
    ...
    Private Sub FrmQCGroup_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = "SELECT S.UID, S.Description [QC Group] " &
                        " FROM QcGroup_Log  S " &
                        " LEFT JOIN SiteMast C On S.Site_Code = C.Code " & mConStr &
    ...
    Private Sub FrmShade_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1  " & AgL.RetDivisionCondition(AgL, "S.Div_Code") & " "
        AgL.PubFindQry = "SELECT S.Code, S.Description [QC Group] " &
                        " FROM QcGroup  S " &
                        " LEFT JOIN SiteMast C On S.Site_Code = C.Code  " & mConStr &
    ...
    Private Sub FrmShade_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "Update QcGroup_Log " &
                "   SET  " &
                "	Site_Code = " & AgL.Chk_Text(AgL.PubSiteCode) & ", " &
                "	Description = " & AgL.Chk_Text(TxtLQCGroup.Text) & " " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Parameter, I).Value <> "" Then
                    ...
                    mQry = " INSERT INTO QcGroupDetail_Log (Code, Sr, Parameter, StdValue, UID) " &
    ...
    Private Sub FrmQuality1_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = "Select Code, Description As QCGroup, Div_Code ,IfNull( IsDeleted,0) as IsDeleted " &
            " From QcGroup  " &
