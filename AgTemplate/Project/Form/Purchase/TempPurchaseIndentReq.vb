...
#End Region
    ...
    Private Sub TempPurchIndentReq_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Approve_InTrans
                ...
                For I = 0 To .RowCount - 1
                    If .Item(Col2Item, I).Value <> "" Then
                        ...
                        If .AgSelectedValue(Col2TempRequisionNo, I) <> "" Then
                            mQry = " UPDATE RequisitionDetail SET PurchaseIndent = NULL " &
                                    " WHERE DocId = " & AgL.Chk_Text(Dgl2.AgSelectedValue(Col2TempRequisionNo, I)) & "  " &
                ...
                For I = 0 To .RowCount - 1
                    If .Item(Col2Item, I).Value <> "" Then
                        ...
                        If .AgSelectedValue(Col2RequisionNo, I) <> "" Then
                            mQry = " UPDATE RequisitionDetail SET PurchaseIndent = " & AgL.Chk_Text(mInternalCode) & " " &
                                    " WHERE DocId = " & AgL.Chk_Text(Dgl2.AgSelectedValue(Col2RequisionNo, I)) & "  " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = "SELECT P.DocID, Vt.Description AS [Entry Type], P.V_Date AS [Entry Date], " &
                            " P.V_No AS [Entry No], Sg.DispName As Indentor, D.Description As Department " &
                            " FROM PurchIndent P " &
                            " LEFT JOIN Voucher_type Vt ON P.V_Type = Vt.V_Type " &
                            " LEFT JOIN Department D On P.Department = D.Code " &
                            " Left Join SubGroup Sg On P.Indentor = Sg.SubCode " &
                            " Where 1=1 " & mCondStr
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT P.UID as SearchCode, P.DocId, Vt.Description AS [Entry Type], " &
                            " P.V_Date AS [Entry Date], P.V_No AS [Entry No], Sg.DispName As Indentor, D.Description As Department " &
                            " FROM PurchIndent_Log P " &
                            " LEFT JOIN Voucher_Type Vt ON P.V_Type = Vt.V_Type " &
                            " LEFT JOIN Department D On P.Department = D.Code " &
                            " Left Join SubGroup Sg On P.Indentor = Sg.SubCode " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        Dim I As Integer, mSr As Integer
        mQry = "UPDATE PurchIndent_Log " &
                " SET Department = " & AgL.Chk_Text(TxtDepartment.AgSelectedValue) & ", " &
                " Indentor = " & AgL.Chk_Text(TxtIndentor.AgSelectedValue) & ", " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = "Insert Into PurchIndentDetail_Log(UID, DocId, Sr, Item, CurrentStock, ReqQty, IndentQty, " &
                            " Unit, MeasurePerPcs, MeasureUnit, TotalReqMeasure, TotalIndentMeasure, OrdQty,  " &
                            " OrdMeasure, PurchQty, PurchMeasure,RequireDate) " &
        ...
        With Dgl2
            For I = 0 To .RowCount - 1
                If .Item(Col2Item, I).Value <> "" Then
                    mSr += 1
                    mQry = " INSERT INTO PurchIndentReq_Log	( DocId, Sr, Requisition, Item, Qty,	" &
                            " Unit,	MeasurePerPcs,	MeasureUnit, TotalMeasure, RequireDate,	UID	)	" &
                            " VALUES ( " & AgL.Chk_Text(mInternalCode) & ",	" & mSr & "," & AgL.Chk_Text(Dgl2.AgSelectedValue(Col2RequisionNo, I)) & ", " &
                            " " & AgL.Chk_Text(Dgl2.AgSelectedValue(Col2Item, I)) & "," & Val(Dgl2.Item(Col2ReqQty, I).Value) & ",	" &
                            " " & AgL.Chk_Text(Dgl2.Item(Col2Unit, I).Value) & ", " & Val(Dgl2.Item(Col2ReqQty, I).Value) & "," & AgL.Chk_Text(Dgl2.Item(Col2MeasureUnit, I).Value) & ", " &
    ...
    Public Sub IniItemHelp(Optional ByVal All_Records As Boolean = True, Optional ByVal bRequisitionDocId As String = "")
        If All_Records = True Then
            mQry = " SELECT I.Code, I.Description, I.Unit, I.ItemType, I.SalesTaxPostingGroup , " &
                    " IfNull(I.IsDeleted ,0) AS IsDeleted, I.Div_Code, IfNull(I.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, " &
                    " I.Measure AS MeasurePerPcs, MeasureUnit, 0 AS Qty,0 AS TotalMeasure,'' AS RequireDate " &
                    " FROM Item I"
            Dgl2.AgHelpDataSet(Col2Item, 11) = AgL.FillData(mQry, AgL.GCn)
        Else
            If bRequisitionDocId <> "" Then
                mQry = " SELECT RD.Item AS Code,I.Description AS [Item Name],I.ItemType,RD.Qty ,RD.Unit,RD.MeasurePerPcs , " &
                        " RD.MeasureUnit, RD.TotalMeasure, RD.RequireDate ,0 As IsDeleted, '" & AgTemplate.ClsMain.EntryStatus.Active & "' AS  Status, " &
                        " '" & TxtDivision.AgSelectedValue & "' AS Div_Code " &
    ...
    Public Sub IniRequisitionHelp(Optional ByVal All_Records As Boolean = True, Optional ByVal bIndentDocId As String = "")
        If All_Records = True Then
            mQry = " SELECT R.DocID AS Code,R.V_Type + '-' +convert(NVARCHAR(5),R.V_No) AS Requisition ," &
                    " IfNull(R.IsDeleted,0) AS IsDeleted, R.Div_Code ," &
                    " IfNull(R.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, " &
                    " 1 AS CountItem, Vt.NCat " &
                    " FROM Requisition R " &
        ...
        Else
            If bIndentDocId <> "" Then
                mQry = " SELECT R.DocID AS Code,R.V_Type + '-' +convert(NVARCHAR(5),R.V_No) AS Requisition ," &
                        " IfNull(R.IsDeleted,0) AS IsDeleted, R.Div_Code ," &
                        " IfNull(R.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, " &
                        " V1.CountItem, Vt.NCat  " &
                        " FROM Requisition R " &
                        " LEFT JOIN  " &
                        " ( " &
                        " SELECT COUNT(RD.Item) AS CountItem ,RD.DocId " &
                        " FROM RequisitionDetail RD " &
                        " WHERE RD.PurchaseIndent Is NULL OR RD.PurchaseIndent = " & AgL.Chk_Text(bIndentDocId) & "" &
                        " GROUP BY RD.DocId " &
                        " ) V1 ON V1.DocId = R.DocID " &
                        " LEFT JOIN Voucher_Type Vt On R.V_Type = Vt.V_Type "
                Dgl2.AgHelpDataSet(Col2RequisionNo, 5) = AgL.FillData(mQry, AgL.GCn)
            End If
        End If
    ...
    Private Sub ProcFillRequisitionDetail()
        ...
        mQry = " SELECT R.DocID, RD.Item ,RD.Unit,RD.Qty ,RD.MeasurePerPcs,RD.MeasureUnit, " &
                " RD.TotalMeasure,RD.RequireDate  " &
                " FROM Requisition R " &
                " LEFT JOIN RequisitionDetail RD ON RD.DocId=R.DocID  " &
                " LEFT JOIN Voucher_Type Vt On R.V_Type = VT.V_Type " &
                " WHERE " & bConStr & " And IfNull(R.IsDeleted, 0) = 0 And R.V_Date <='" & TxtV_Date.Text & "' " &
                " And IfNull(R.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "')= '" & AgTemplate.ClsMain.EntryStatus.Active & "' " &
                " AND ( RD.PurchaseIndent IS NULL OR RD.PurchaseIndent = " & AgL.Chk_Text(mInternalCode) & " ) "
        DsTemp = AgL.FillData(mQry, AgL.GCn)
        With DsTemp.Tables(0)
            Dgl2.RowCount = 1
    ...
    Private Sub ProcFillIndentDetail()
        ...
        mQry = "  Declare @TmpTable as Table " &
                    " ( " &
                    " RequisionNo nVarchar(21), " &
                    " Item nVarchar(10), " &
                    " Qty Float, " &
                    " Unit nVarchar(10), " &
                    " MeasurePerPcs Float, " &
                    " MeasureUnit nVarchar(10), " &
                    " TotalReqMeasure Float, " &
                    " ReqDate SmallDateTime " &
                    " )"

        For I = 0 To Dgl2.RowCount - 1
            mQry += " Insert Into @TmpTable(  " &
                " RequisionNo, " &
                " Item,   " &
                " Qty,   " &
                " Unit,   " &
                ...
                " ) " &
                " Values ( " &
                " " & AgL.Chk_Text(Dgl2.AgSelectedValue(Col2RequisionNo, I)) & ", " &
                " " & AgL.Chk_Text(Dgl2.AgSelectedValue(Col2Item, I)) & ", " &
                " " & Val(Dgl2.Item(Col2ReqQty, I).Value) & ", " &
                " " & AgL.Chk_Text(Dgl2.Item(Col2Unit, I).Value) & ", " &
                " " & Val(Dgl2.Item(Col2MeasurePerPcs, I).Value) & ", " &
                " " & AgL.Chk_Text(Dgl2.Item(Col2MeasureUnit, I).Value) & ", " &
                " " & Val(Dgl2.Item(Col2TotalReqMeasure, I).Value) & ", " &
                " " & AgL.Chk_Text(Dgl2.Item(Col2ReqDate, I).Value) & " " &
                ")"
        Next

        mQry += " Select Item,sum(IfNull(Qty,0)) AS Qty,max(unit) AS Unit, " &
                " max(MeasurePerPcs) AS MeasurePerPcs,max(MeasureUnit) AS MeasureUnit, " &
                " sum(TotalReqMeasure) AS TotalReqMeasure,min(ReqDate) AS ReqDate " &
                " from @TmpTable " &
    ...
    Private Sub TempPurchIndentReq_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = "SELECT I.Code, I.Description, I.Unit, I.ItemType, I.SalesTaxPostingGroup , " &
               " IfNull(I.IsDeleted ,0) AS IsDeleted, I.Div_Code, IfNull(I.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, I.Measure, MeasureUnit " &
               " FROM Item I"
        ...
        mQry = " SELECT SG.SubCode AS Code,SG.DispName ,IfNull(SG.IsDeleted,0) AS IsDeleted, SG.Div_Code , " &
