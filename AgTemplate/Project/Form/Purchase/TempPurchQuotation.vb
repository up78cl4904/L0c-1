...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        '                    " Where IfNull(P.IsDeleted,0) = 0   " & mCondStr

        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Quotation Type], H.V_Prefix AS [Prefix], H.V_Date AS Date, H.V_No AS [Quotation No], " &
                            " H.VendorName AS [Vendor Name], H.Currency, H.Structure, H.BillingType AS [Billing Type], H.VendorQuoteNo AS [Vendor Quote No], H.VendorQuoteDate AS [Vendor Quote Date],  " &
                            " H.TermsAndConditions AS [Terms And Conditions], H.Remarks, H.TotalQty AS [Total Qty], H.TotalMeasure AS [Total Measure], H.TotalAmount AS [Total Amount],  " &
                            " H.PostingGroupSalesTaxParty AS [Posting Group Sales Tax Party], H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status],  " &
                            " H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status,  H.PriceMode AS [Price Mode], " &
                            " D.Div_Name AS Division, SM.Name AS [Site Name], PI.V_Type +'-'+ convert(NVARCHAR,PI.V_No) AS [Indent No], SF.* " &
                            " FROM  PurchQuotation H " &
                            " LEFT JOIN Division D ON D.Div_Code =H.Div_Code   " &
                            " LEFT JOIN SiteMast SM ON SM.Code=H.Site_Code   " &
                            " LEFT JOIN voucher_type Vt ON H.V_Type = vt.V_Type  " &
                            " LEFT JOIN PurchIndent PI ON PI.DocID=H.PurchIndent  " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Type AS [Quotation Type], H.V_Prefix AS [Prefix], H.V_Date AS Date, H.V_No AS [Quotation No], " &
                    " H.VendorName AS [Vendor Name], H.Currency, H.Structure, H.BillingType AS [Billing Type], H.VendorQuoteNo AS [Vendor Quote No], H.VendorQuoteDate AS [Vendor Quote Date],  " &
                    " H.TermsAndConditions AS [Terms And Conditions], H.Remarks, H.TotalQty AS [Total Qty], H.TotalMeasure AS [Total Measure], H.TotalAmount AS [Total Amount],  " &
                    " H.PostingGroupSalesTaxParty AS [Posting Group Sales Tax Party], H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status],  " &
                    " H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status,  H.PriceMode AS [Price Mode], " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE PurchQuotation_Log " &
                " SET " &
                ...
                " VendorQuoteNo = " & AgL.Chk_Text(TxtVendorQuoteNo.Text) & ", " &
                " VendorQuoteDate = " & AgL.ConvertDate(TxtVendorQuoteDate.Text) & ", " &
                " TermsAndConditions = " & AgL.Chk_Text(TxtTermsAndConditions.Text) & ", " &
                " BillingType = " & AgL.Chk_Text(TxtBillingType.AgSelectedValue) & ", " &
                " PostingGroupSalesTaxParty = " & AgL.Chk_Text(TxtSalesTaxGroupParty.AgSelectedValue) & ", " &
                ...
                " PurchIndent = " & AgL.Chk_Text(TxtIndentNo.AgSelectedValue) & " " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1Item, I).Value <> "" Then
                    mSr += 1
                    mQry = "Insert Into PurchQuotationDetail_Log(UID, DocId, Sr, PurchIndent, Item, Qty, Unit, MeasurePerPcs, " &
                            " MeasureUnit, TotalMeasure, Rate, Amount, OrdQty, OrdMeasure, " &
                            " PurchQty, PurchMeasure, PostingGroupSalesTaxItem, " &
                            " QuotSelection, QuotSelectionIndex, QuotSelectionV_Type, " &
                            " QuotSelectionV_No, QuotSelectionV_Date)" &
                            " Values(" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(mInternalCode) & ", " &
                            " " & mSr & ", " &
                            " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1IndentNo, I)) & ", " &
                            ...
                            " " & Val(Dgl1.Item(Col1OrdQty, I).Value) & ", " &
                            " " & Val(Dgl1.Item(Col1OrdMeasure, I).Value) & ", " &
                            " " & Val(Dgl1.Item(Col1PurchQty, I).Value) & ", " &
                            " " & Val(Dgl1.Item(Col1PurchMeasure, I).Value) & "," &
                            " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1SalesTaxGroup, I)) & ", " &
                            " " & AgL.Chk_Text(Dgl1.Item(Col1QuotSelection, I).Value) & ", " &
                            " " & Val(Dgl1.Item(Col1QuotSelectionIndex, I).Value) & ", " &
                            " " & AgL.Chk_Text(Dgl1.Item(Col1QuotSelectionV_Type, I).Value) & ", " &
                            " " & Val(Dgl1.Item(Col1QuotSelectionV_No, I).Value) & ", " &
                            " " & AgL.Chk_Text(Dgl1.Item(Col1QuotSelectionV_Date, I).Value) & " " &
                            " ) "
                    AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
        ...
        Select Case Dgl1.Columns(Dgl1.CurrentCell.ColumnIndex).Name
            Case Col1Item
                If Dgl1.Item(Col1IndentNo, Dgl1.CurrentCell.RowIndex).Value <> "" Then
                    mQry = " SELECT PID.Item AS Code ,I.Description AS Item,PID.IndentQty AS Qty,PID.Unit, " &
                            " I.ItemType,IfNull(I.IsDeleted ,0) AS IsDeleted, I.Div_Code,I.Measure, I.MeasureUnit, I.SalesTaxPostingGroup," &
                            " IfNull(I.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') AS Status " &
                            " FROM PurchIndentDetail PID " &
                            " LEFT JOIN Item I ON I.Code=PID.Item  " &
    ...
    Private Function FCheckDuplicateVendorNo() As Boolean
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = " SELECT COUNT(*) FROM PurchQuotation WHERE Vendor = '" & TxtVendor.AgSelectedValue & "' And VendorQuoteNo = '" & TxtVendorQuoteNo.Text & "'   " &
    ...
    Private Sub Txt_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles _
                TxtV_Type.Validating, TxtVendor.Validating, TxtBillingType.Validating,
                TxtRemarks.Validating, TxtTermsAndConditions.Validating, TxtV_Date.Validating,
        ...
        Try
            Select Case sender.name
                ...
                Case TxtVendor.Name
                    ...
                    TxtVendor.AgRowFilter = " IsDeleted = 0  " &
                ...
                Case TxtIndentNo.Name
                    TxtIndentNo.AgRowFilter = " IsDeleted = 0 " &
                        " And Status = '" & AgTemplate.ClsMain.EntryStatus.Active & "'  " &
                        " And " & ClsMain.RetDivFilterStr & " "
            End Select
        Catch ex As Exception
    ...
    Private Sub ProcFillPurchIndentDetail(ByVal PurchIndent As String)
        ...
        Try
            ...
            mQry = "SELECT L.*, I.SalesTaxPostingGroup " &
    ...
    Private Sub TempPurchQuotation_BaseFunction_Create() Handles Me.BaseFunction_CreateHelpDataSet
        mQry = "Select V.SubCode As Code, Sg.DispName As Vendor, Sg.ManualCode , " &
                " C.CityName As VendorCity, C.Country As VendorCountry, " &
                " IfNull(Sg.IsDeleted,0) As IsDeleted,  " &
                " IfNull(Sg.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) As Status,  " &
                " Sg.SalesTaxPostingGroup, Sg.Div_Code  " &
                " From Vendor V " &
                " LEFT JOIN SubGroup Sg On V.SubCode = Sg.SubCode " &
                " LEFT JOIN City C On Sg.CityCode = C.CityCode "
        HelpDataSet.Vendor = AgL.FillData(mQry, AgL.GCn)

        mQry = "SELECT C.Code, C.Code AS Currency, IfNull(C.IsDeleted,0) as IsDeleted   FROM Currency C"
        ...
        mQry = "SELECT DISTINCT Description AS Code, Description, IfNull(Active,0) As Active  FROM PostingGroupSalesTaxParty "
        HelpDataSet.SalesTaxGroupParty = AgL.FillData(mQry, AgL.GCn)

        'Start Code Change By Satyam on 18/11/2011
        mQry = " SELECT P.DocID AS Code,P.V_Type + '-' +convert(NVARCHAR(5),P.V_No) AS [Indent No], " &
                " SG.DispName AS [Indent By],P.V_Date AS [Indent Date], " &
                " IfNull(P.IsDeleted,0) AS IsDeleted, P.Div_Code , " &
                " IfNull(P.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "' ) AS Status, Vt.NCat, " &
                " P.MoveToLog " &
                " FROM PurchIndent  P " &
                " LEFT JOIN SubGroup SG On SG.SubCode = P.Indentor " &
                " LEFT JOIN Voucher_Type Vt On P.V_Type = Vt.V_Type "
        HelpDataSet.IndentNo = AgL.FillData(mQry, AgL.GCn)
        'End Code Change By Satyam on 18/11/2011
        mQry = "SELECT I.Code, I.Description, I.Unit, I.ItemType, I.SalesTaxPostingGroup , " &
                " IfNull(I.IsDeleted ,0) AS IsDeleted, I.Div_Code,  " &
                " IfNull(I.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status, I.Measure, " &
                " I.MeasureUnit, I.SalesTaxPostingGroup " &
                " FROM Item I "
