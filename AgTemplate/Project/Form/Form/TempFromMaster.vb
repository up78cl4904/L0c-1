...
#End Region
    ...
    Private Sub FrmDocument_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.Description AS Form, H.Category, H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], " &
                    " H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date],  " &
                    " H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status,D.Div_Name AS Division  " &
                    " FROM  Form_Master_Log H " &
                    " LEFT JOIN Division D ON D.Div_Code=H.Div_Code  " & mConStr &
    ...
    Private Sub FrmDocument_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.Code AS SearchCode, H.Description AS Form, H.Category, H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], " &
                            " H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By], H.ApproveDate AS [Approve Date],  " &
                            " H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status,D.Div_Name AS Division  " &
                            " FROM  Form_Master H " &
                            " LEFT JOIN Division D ON D.Div_Code=H.Div_Code  " & mConStr &
    ...
    Private Sub FrmDocument_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = "Update Form_Master_Log " &
                "   SET  " &
                "	Description = " & AgL.Chk_Text(TxtDescription.Text) & ", " &
                "	Category = " & AgL.Chk_Text(TxtCategory.Text) & " " &
    ...
    Private Sub FrmQuality1_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        ...
        mQry = " Select 'Road Permit' As Code, 'Road Permit' As Category " &
                " Union All " &
                " Select 'From C' As Code, 'From C' As Category " &
                " Union All " &
