...
Public Class TempFormIssue
    ...
    Protected Const Col1IssueDate As String = "Issue Date"
    Protected Const Col1IssueTo As String = "Issue To"
...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.LblPartyReq.Location = New System.Drawing.Point(122, 80)
        Me.LblPartyReq.Name = "LblPartyReq"
        Me.LblPartyReq.Size = New System.Drawing.Size(10, 7)
        Me.LblPartyReq.TabIndex = 769
        ...
        Me.Name = "FrmFormIssue"
        Me.Text = "From Issue Entry"
...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Type AS [Receive Type], H.V_Prefix AS Prefix, H.V_Date AS [Receive Date], H.V_No AS [Receive No], " &
                            " H.FormPrefix AS [Form Prefix], H.FormNoFrom AS [Form No From], H.FormNoUpTo AS [Form No Up To], H.IsFromDepartment AS [IS From Department],  " &
                            ...
                            " H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, " &
                            " D.Div_Name AS Division,SM.Name AS [Site Name], FM.Description AS Form, SGP.DispName AS [Party Name] " &
                            " FROM Form_Receive H " &
                            " LEFT JOIN Division D ON D.Div_Code=H.Div_Code  " &
                            " LEFT JOIN SiteMast SM ON SM.Code=H.Site_Code  " &
                            " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                            " LEFT JOIN Form_Master FM ON FM.Code=H.Form  " &
                            " LEFT JOIN SubGroup SGP ON SGP.SubCode  = H.Party  " &
                            " Where 1=1 " & mCondStr
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Type AS [Receive Type], H.V_Prefix AS Prefix, H.V_Date AS [Receive Date], H.V_No AS [Receive No], " &
                    " H.FormPrefix AS [Form Prefix], H.FormNoFrom AS [Form No From], H.FormNoUpTo AS [Form No Up To], H.IsFromDepartment AS [IS From Department],  " &
                    " H.EntryBy AS [Entry By], H.EntryDate AS [Entry Date], H.EntryType AS [Entry Type], H.EntryStatus AS [Entry Status], H.ApproveBy AS [Approve By],  " &
                    " H.ApproveDate AS [Approve Date], H.MoveToLog AS [Move To Log], H.MoveToLogDate AS [Move To Log Date], H.Status, " &
                    " D.Div_Name AS Division,SM.Name AS [Site Name], FM.Description AS Form, SGP.DispName AS [Party Name] " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_IniGrid() Handles Me.BaseFunction_IniGrid
        ...
        With AgCL
            ...
            .AddAgDateColumn(Dgl1, Col1IssueDate, 100, Col1IssueDate, True, True, False)
            .AddAgTextColumn(Dgl1, Col1IssueTo, 210, 5, Col1IssueTo, True, True, False)
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        Dim I As Integer
        mQry = "UPDATE Form_Receive_Log " &
                " SET " &
                " Form = " & AgL.Chk_Text(TxtForm.AgSelectedValue) & ", " &
                " FormPrefix = " & AgL.Chk_Text(TxtFormPrefix.Text) & ", " &
                " FormNoFrom = " & AgL.Chk_Text(TxtFormNoFrom.Text) & ", " &
                " FormNoUpTo = " & AgL.Chk_Text(TxtFormNoUpto.Text) & ", " &
                " Party = " & AgL.Chk_Text(TxtParty.AgSelectedValue) & " " &
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1FormNo, I).Value <> "" Then
                    mQry = " UPDATE Form_ReceiveDetail_Log " &
                                " SET IssueDate = " & AgL.Chk_Text(.Item(Col1IssueDate, I).Value) & ", " &
                                " IssueTo = " & AgL.Chk_Text(.AgSelectedValue(Col1IssueTo, I)) & ", " &
                                " IssueDocId = " & AgL.Chk_Text(mInternalCode) & ", " &
                                " IsInStock = " & Val(.Item(Col1IsInStock, I).Value) & ", " &
                                " Status = 'Issued' " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                '-------------------------------------------------------------
                If FrmType = ClsMain.EntryPointType.Main Then
                    mQry = "Select * from Form_ReceiveDetail where IssueDocId = '" & SearchCode & "' Order By Sr"
                Else
                    mQry = "Select * from Form_ReceiveDetail_Log where IssueDocId = '" & mInternalCode & "' Order By Sr"
                End If
                ...
                With DsTemp.Tables(0)
                    ...
                    If .Rows.Count > 0 Then
                        For I = 0 To DsTemp.Tables(0).Rows.Count - 1
                            ...
                            Dgl1.Item(Col1IssueDate, I).Value = AgL.XNull(.Rows(I)("IssueDate"))
                            Dgl1.AgSelectedValue(Col1IssueTo, I) = AgL.XNull(.Rows(I)("IssueTo"))
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = "SELECT Rd.Form AS Code, Max(M.Description) AS Form, " &
                " Sum(Convert(INT, IfNull(Rd.IsInStock,0))) AS IsInStock, " &
                " Sum(Convert(INT, IfNull(R.IsFromDepartment,0))) AS IsFromDepartment " &
                " FROM Form_ReceiveDetail Rd  " &
                " LEFT JOIN Form_Receive R ON Rd.DocId = R.DocID " &
                " LEFT JOIN Form_Master M ON Rd.Form = M.Code " &
                " GROUP BY Rd.Form "
        TxtForm.AgHelpDataSet(2, TabControl1.Top + TP1.Top, TabControl1.Left + TP1.Left) = AgL.FillData(mQry, AgL.GCn)
        Dgl1.AgHelpDataSet(Col1Form) = TxtForm.AgHelpDataSet

        mQry = "SELECT Sg.SubCode AS Code, Sg.DispName AS Name, Sg.ManualCode " &
                " FROM SubGroup Sg " &
        ...
        Dgl1.AgHelpDataSet(Col1IssueTo) = TxtParty.AgHelpDataSet

        mQry = " SELECT DISTINCT  F.FormPrefix AS Code, F.FormPrefix AS FormPrefix, F.Form " &
                " FROM Form_Receive F "
        TxtFormPrefix.AgHelpDataSet(1, TabControl1.Top + TP1.Top, TabControl1.Left + TP1.Left) = AgL.FillData(mQry, AgL.GCn)
    End Sub
    ...
    Private Sub ProcFill()
        ...
        Try
            mQry = " SELECT Rd.Code, Rd.Form, Rd.FormNo " &
                    " FROM Form_ReceiveDetail Rd " &
                    " Where Rd.FormPrefix = '" & TxtFormPrefix.Text & "' " &
                    " And FormNoSerial BETWEEN " & Val(TxtFormNoFrom.Text) & " And " & Val(TxtFormNoUpto.Text) & " " &
                    " And IfNull(IsInStock,1) <> 0 And IfNull(Status,'Stock') = 'Stock'    "
            ...
            With DTTemp
                ...
                If .Rows.Count > 0 Then
                    For I = 0 To .Rows.Count - 1
                        ...
                        Dgl1.AgSelectedValue(Col1Form, I) = AgL.XNull(.Rows(I)("Form"))
                        Dgl1.Item(Col1FormNo, I).Value = AgL.XNull(.Rows(I)("FormNo"))
                        Dgl1.Item(Col1IssueDate, I).Value = TxtV_Date.Text
                        Dgl1.AgSelectedValue(Col1IssueTo, I) = TxtParty.AgSelectedValue
                        Dgl1.Item(Col1IsInStock, I).Value = 0
                    Next I
                End If
            End With
    ...
    Private Sub FrmFormIssue_BaseFunction_DispText() Handles Me.BaseFunction_DispText
    ...
    Private Sub FrmFormIssue_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Approve_InTrans
        Dim I As Integer
        With Dgl1
            For I = 0 To .Rows.Count - 1
                mQry = " UPDATE Form_ReceiveDetail " &
                        " SET Form_ReceiveDetail.IssueDate = Form_ReceiveDetail_Log.IssueDate ,  " &
                        " Form_ReceiveDetail.IssueTo = Form_ReceiveDetail_Log.IssueTo , " &
                        " Form_ReceiveDetail.IssueDocId = Form_ReceiveDetail_Log.IssueDocId, " &
                        " Form_ReceiveDetail.IsInStock = Form_ReceiveDetail_Log.IsInStock, " &
                        " Form_ReceiveDetail.Status = Form_ReceiveDetail_Log.Status " &
                        " FROM Form_ReceiveDetail_Log " &
                        " WHERE Form_ReceiveDetail.Code = Form_ReceiveDetail_Log.Code " &
                        " AND Form_ReceiveDetail_Log.Code = '" & .Item(Col1Code, I).Value & "'  "
                AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
            Next
        End With
    End Sub

    Private Sub FrmFormIssue_BaseEvent_Topctrl_tbDel() Handles Me.BaseEvent_Topctrl_tbDel
        mQry = " UPDATE Form_ReceiveDetail_Log " &
                   " SET IssueDate = NULL, " &
                   " IssueTo = NULL, " &
                   " IssueDocId = NULL, " &
                   " IsInStock = 1, " &
                   " Status = 'Stock' " &
                   " WHERE IssueDocId = " & AgL.Chk_Text(mInternalCode) & " "
        AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
    End Sub

    Private Sub TxtShipToPartyCity_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtForm.Enter, TxtFormPrefix.Enter
        Try
            Select Case sender.name
                Case TxtForm.Name
                    TxtForm.AgRowFilter = " IsInStock > 0 And IsFromDepartment > 0   "

                Case TxtFormPrefix.Name
                    TxtFormPrefix.AgRowFilter = " Form = '" & TxtForm.AgSelectedValue & "'   "
