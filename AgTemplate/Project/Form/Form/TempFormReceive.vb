...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = "UPDATE Form_Receive_Log " &
                ...
                " IsFromDepartment = " & Val(IIf(RbtFromDepartment.Checked, 1, 0)) & ", " &
            ...
            For I = 0 To .RowCount - 1
                If .Item(Col1FormNo, I).Value <> "" Then
                    If .Item(Col1Code, I).Value = "" Then
                        ...
                        mSr += 1
                        mQry = " INSERT INTO Form_ReceiveDetail_Log(UID, DocId, Sr, Form, FormNo, ReceiveDate, ReceiveFrom, " &
                                    " IsInStock, Status, FormPrefix, FormNoSerial, Code) " &
                                    " VALUES(" & AgL.Chk_Text(mSearchCode) & ", " & AgL.Chk_Text(mInternalCode) & ",  " &
                                    " " & mSr & ", " & AgL.Chk_Text(.AgSelectedValue(Col1Form, I)) & ", " &
                                    " " & AgL.Chk_Text(.Item(Col1FormNo, I).Value) & ",  " &
                                    " " & AgL.Chk_Text(.Item(Col1ReceiveDate, I).Value) & ", " &
                                    " " & AgL.Chk_Text(.AgSelectedValue(Col1ReceiveFrom, I)) & ", " &
                                    " " & Val(.Item(Col1IsInStock, I).Value) & ", " &
                                    " " & AgL.Chk_Text(.Item(Col1Status, I).Value) & ", " &
                                    " " & AgL.Chk_Text(TxtFormPrefix.Text) & ", " &
                                    " " & Val(.Item(Col1FormNoSerial, I).Value) & ", " &
                                    " " & AgL.Chk_Text(bLineCode) & ") "
                        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
                    Else
                        mQry = " UPDATE Form_ReceiveDetail_Log SET " &
                                " Status = " & AgL.Chk_Text(.Item(Col1Status, I).Value) & " " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        ...
        mQry = " Select 'Stock' As Code, 'Stock' As Status " &
                " UNION ALL " &
                " Select 'Lost' As Code, 'Lost' As Status " &
                " UNION ALL " &
