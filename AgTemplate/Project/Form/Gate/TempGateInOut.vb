...
#End Region
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        ...
        AgL.PubFindQry = " SELECT H.DocID AS SearchCode, H.V_Date AS [IN date], H.V_Time AS [IN_Time], H.V_No AS [Entry_No], " &
                        " SG.DispName AS [Party_Name],H.PassNo , H.VehicleType AS [Vehicle_Type], H.VehicleNo AS [Vehicle_No], H.Item, H.Weight, H.Qty, H.Driver, H.Remarks,  " &
                        " H.Close_Date AS [OUT_Date], H.Close_Time AS [OUT_Time], H.Close_Remarks AS [OUT_Remarks], " &
                        " H.Close_EntryBy AS [OUT_EntryBy],  " &
                        " H.EntryBy AS [Entry_By], H.EntryDate AS [Entry_Date], H.EntryType AS [Entry_Type], " &
                        " H.EntryStatus AS [Entry_Status],  " &
                        " H.ApproveBy AS [Approve_By], H.ApproveDate AS [Approve_Date], H.MoveToLog AS [Move_To_Log], " &
                        " H.MoveToLogDate AS [Move_To_Log_Date],  " &
                        " H.Status, H.LrNo AS [LR_No], H.LrDate AS [LR_Date], H.Manual_RefNo AS [Manual_No], " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = " SELECT H.UID AS SearchCode, H.V_Date AS [IN date], H.V_Time AS [IN_Time], H.V_No AS [Entry_No], " &
                        " SG.DispName AS [Party_Name],H.PassNo , H.VehicleType AS [Vehicle_Type], H.VehicleNo AS [Vehicle_No], H.Item, H.Weight, H.Qty, H.Driver, H.Remarks,  " &
                        " H.Close_Date AS [OUT_Date], H.Close_Time AS [OUT_Time], H.Close_Remarks AS [OUT_Remarks], " &
                        " H.Close_EntryBy AS [OUT_EntryBy],  " &
                        " H.EntryBy AS [Entry_By], H.EntryDate AS [Entry_Date], H.EntryType AS [Entry_Type], " &
    ...
    Private Sub FrmProductionOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        mQry = " UPDATE GateInOut_Log " &
                " SET	SubCode = " & AgL.Chk_Text(TxtParty.AgSelectedValue) & ", " &
                " PassNo = " & AgL.Chk_Text(TxtPassNo.Text) & ", " &
                " Manual_RefNo = " & AgL.Chk_Text(TxtManualNo.Text) & ", " &
                " VehicleType = " & AgL.Chk_Text(TxtVehicleType.Text) & ", " &
                " V_Time = " & AgL.Chk_Text(TxtEntryTime.Text) & ", " &
                " VehicleNo = " & AgL.Chk_Text(TxtVehicleNo.Text) & ", " &
                " LrNo = " & AgL.Chk_Text(TxtLrNo.Text) & ", " &
                " LrDate = " & AgL.Chk_Text(TxtLrDate.Text) & ", " &
                " Item = " & AgL.Chk_Text(TxtItem.Text) & ", " &
                " Weight =" & Val(TxtWeight.Text) & ", " &
                " Qty = " & Val(TxtQty.Text) & ", " &
                " Transporter = " & AgL.Chk_Text(TxtTransporter.AgSelectedValue) & ", " &
                " Driver = " & AgL.Chk_Text(TxtDriverName.Text) & ", " &
    ...
    Private Sub FrmProductionOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        If FrmType = ClsMain.EntryPointType.Main Then
            mQry = "Select P.* , IfNull(SG.Add1,'')+IfNull(SG.Add2,'') AS Address ," &
                " SG.CityCode, SG.Phone" &
                " From GateInOut P " &
                " LEFT JOIN SubGroup SG ON SG.SubCode = P.SubCode" &
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = " SELECT SG.SubCode AS Code,SG.DispName AS Name,SG.Nature, " &
                " IfNull(SG.IsDeleted ,0) AS IsDeleted, SG.Div_Code, " &
                " IfNull(SG.Add1,'')+IfNull(SG.Add2,'') AS Address, SG.CityCode , SG.Phone ," &
    ...
    Private Sub TempGateInOut_BaseEvent_Topctrl_tbAdd() Handles Me.BaseEvent_Topctrl_tbAdd
        ...
        mQry = " SELECT IfNull(Max(substring(Manual_RefNo,7,4)),0) + 1  FROM GateInOut_Log " &
    ...
    Private Sub ProcSaveOutDetail(ByVal SearchCode As String)
        mQry = " UPDATE GateInOut_Log " &
                " SET " &
                " Close_Date = " & AgL.Chk_Text(TxtOutDate.Text) & ", " &
                " Close_Time = " & AgL.Chk_Text(TxtOutTime.Text) & ", " &
                " Close_EntryBy = " & AgL.Chk_Text(TxtOutEntryBy.Text) & ", " &
                " Close_Item = " & AgL.Chk_Text(TxtOutItem.Text) & ", " &
                " Close_Weight =" & Val(TxtOutWeight.Text) & ", " &
                " Close_Qty = " & Val(TxtOutQty.Text) & ", " &
                " Close_GatePassNo = " & AgL.Chk_Text(TxtGatePassNo.Text) & ", " &
                " Close_Remarks = " & AgL.Chk_Text(TxtOutRemark.Text) & " " &
                " Where UID = '" & mSearchCode & "'"
        AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)

        mQry = " UPDATE GateInOut " &
                " SET " &
                " Close_Date = " & AgL.Chk_Text(TxtOutDate.Text) & ", " &
                " Close_Time = " & AgL.Chk_Text(TxtOutTime.Text) & ", " &
                " Close_EntryBy = " & AgL.Chk_Text(TxtOutEntryBy.Text) & ", " &
