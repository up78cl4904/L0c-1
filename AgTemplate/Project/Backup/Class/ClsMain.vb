    ...
    Public Shared Function FunRetStock(ByVal bItem As String, ByVal bInternalCode As String, Optional ByVal bCondStr As String = "", _
        ...
        Try
            ...
            mQry = "SELECT IsNull(Sum(S.Qty_Rec),0) - IsNull(Sum(S.Qty_Iss),0) " & _
                    " FROM Stock S With (NoLock) " & bConStr
...
#End Region
    ...
    Public Shared Sub ProcStockPost(ByVal bTableName As String, ByVal bStock As ClsMain.StructStock, _
        ...
        With bStock
            mQry = "INSERT INTO " & bTableName & "(DocID, Sr, V_Type, V_Prefix, V_Date, V_No, RecID, Div_Code, Site_Code, " & _
                    " CostCenter, SubCode, Currency, SalesTaxGroupParty, Structure, BillingType, Item_Uid, Item, ProcessGroup, " & _
                    ...
                    " FIFORate, FIFOAmt, AVGRate, AVGAmt, Doc_Qty, ReferenceDocID) " & _
                    " VALUES (" & AgL.Chk_Text(.DocID) & ", " & Val(.Sr) & ", " & _
                    " " & AgL.Chk_Text(.V_Type) & ", " & AgL.Chk_Text(.V_Prefix) & ", " & _
                    " " & AgL.ConvertDate(.V_Date) & ",	" & Val(.V_No) & ",	" & AgL.Chk_Text(.RecID) & ", " & AgL.Chk_Text(.Div_Code) & ", " & _
                    " " & AgL.Chk_Text(.Site_Code) & ",	" & AgL.Chk_Text(.CostCenter) & ",	" & AgL.Chk_Text(.SubCode) & ",	" & _
                    " " & AgL.Chk_Text(.Currency) & ", " & AgL.Chk_Text(.SalesTaxGroupParty) & ", " & _
                    " " & AgL.Chk_Text(.Structure1) & ", " & AgL.Chk_Text(.BillingType) & ", " & _
                    " " & AgL.Chk_Text(.Item_Uid) & ",	" & AgL.Chk_Text(.Item) & ",	" & AgL.Chk_Text(.ProcessGroup) & ", " & _
    ...
    Public Shared Sub PostStructureLineToAccounts(ByVal FGMain As AgStructure.AgCalcGrid, ByVal mNarr As String, ByVal mDocID As String, ByVal mDiv_Code As String, _
                                               ...
                                               ByVal Conn As SqlClient.SqlConnection, ByVal Cmd As SqlClient.SqlCommand, Optional ByVal mCostCenter As String = "")
            ...
            For I = 0 To .Rows.Count - 1
                If Trim(AgL.XNull(.Rows(I)("PostAc"))) <> "" And Val(AgL.VNull(.Rows(I)("Amount"))) <> 0 Then
                    ...
                    mQry = "Insert Into Ledger(DocId,RecId,V_SNo,V_Date,SubCode,ContraSub,AmtDr,AmtCr," & _
                         ...
                         " TDSPer,TDS_Of_V_SNo,System_Generated,FormulaString,ContraText, CostCenter) Values " & _
    ...
    '            mQry = "Select IsNull(Max(Convert(Numeric,Replace(Replace(" & FieldName & ",'-',''),'.',''))),0)+1 From " & TableName & " With (NoLock) WHERE isnumeric(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "' And EntryStatus <> 'Discard' And V_Date = '" & AgL.PubLoginDate & "'  AND IsNumeric(" & FieldName & ") = 0 "
    ...
    '        Case Else
    '            If CDate(V_Date) >= CDate("01/Apr/2013") And CDate(V_Date) <= CDate("31/Mar/2014") Then
    '                mQry = "Select IsNull(Max(Convert(Numeric,Replace(Replace(" & FieldName & ",'-',''),'.',''))),0)+1 From " & TableName & " With (NoLock) WHERE isnumeric(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "' And EntryStatus <> 'Discard' And V_Date Between '" & mStartDate & "' and  '" & mEndDate & "'  AND IsNumeric(" & FieldName & ") = 0 "
    '                FGetManualRefNo = AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar
    ...
    Public Shared Function FGetManualRefNo(ByVal FieldName As String, ByVal TableName As String, ByVal V_Type As String, ByVal V_Date As String, ByVal Div_Code As String, ByVal Site_Code As String, Optional ByVal RefType As ManualRefType = ManualRefType.Max) As String
        ...
        Select Case RefType
            ...
            Case Else
                mRef_Prefix = AgL.XNull(AgL.Dman_Execute("Select Ref_Prefix From Voucher_Prefix Where V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' And Site_Code = '" & Site_Code & "' And Date_From = '" & mStartDate & "' And Date_To = '" & mEndDate & "'", AgL.GcnRead).ExecuteScalar)
                If mRef_Prefix = "" Then
                    If CDate(V_Date) >= CDate("01/Apr/2013") And CDate(V_Date) <= CDate("31/Mar/2014") Then
                        mQry = "Select IsNull(Max(Convert(Numeric,Replace(Replace(" & FieldName & ",'-',''),'.',''))),0)+1 From " & TableName & " With (NoLock) WHERE isnumeric(Replace(Replace(" & FieldName & ",'-',''),'.',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "' And EntryStatus <> 'Discard' And V_Date Between '" & mStartDate & "' and  '" & mEndDate & "'  AND IsNumeric(" & FieldName & ") = 0 "
                        FGetManualRefNo = AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar
                    ...
                    End If
                Else
                    mQry = "Select IsNull(Max(Convert(Numeric,Replace(Replace(Replace(" & FieldName & ",'-',''),'.',''),'" & mRef_Prefix & "',''))),0) + 1 From " & TableName & " With (NoLock) WHERE isnumeric(Replace(Replace(Replace(" & FieldName & ",'-',''),'.',''),'" & mRef_Prefix & "',''))>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "' And EntryStatus <> 'Discard' And V_Date Between '" & mStartDate & "' and  '" & mEndDate & "'  AND IsNumeric(" & FieldName & ") = 0 "
                    FGetManualRefNo = AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar

                    mRef_PadLength = AgL.VNull(AgL.Dman_Execute("Select Ref_PadLength From Voucher_Prefix Where V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' And Site_Code = '" & Site_Code & "' And Date_From = '" & mStartDate & "' And Date_To = '" & mEndDate & "'", AgL.GcnRead).ExecuteScalar)
	...
	Public Shared Function FCheckDuplicateRefNo(ByVal FieldName As String, ByVal TableName As String, _
                                          ByVal V_Type As String, ByVal V_Date As String, _
                                          ByVal Div_Code As String, ByVal Site_Code As String, _
                                          ByVal EntryMode As String, ByVal ReferenceNo As String, _
        ...
        If AgL.StrCmp(EntryMode, "Add") Then
            mQry = " SELECT COUNT(*) " & _
                    " FROM " & TableName & " " & _
                    " WHERE " & FieldName & " = '" & ReferenceNo & "'   " & _
                    " AND V_Type ='" & V_Type & "'  " & _
                    " And Div_Code = '" & Div_Code & "' " & _
                    " And Site_Code = '" & Site_Code & "' " & _
    ...
    '                        " Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then L.LotNo ELse Null End As LotNo, " & _
    ...
    '                        " Group By L.Item, Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then L.LotNo ELse Null End, " & _
    '                        " L.Process, L.Dimension1, L.Dimension2 "

    '        mTillTransactionDateStockQry = " Select S.Item, " & _
    '                            " Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo ELse Null End As LotNo, " & _
    '                            " S.Process, S.Dimension1, S.Dimension2, " & _
    '                            " IsNull(Sum(S.Qty_Rec),0) - IsNull(Sum(S.Qty_Iss),0) As TillTransactionDateStock " & _
    ...
    '                            " Group By S.Item, Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo ELse Null End, " & _
    '                            " S.Process, S.Dimension1, S.Dimension2 "

    '        mCurrentStockQry = " Select S.Item, " & _
    '                " Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo ELse Null End As LotNo, " & _
    '                " S.Process, S.Dimension1, S.Dimension2, " & _
    '                " Round(IsNull(Sum(S.Qty_Rec),0),isnull(Max(U.Decimalplaces),0)) - Round(IsNull(Sum(S.Qty_Iss),0),isnull(Max(U.Decimalplaces),0)) As CurrentStock " & _
    ...
    '        mQry = " Select H.Item, H.LotNo, I.Description As ItemDesc, P.Description As ProcessDesc, IsNull(H.Qty,0) As Qty, Round(IsNull(L.CurrentStock,0),isnull(U.Decimalplaces,0)) As Stock " & _
    '                " From (" & mItemInTransactionQry & ") As H " & _
    '                " LEFT JOIN (" & mCurrentStockQry & ") As L On H.Item = L.Item COLLATE DATABASE_DEFAULT  " & _
    '                "                  And IsNull(H.LotNo,'') = IsNull(L.LotNo,'') COLLATE DATABASE_DEFAULT  " & _
    '                "                  And IsNull(H.Process,'') = IsNull(L.Process,'') COLLATE DATABASE_DEFAULT  " & _
    '                "                  And IsNull(H.Dimension1,'') = IsNull(L.Dimension1,'') COLLATE DATABASE_DEFAULT  " & _
    '                "                  And IsNull(H.Dimension2,'') = IsNull(L.Dimension2,'') COLLATE DATABASE_DEFAULT  " & _
    ...
    '                " Where Round(IsNull(L.CurrentStock,0),isnull(U.Decimalplaces,0)) - IsNull(H.Qty,0) < 0 "
    '        DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
    '        If DtTemp.Rows.Count > 0 Then
    ...
    '            Next
    '        Else
    '            mQry = " Select H.Item, L.LotNo, I.Description As ItemDesc, P.Description As ProcessDesc, IsNull(H.Qty,0) As Qty, Round(IsNull(L.TillTransactionDateStock,0),isnull(U.Decimalplaces,0)) As Stock " & _
    '                    " From (" & mItemInTransactionQry & ") As H " & _
    ...
    '                    " LEFT JOIN Unit U ON U.Code = I.Unit COLLATE DATABASE_DEFAULT " & _
    '                    " LEFT JOIN Process P On H.Process = P.NCat COLLATE DATABASE_DEFAULT " & _
    '                    " Where  Round(IsNull(L.TillTransactionDateStock,0),isnull(U.Decimalplaces,0)) - IsNull(H.Qty,0) < 0 "
    '            DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
    ...
    Public Shared Function FIsNegativeStock(ByVal mSelectionQry As String, ByVal SearchCode As String, ByVal Godown As String, ByVal V_Date As String) As Boolean
        ...
        If AgL.VNull(DtGodownSettings.Rows(0)("RestrictNegetiveStock")) <> 0 Or AgL.VNull(DtGodownSettings.Rows(0)("AlertOnNegetiveStock")) <> 0 Then
            ...
            mItemInTransactionQry = " Select L.Item, " & _
                            ...
                            " From [#" & bTempTable & "] As L  " & _
                            " LEFT JOIN (Select Code, IsRequired_LotNo From ItemSiteDetail Where Site_Code = '" & AgL.PubSiteCode & "') Isd ON L.Item = Isd.Code  COLLATE DATABASE_DEFAULT " & _
                            " Group By L.Item, Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then L.LotNo ELse Null End "


            mTillTransactionDateStockQry = " Select S.Item, " & _
                                " Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo ELse Null End As LotNo, " & _
                                " IsNull(Sum(S.Qty_Rec),0) - IsNull(Sum(S.Qty_Iss),0) As TillTransactionDateStock " & _
                                " From (Select DocId, Godown, V_Date, Item, LotNo, Qty_Rec, Qty_Iss From Stock Where Item In (Select Item COLLATE DATABASE_DEFAULT From [#" & bTempTable & "])) As S " & _
                                " LEFT JOIN (Select Code, IsRequired_LotNo From ItemSiteDetail Where Site_Code = '" & AgL.PubSiteCode & "') As Isd ON S.Item = Isd.Code  COLLATE DATABASE_DEFAULT " & _
                                " Where S.DocId <> '" & SearchCode & "' " & _
                                " And S.Godown = '" & Godown & "' " & _
                                " And S.V_Date <= '" & V_Date & "' " & _
                                " Group By S.Item, Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo ELse Null End "

            mCurrentStockQry = " Select S.Item, " & _
                    " Case When IsNull(Isd.IsRequired_LotNo,0) <> 0 Then S.LotNo ELse Null End As LotNo, " & _
                    " Round(IsNull(Sum(S.Qty_Rec),0),isnull(Max(U.Decimalplaces),0)) - Round(IsNull(Sum(S.Qty_Iss),0),isnull(Max(U.Decimalplaces),0)) As CurrentStock " & _
                    " From (Select DocId, Godown, V_Date, Unit, Item, LotNo, Qty_Rec, Qty_Iss From Stock Where Item In (Select Item COLLATE DATABASE_DEFAULT From [#" & bTempTable & "])) As S " & _
                    " LEFT JOIN (Select Code, IsRequired_LotNo From ItemSiteDetail Where Site_Code = '" & AgL.PubSiteCode & "') As Isd ON S.Item = Isd.Code  COLLATE DATABASE_DEFAULT " & _
                    " LEFT JOIN Unit U ON U.Code = S.Unit COLLATE DATABASE_DEFAULT " & _
                    " Where S.DocId <> '" & SearchCode & "' " & _
                    " And S.Godown = '" & Godown & "' " & _
            ...
            mQry = " Select H.Item, H.LotNo, I.Description As ItemDesc, IsNull(H.Qty,0) As Qty, Round(IsNull(L.CurrentStock,0),isnull(U.Decimalplaces,0)) As Stock " & _
                    " From (" & mItemInTransactionQry & ") As H " & _
                    " LEFT JOIN (" & mCurrentStockQry & ") As L On H.Item = L.Item COLLATE DATABASE_DEFAULT  " & _
                    "                  And IsNull(H.LotNo,'') = IsNull(L.LotNo,'') COLLATE DATABASE_DEFAULT  " & _
                    " LEFT JOIN Item I On H.Item = I.Code COLLATE DATABASE_DEFAULT " & _
            ...
            If DtTemp.Rows.Count > 0 Then
                ...
                Next
            Else
                mQry = " Select H.Item, L.LotNo, I.Description As ItemDesc, IsNull(H.Qty,0) As Qty, Round(IsNull(L.TillTransactionDateStock,0),isnull(U.Decimalplaces,0)) As Stock " & _
                        " From (" & mItemInTransactionQry & ") As H " & _
                        " LEFT JOIN (" & mTillTransactionDateStockQry & ") As L On H.Item = L.Item COLLATE DATABASE_DEFAULT " & _
                        "                  And IsNull(H.LotNo,'') = IsNull(L.LotNo,'') COLLATE DATABASE_DEFAULT  " & _
                        " LEFT JOIN Item I On H.Item = I.Code COLLATE DATABASE_DEFAULT " & _
    ...
    Public Shared Sub ProcExecuteQry_FromTextFile(ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand)
    ...
    Public Shared Function FGetJobRate(ByVal mProcess As String, ByVal mParty As String, ByVal mItem As String) As Double
        ...
        mQry = " Select Rate From RateListDetail L  " & _
                " Where IsNull(SubCode,'') = (SELECT CASE WHEN  Count(*) > 0 THEN IsNull(Max(PartyRateGroup),'') ELSE '' END From SubGroup Where SubCode = '" & mParty & "') " & _
                " And IsNull(Item,'') = (SELECT CASE WHEN Count(*) > 0 THEN IsNull(Max(ItemRateGroup),'') ELSE '' END  From ItemProcessDetail Where Code = '" & mItem & "' And Process = '" & mProcess & "') " & _
    ...
    Public Shared Sub FSaveAlerts(ByVal SearchCode As String, ByVal SearchField As String, ByVal SearchTable As String, _
                                  ByVal AlertType As Integer, ByVal AlertMsg As String)
        Dim mQry$ = ""
        Dim mConn As New SqlClient.SqlConnection
        Dim mCmd As New SqlClient.SqlCommand

        Try
            mConn = AgL.GCn
            mCmd.Connection = mConn

            mQry = " INSERT INTO Alerts(SearchCode, SearchField, SearchTable, AlertType, AlertMsg, AlertDateTime) " & _
                    " VALUES (" & AgL.Chk_Text(SearchCode) & ", " & _
                    " " & AgL.Chk_Text(SearchField) & ", " & _
                    " " & AgL.Chk_Text(SearchTable) & ", " & _
                    " " & AlertType & ", " & _
                    " " & AgL.Chk_Text(AlertMsg) & ", " & _
                    " GetDate()) "
    ...
    '    mQry = " Select L.Item, L.FromProcess, Sum(L.Qty) As IssueQty, IsNull(Max(VStock.Stock),0) As CurrentStock " & _
    '            " From JobOrderDetail L With (NoLock) " & _
    '            " LEFT JOIN ( " & _
    '            "       Select S.Item, S.Process, IsNull(Sum(S.Qty_Rec),0) - IsNull(Sum(S.Qty_Iss),0) As Stock " & _
    '            "       From Stock S With (NoLock) " & _
    '            "       Where S.Item In (Select Item From JobOrderDetail With (NoLock) Where DocId = '" & mSearchCode & "') " & _
    '            "       And S.Site_Code = '" & AgL.PubSiteCode & "' " & _
    '            "       And S.Godown = '" & Godown & "' " & _
    '            "       And S.V_Date <= '" & V_Date & "' " & _
    '            "       And S.DocId <> '" & mSearchCode & "'" & _
    '            "       Group By S.Item, S.Process " & _
    '            " ) As VStock ON L.Item = VStock.Item And IsNull(L.FromProcess,'') = IsNull(VStock.Process,'') " & _
    ...
    '                    " Where IsNull(L.Item,'') = '" & AgL.XNull(DtTemp.Rows(I)("Item")) & "' " & _
    '                    " And IsNull(L.FromProcess,'') = '" & AgL.XNull(DtTemp.Rows(I)("FromProcess")) & "' " & _
    ...
    Public Shared Sub FPostInStockWithProcess(ByVal StockView As String, _
                                              ByVal mSearchCode As String, ByVal Godown As String, _
                                              ByVal V_Date As String, _
                                              ByVal Conn As SqlClient.SqlConnection, _
                                              ByVal Cmd As SqlClient.SqlCommand)
        Dim mQry$ = ""
        Dim DtTemp As DataTable = Nothing
        ...
        mInstertionQry = "INSERT INTO Stock(DocID, Sr, V_Type, V_Prefix, V_Date, V_No, RecID, Div_Code, Site_Code, " & _
        ...
        mAvailableInPrevProcessQry = " SELECT L.DocId, row_number() Over (Order By L.Item) As Sr, Max(L.V_Type) As V_Type, Max(L.V_Prefix) As V_Prefix, " & _
                    " Max(L.V_Date) As V_Date, Max(L.V_No) As V_No, Max(L.RecId) As RecId, " & _
                    " Max(L.Div_Code) As Div_Code, Max(L.Site_Code) As Site_Code, " & _
                    " Max(L.SubCode) As SubCode, L.Item, Max(L.Godown) As Godown, " & _
                    " Max(L.Unit) As Unit, " & _
                    " Max(L.MeasurePerPcs) As MeasurePerPcs, " & _
                    " Max(L.MeasureUnit) As MeasureUnit, " & _
                    " L.Process, " & _
                    " Case When IsNull(Sum(L.Qty),0) > IsNull(Max(VStock.CurrentStock),0) " & _
                    "      Then IsNull(Max(VStock.CurrentStock),0) " & _
                    "      Else IsNull(Sum(L.Qty),0) End As Qty, " & _
                    " 0 As TotalMeasure " & _
                    " FROM (" & StockView & ") As L " & _
                    " LEFT JOIN ( " & _
                    "       Select S.Item, S.Process, IsNull(Sum(S.Qty_Rec),0) - IsNull(Sum(S.Qty_Iss),0) As CurrentStock " & _
                    "       From Stock S With (NoLock) " & _
                    "       Where S.Item In (Select Item From (" & StockView & ") As L Where DocId = '" & mSearchCode & "') " & _
                    "       And S.Site_Code = '" & AgL.PubSiteCode & "' " & _
                    "       And S.V_Date <= '" & V_Date & "' " & _
                    "       Group By S.Item, S.Process " & _
                    " ) As VStock ON L.Item = VStock.Item And IsNull(L.Process,'') = IsNull(VStock.Process,'') " & _
                    " WHERE L.DocId = '" & mSearchCode & "' " & _
                    " GROUP BY L.DocId, L.Item, L.Process " & _
                    " HAVING IsNull(Max(VStock.CurrentStock),0) > 0 "
        ...
        mPendingToSaveQry = " SELECT L.DocId, Max(L.V_Type) As V_Type, Max(L.V_Prefix) As V_Prefix, " & _
                    " Max(L.V_Date) As V_Date, Max(L.V_No) As V_No, Max(L.RecId) As RecId, " & _
                    " Max(L.Div_Code) As Div_Code, Max(L.Site_Code) As Site_Code, " & _
                    " Max(L.SubCode) As SubCode, L.Item, Max(L.Godown) As Godown, " & _
                    " IsNull(Sum(L.Qty),0) - IsNull(Max(VStock.StockSavedQty),0) AS PendingToSaveQty, " & _
                    " Max(L.Unit) As Unit, " & _
                    " Max(L.MeasurePerPcs) As MeasurePerPcs, " & _
                    " (IsNull(Sum(L.Qty),0) - IsNull(Max(VStock.StockSavedQty),0)) * Max(L.MeasurePerPcs) AS Measure_Iss, " & _
                    " Max(L.MeasureUnit) As MeasureUnit " & _
                    " FROM (" & StockView & ") As L " & _
                    " LEFT JOIN ( " & _
                    " 	    SELECT L.Item, IsNull(Sum(L.Qty_Iss),0) AS StockSavedQty " & _
                    " 	    FROM Stock L With (NoLock)  " & _
                    " 	    WHERE L.DocID = '" & mSearchCode & "' " & _
                    " 	    GROUP BY L.Item " & _
                    " ) AS VStock ON L.Item = VStock.Item " & _
                    " WHERE L.DocId = '" & mSearchCode & "' " & _
                    " GROUP BY L.DocId, L.Item " & _
                    " HAVING IsNull(Sum(L.Qty),0) - IsNull(Max(VStock.StockSavedQty),0) > 0 "

        mProcessWiseStockQry = " SELECT L.Item, L.Process, IsNull(Max(L.Sr),0) As ProcessSr, IsNull(Sum(L.Qty_Rec),0) - IsNull(Sum(L.Qty_Iss),0) AS Qty " & _
                " FROM Stock L With (NoLock)  " & _
                " LEFT JOIN Process P ON L.Process = P.NCat " & _
                " WHERE L.Item IN (SELECT Item FROM (" & StockView & ") As L WHERE DocId = '" & mSearchCode & "') " & _
                " And L.V_Date <= '" & V_Date & "' " & _
                " And L.Site_Code = '" & AgL.PubSiteCode & "'" & _
                " GROUP BY L.Item, L.Process " & _
                " Having IsNull(Sum(L.Qty_Rec),0) - IsNull(Sum(L.Qty_Iss),0) > 0 "

        mSr = AgL.VNull(AgL.Dman_Execute(" Select Max(Sr) From Stock With (NoLock) Where DocId = '" & mSearchCode & "'", AgL.GcnRead).ExecuteScalar)

        mAdjustedStockQry = " Select p.DocId, " & mSr & " + row_number() Over (Order By p.Item) As Sr, p.V_Type, " & _
                " p.V_Prefix, p.V_Date, p.V_No, p.RecID, p.Div_Code, p.Site_Code,  " & _
                " p.SubCode, p.Item, p.Godown, P.Unit, P.MeasurePerPcs, " & _
                " P.MeasureUnit, s.Process, " & _
                " CASE WHEN IsNull(p.sum_order,0) - IsNull(s.sum_stock,0) + IsNull(s.Qty,0) < IsNull(s.sum_stock,0) -IsNull(p.sum_order,0) + IsNull(p.PendingToSaveQty,0) " & _
                "      THEN CASE WHEN IsNull(p.PendingToSaveQty,0) <  IsNull(p.sum_order,0)  - IsNull(s.sum_stock,0)  + IsNull(s.Qty,0)  " & _
                "                THEN IsNull(p.PendingToSaveQty,0)  " & _
                "                ELSE CASE WHEN IsNull(s.Qty,0) < IsNull(p.sum_order,0)  - IsNull(s.sum_stock,0)  + IsNull(s.Qty,0) " & _
                "                          THEN IsNull(s.Qty,0) ELSE IsNull(p.sum_order,0)  - IsNull(s.sum_stock,0)  + IsNull(s.Qty,0) END END " & _
                " ELSE " & _
                " CASE WHEN IsNull(p.PendingToSaveQty,0) <  IsNull(s.sum_stock,0)  - IsNull(p.sum_order,0)  + IsNull(p.PendingToSaveQty,0) " & _
                "      THEN IsNull(p.PendingToSaveQty,0) ELSE  " & _
                "           CASE WHEN IsNull(s.Qty,0) < IsNull(s.sum_stock,0)  - IsNull(p.sum_order,0)  + IsNull(p.PendingToSaveQty,0) " & _
                "                THEN IsNull(s.Qty,0) ELSE IsNull(s.sum_stock,0)  - IsNull(p.sum_order,0)  + IsNull(p.PendingToSaveQty,0) END END END AS Qty, " & _
                " 0 As MeasureIss " & _
                " FROM ( " & _
                "       SELECT p.*,  SUM(PendingToSaveQty) OVER(PARTITION BY Item ORDER BY Item) sum_order  " & _
                "       FROM  (" & mPendingToSaveQry & ") p  " & _
                "       ) As p  " & _
                " LEFT JOIN ( " & _
                "       SELECT s.*, SUM(Qty) OVER( PARTITION BY Item ORDER BY Process) sum_stock  " & _
                "       FROM  (" & mProcessWiseStockQry & ") s  " & _
                "       WHERE s.Qty > 0)  s  " & _
                " ON  s.Item   = p.Item  " & _
                " AND IsNull(p.sum_order,0)  > IsNull(s.sum_stock,0)  - IsNull(s.Qty,0)   " & _
                " AND IsNull(s.sum_stock,0)  > IsNull(p.sum_order,0)  - IsNull(p.PendingToSaveQty,0)   " & _
        ...
        mPendingToSaveQry = " SELECT L.DocId, " & mSr & " + row_number() Over (Order By L.Item) As Sr, Max(L.V_Type) As V_Type, Max(L.V_Prefix) As V_Prefix, " & _
               " Max(L.V_Date) As V_Date, Max(L.V_No) As V_No, Max(L.RecId) As RecId, " & _
               " Max(L.Div_Code) As Div_Code, Max(L.Site_Code) As Site_Code, " & _
               " Max(L.SubCode) As SubCode, L.Item, Max(L.Godown) As Godown, " & _
               " Max(L.Unit) As Unit, " & _
        ...
        mQry = " UPDATE Stock " & _
                " Set Measure_Iss = IsNull(Qty_Iss,0) * IsNull(MeasurePerPcs,0), " & _
                " Measure_Rec = IsNull(Qty_Rec,0) * IsNull(MeasurePerPcs,0) " & _
                " Where DocId = '" & mSearchCode & "'"
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)

        'if any qty is saved with 0 then it will be deleted
        mQry = " Delete From Stock Where DocId = '" & mSearchCode & "' And IsNull(Qty_Rec,0) + IsNull(Qty_Iss,0) = 0 "
        AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
    End Sub
