...
#End Region
        ...
        With Dgl1
            For I = 0 To .RowCount - 1
                If .Item(Col1FormNo, I).Value <> "" Then
                    mQry = " UPDATE Form_ReceiveDetail_Log " & _
                                " SET IssueDate = " & AgL.Chk_Text(.Item(Col1IssueDate, I).Value) & ", " & _
                                " IssueTo = " & AgL.Chk_Text(.AgSelectedValue(Col1IssueTo, I)) & ", " & _
                                " IssueDocId = " & AgL.Chk_Text(mInternalCode) & ", " & _
                                " IsInStock = " & Val(.Item(Col1IsInStock, I).Value) & ", " & _
                                " Status = 'Issued' " & _
    ...
    Private Sub FrmProductionOrder_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = "SELECT Rd.Form AS Code, Max(M.Description) AS Form, " & _
                " Sum(Convert(INT, IsNull(Rd.IsInStock,0))) AS IsInStock, " & _
                " Sum(Convert(INT, IsNull(R.IsFromDepartment,0))) AS IsFromDepartment " & _
                " FROM Form_ReceiveDetail Rd  " & _
                " LEFT JOIN Form_Receive R ON Rd.DocId = R.DocID " & _
                " LEFT JOIN Form_Master M ON Rd.Form = M.Code " & _
                " GROUP BY Rd.Form "
        ...
        mQry = " SELECT DISTINCT  F.FormPrefix AS Code, F.FormPrefix AS FormPrefix, F.Form " & _
                " FROM Form_Receive F "
    ...
    Private Sub ProcFill()
        ...
        Try
            mQry = " SELECT Rd.Code, Rd.Form, Rd.FormNo " & _
                    " FROM Form_ReceiveDetail Rd " & _
                    " Where Rd.FormPrefix = '" & TxtFormPrefix.Text & "' " & _
                    " And FormNoSerial BETWEEN " & Val(TxtFormNoFrom.Text) & " And " & Val(TxtFormNoUpto.Text) & " " & _
                    " And IsNull(IsInStock,1) <> 0 And IsNull(Status,'Stock') = 'Stock'    "
    ...
    Private Sub FrmFormIssue_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Approve_InTrans
        Dim I As Integer
        With Dgl1
            For I = 0 To .Rows.Count - 1
                mQry = " UPDATE Form_ReceiveDetail " & _
                        " SET Form_ReceiveDetail.IssueDate = Form_ReceiveDetail_Log.IssueDate ,  " & _
                        " Form_ReceiveDetail.IssueTo = Form_ReceiveDetail_Log.IssueTo , " & _
                        " Form_ReceiveDetail.IssueDocId = Form_ReceiveDetail_Log.IssueDocId, " & _
                        " Form_ReceiveDetail.IsInStock = Form_ReceiveDetail_Log.IsInStock, " & _
                        " Form_ReceiveDetail.Status = Form_ReceiveDetail_Log.Status " & _
                        " FROM Form_ReceiveDetail_Log " & _
                        " WHERE Form_ReceiveDetail.Code = Form_ReceiveDetail_Log.Code " & _
