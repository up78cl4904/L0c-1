...
#End Region
            ...
            Select Case sender.name
                ...
                Case TxtJobWorker.Name
                    TxtJobWorker.AgRowFilter = " IsDeleted = 0 " & _
                            ...
                            " AND Div_Code = '" & TxtDivision.AgSelectedValue & "' " & _
                            " And Site_Code = '" & TxtSite_Code.AgSelectedValue & "'" & _
                            " And IsNull(Process,'') = '" & mJobProcess & "' "
    ...
    Private Sub TempJobOrder_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        ...
        mQry = " SELECT J.SubCode AS Code, Sg.Name AS JobWorker, H.Process, " & _
                ...
                " FROM JobWorker J  With (NoLock) " & _
                " LEFT JOIN JobWorkerProcess H With (NoLock)  On J.SubCode = H.SubCode  " & _
                " LEFT JOIN SubGroup Sg With (NoLock)  ON J.SubCode = Sg.SubCode "
        ...
        mQry = "SELECT Pp.DocId AS Code,  " & _
                ...
                " FROM MaterialPlan  Pp  With (NoLock) " & _
                " LEFT JOIN Voucher_Type Vt With (NoLock)  On Pp.V_Type = Vt.V_Type "
        ...
        mQry = " SELECT H.NCat AS Code, L.NCatDescription AS Process " & _
                ...
                " LEFT JOIN VoucherCat L  With (NoLock) ON H.NCat = L.NCat "
        ...
        mQry = " SELECT L.SubCode AS Code, L.DispName AS OrderBy, " & _
                ...
                " FROM Employee H  With (NoLock) " & _
                " LEFT JOIN SubGroup L  With (NoLock) ON H.SubCode = L.SubCode "
        ...
        mQry = " SELECT I.Code, I.Description AS Item, Po.V_Date AS [Prod.Order Date], PO.ManualRefNo as [Prod.Order No.], Po.DueDate As [Prod Order Due Date], " & _
                ...
                " IsNull(I.IsDeleted ,0) AS IsDeleted, H.ProdOrder,  H.Div_Code, H.Site_Code, " & _
                ...
                " FROM (Select * from MaterialPlan  With (NoLock)  Where IsNull(IsDeleted,0)=0) H  " & _
                " LEFT JOIN MaterialPlanDetail L  With (NoLock)  ON L.DocId = H.DocID " & _
                " LEFT JOIN (Select * from Item  With (NoLock)  Where IsNull(IsDeleted,0)=0) I ON L.Item = I.Code " & _
                " LEFT JOIN (Select * from ProdOrder  With (NoLock)  Where IsNull(IsDeleted,0)=0) Po ON H.ProdOrder = Po.DocID " & _
        ...
        mQry = " SELECT I.Code, I.Description As Item, L.ProdOrder, IsNull(L.Qty,0) - IsNull(L.CancelQty,0) As PendingQty, " & _
                ...
                " IsNull(H.IsDeleted,0) As IsDeleted, H.Div_Code, H.Site_Code, " & _
                " IsNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status " & _
                " FROM JobOrderDetail L  With (NoLock) " & _
        ...
        mQry = "SELECT J.DocId, J.ManualRefNo As JobOrderNo, " & _
                " IsNull(J.IsDeleted,0) As IsDeleted, J.Div_Code,Vt.NCat,  J.Site_Code,  " & _
        ...
        mQry = " SELECT H.Code AS Code, H.Description, H.Process, IsNull(H.IsDeleted,0) as IsDeleted, IsNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') As Status FROM JobWorkerRateGroup H  With (NoLock)  "
