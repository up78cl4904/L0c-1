...
#End Region
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1 " & AgL.RetDivisionCondition(AgL, "J.Div_Code") & " AND Isnull(J.IsDeleted,0) = 0 And J.MasterType = '" & ClsMain.RateMasterType.General & "'"
        AgL.PubFindQry = "SELECT H.UID, Vt.Description As Process, I.Description As Item, " & _
                        " Ig.Description As ItemGroup, Ic.Description As ItemCategory, H.Rate, H.RateOutSide, G.Description as RateGroup " & _
                        " FROM JobWorkerRateDetail_Log H " & _
                        " LEFT JOIN JobWorkerRate_Log J ON H.UID = J.UID " & _
                        ...
                        " LEFT JOIN JobWorkerRateGroup G On G.Code  = H.JobWorkerRateGroup " & _
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_FindMain() Handles Me.BaseEvent_FindMain
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1  And L.MasterType = '" & ClsMain.RateMasterType.General & "'" '& AgL.RetDivisionCondition(AgL, "H.Div_Code") & " AND Isnull(H.IsDeleted,0) = 0  
        AgL.PubFindQry = "SELECT H.Code, Vt.Description As Process,  I.Description As Item, " & _
                        " Ig.Description As ItemGroup, Ic.Description As ItemCategory, H.Rate, H.RateOutSide, G.Description as RateGroup " & _
                        " FROM JobWorkerRateDetail H  " & _
                        " LEFT JOIN JobWorkerRate L ON H.Code = L.Code " & _
                        " LEFT JOIN Voucher_Type Vt ON Vt.V_Type = H.Process " & _
                        " LEFT JOIN Item I On H.Item  = I.Code " & _
                        " LEFT JOIN ItemGroup Ig On H.ItemGroup  = Ig.Code " & _
                        " LEFT JOIN ItemCategory Ic On H.ItemCategory = I.Code " & _
                        " LEFT JOIN Subgroup J On H.JobWorker  = J.SubCode " & _
                        " LEFT JOIN JobWorkerRateGroup G On G.Code  = H.JobWorkerRateGroup " & _
                        "  " & mConStr
        AgL.PubFindQryOrdBy = "[Process]"
    End Sub
    ...
    Private Sub FrmJobWorkerRateMaster_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        mQry = " Update JobWorkerRate_LOG " & _
                ...
                " MasterType = " & AgL.Chk_Text(ClsMain.RateMasterType.General) & ", " & _
    ...
    Private Sub FrmJobWorkerRateMaster_BaseFunction_FIniMast(ByVal BytDel As Byte, ByVal BytRefresh As Byte) Handles Me.BaseFunction_FIniMast
        Dim mConStr$ = ""
        mConStr = "WHERE 1=1 " & AgL.RetDivisionCondition(AgL, "Div_Code") & " AND Isnull(IsDeleted,0) = 0   And MasterType = '" & ClsMain.RateMasterType.General & "'"
    ...
    Private Sub FrmJobWorkerRateMaster_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        ...
        mQry = " SELECT H.Code, H.Description AS Item, H.ItemGroup, IsNull(H.IsDeleted,0) AS IsDeleted, " & _
        ...
        Select Case sender.name
            Case TxtItemGroup.Name
                If TxtItemCategory.AgSelectedValue <> "" Then
                    TxtItemGroup.AgRowFilter = " IsNull(ItemCategory,'') = '" & TxtItemCategory.AgSelectedValue & "' "
                End If

            Case TxtItem.Name
                If TxtItemGroup.AgSelectedValue <> "" Then
                    TxtItem.AgRowFilter = " IsNull(ItemGroup,'') = '" & TxtItemGroup.AgSelectedValue & "' "
                End If
