...
#End Region
    ...
    Public Function FGetManualRefNo(ByVal FieldName As String, ByVal TableName As String, ByVal V_Type As String, ByVal V_Date As String, ByVal Div_Code As String, ByVal Site_Code As String, ByVal Godown_Code As String, Optional ByVal RefType As ClsMain.ManualRefType = ClsMain.ManualRefType.Max) As String
        ...
        Select Case RefType
            Case Else
                mQry = "Select IsNull(Max(Convert(Numeric," & FieldName & ")),0)+1 From " & TableName & " With (NoLock) WHERE isnumeric(" & FieldName & ")>0 And V_Type = '" & V_Type & "' And Div_Code = '" & Div_Code & "' and Site_Code = '" & Site_Code & "' and ToGodown = '" & Godown_Code & "' And V_Date Between '" & mStartDate & "' and  '" & mEndDate & "'  "
    ...
    Private Function FCheckDuplicateRefNo() As Boolean
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = " SELECT COUNT(*) FROM StockHead WHERE ManualRefNo = '" & TxtManualRefNo.Text & "'   " & _
                    " AND V_Type ='" & TxtV_Type.AgSelectedValue & "'  And Div_Code = '" & TxtDivision.AgSelectedValue & "' And Site_Code = '" & TxtSite_Code.AgSelectedValue & "' AND ToGodown = '" & TxtToGodown.AgSelectedValue & "' And IsNull(IsDeleted,0) = 0  "
        ...
        Else
            mQry = " SELECT COUNT(*) FROM StockHead WHERE ManualRefNo = '" & TxtManualRefNo.Text & "'  " & _
                    " AND V_Type ='" & TxtV_Type.AgSelectedValue & "'  And Div_Code = '" & TxtDivision.AgSelectedValue & "' And Site_Code = '" & TxtSite_Code.AgSelectedValue & "' AND ToGodown = '" & TxtToGodown.AgSelectedValue & "' And IsNull(IsDeleted,0) = 0 AND DocID <>'" & mInternalCode & "'  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then FCheckDuplicateRefNo = False : MsgBox("Reference No. Already Exists") : TxtManualRefNo.Focus()
        End If
    End Function
    ...
    Private Sub Txt_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles TxtFromGodown.Enter, TxtToGodown.Enter, TxtIssueNo.Enter
        Select Case sender.name
            Case TxtFromGodown.Name
                TxtFromGodown.AgRowFilter = " Site_Code = '" & TxtSite_Code.AgSelectedValue & "' " & _
                          " And Status = '" & AgTemplate.ClsMain.EntryStatus.Active & "' " & _
                          " And IsDeleted = 0 "


            Case TxtToGodown.Name
                TxtToGodown.AgRowFilter = " Div_Code = '" & AgL.PubDivCode & "' " & _
    ...
    Private Sub TempStockTransferReceive_BaseFunction_CreateHelpDataSet() Handles Me.BaseFunction_CreateHelpDataSet
        ...
        mQry = "SELECT S.DocID AS Code, S.ManualRefNo, Vt.Description + '/' + convert(NVARCHAR,S.V_No)  AS IssueNo, " & _
