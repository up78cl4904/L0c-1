...
#Region "Sale Report"
    Private Sub ProcSaleReport(ByVal HeaderTable As String, ByVal LineTable As String)
        Try
            ...
            mCondStr = mCondStr & " AND H.V_Date Between '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
            ...
            mQry = " SELECT " & strGrpFld & " as GrpField, " & strGrpFldDesc & " as GrpFieldDesc, " & strGrpFldHead & " as GrpFieldHead, " & _
                    " H.DocID, H.V_Date, " & _
                    " Party.Name || ',' || IfNull(City.CityName,'') As SaleToPartyName , " & _
                    " Agent.Name || ',' || IfNull(City.CityName,'') As AgentName , " & _
                    " H.ReferenceNo,  " & _
    ...
    Private Sub ProcPurchaseInvoiceReport(ByVal HeaderTable As String, ByVal LineTable As String)
        ...
        Try
            ...
            mQry = " SELECT " & strGrpFld & " as GrpField, " & strGrpFldDesc & " as GrpFieldDesc, " & strGrpFldHead & " as GrpFieldHead, " & _
                        ...
                        " I.Description AS ItemDesc, H.V_Date, H.ReferenceNo, Sg.DispName || ',' || IfNull(C.CityName,'') As VendorName, L.Remark " & _
                        " FROM " & LineTable & " L " & _
    ...
    Private Sub ProcPurchaseAdviseReport()
        Try
            ...
            mQry = " SELECT L.Item, Max(I.Description) AS ItemDesc, Max(I.ReorderLevel) AS ReorderLevel, " & _
                    " IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0) AS StockQty " & _
                    " FROM Stock L " & _
...
#Region "Purchase Indent Report"
    Private Sub ProcPurchaseIndentReport()
        Try
            ...
            mQry = " SELECT H.V_Date, I.Description As ItemDesc, IfNull(VStock.CurrentStock,0) As StockQty " & _
                    " FROM PurchIndent H  " & _
                    " LEFT JOIN PurchIndentDetail L ON H.DocID = L.DocId " & _
                    " LEFT JOIN ( " & _
                    "   Select S.Item, IfNull(Sum(S.Qty_Rec),0) - IfNull(Sum(S.Qty_Iss),0) As CurrentStock " & _
    ...
    Private Sub ProcCurrentStockReport()
        Try
            ...
            mQry = "SELECT I.Code, I.Description, H.LotNo AS LotNo, " & _
                    "IfNull(H.Qty_Rec, 0) - IfNull(SAdj.AdjQty, 0) AS [Bal.Qty], I.Unit,  " & _
                    ...
                    "      (H.Landed_Value/H.Qty_Rec)  as PurchaseRate, (H.Landed_Value/H.Qty_Rec)*(IfNull(H.Qty_Rec, 0) - IfNull(SAdj.AdjQty, 0)) as StockAmount  " & _
                    "     FROM Stock H    " & _
                    "    LEFT JOIN (  " & _
                    ...
                    "    Where IfNull(H.Qty_Rec, 0) - IfNull(SAdj.AdjQty, 0) > 0  And IfNull(H.Qty_Rec, 0)>0   " & mCondStr & _
...
#End Region
    ...
    Private Sub FWriteTaxReturn(ByVal DsTemp As DataSet)
        ...
        Dim PurchaseInOwnAcAgainstTaxInvoice$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NOT NULL "
        Dim PurchaseInOwnAcFromNonRegisteredDealer$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NULL "
        Dim PurchaseOfExcemptGoods$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  "
        Dim PurchaseOfExUp$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Central & "' "

        Dim TotalPurchase$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)) & "' And IfNull(H.Qty,0) > 0 "
        Dim TotalPurchaseReturn$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) < 0  "

        Dim NetAmtOfPurchase$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
        Dim GrandTotalOfPurchase$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "

        Dim TurnOverOfPurchaseOnePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 1 "
        Dim TurnOverOfPurchaseFivePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 5 "
        Dim TurnOverOfPurchaseTwelvePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 12.5 "

        Dim TotalSalesTaxVatOnePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 1 "
        Dim TotalSalesTaxVatFivePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 5 "
        Dim TotalSalesTaxVatTwelvePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 12.5 "

        Dim TotalAddSalesTaxPointFivePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)) & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 0.5 "
        Dim TotalAddSalesTaxOnePerOnPurchase$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 1 "

        Dim TotalSalesAndAddSalesTaxOnPurchase$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 "

        Dim TotalTurnOverOfPurchase$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) <> 0 "



        Dim SaleInOwnAcAgainstTaxInvoice$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' And H.TINNo IS NOT NULL "
        Dim SaleInOwnAcFromNonRegisteredDealer$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'   And H.TINNo IS NULL"
        Dim SaleOfExcemptGoods$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  "
        Dim SaleOfExUp$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0  And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Central & "' "

        Dim TotalSale$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 "
        Dim TotalSaleReturn$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) < 0  "

        Dim NetAmtOfSale$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
        Dim GrandTotalOfSale$ = " SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "

        Dim TurnOverOfSaleOnePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 1 "
        Dim TurnOverOfSaleFivePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 5 "
        Dim TurnOverOfSaleTwelvePer$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 12.5 "

        Dim TotalSalesTaxVatOnePerOnSale$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 1 "
        Dim TotalSalesTaxVatFivePerOnSale$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 5 "
        Dim TotalSalesTaxVatTwelvePerOnSale$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) = 12.5 "

        Dim TotalAddSalesTaxPointFivePerOnSale$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 0.5 "
        Dim TotalAddSalesTaxOnePerOnSale$ = "SELECT Str(IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Additional_Sales_Tax_Per,0) = 1 "

        Dim TotalSalesAndAddSalesTaxOnSale$ = "SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 "

        Dim TotalTurnOverOfSale$ = "SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax_Per,0) <> 0 "

        Dim TotalTax$ = " SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category In ('" & ClsMain.Voucher_Category.Sale & "','" & ClsMain.Voucher_Category.Purchase & "')  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 "

        Dim NetTax$ = " Select " &
                " Str((SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0) " &
                " - (SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0),15,2)   "
        ...
        mQry = " SELECT H.Sales_Tax_Per, Sum(H.Sales_Tax_Taxable_Amt) AS Sales_Tax_Taxable_Amt, " & _
                ...
                " AND IfNull(H.Sales_Tax_Per,0) <> 0 " & _
        ...
        mQry = " SELECT H.Additional_Sales_Tax_Per, Sum(H.Additional_Sales_Tax) AS Additional_Sales_Tax " & _
                ...
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " & _
                " WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' " & _
                " AND IfNull(H.Additional_Sales_Tax,0) <> 0 " & _
    ...
    Private Sub ProcBillWiseProfitabilty()
        Try
            ...
            mQry = " SELECT H.DocID , H.V_Type, H.V_Date, H.ReferenceNo, Sg.Name As SaleToPartyName, H.SaleToParty , " & _
                    ...
                    " CASE WHEN IfNull(PCD.Landed_Value/PCD.Qty * L.Qty,0) > 0 THEN ( L.Landed_Value - PCD.Landed_Value/PCD.Qty * L.Qty) * 100 / ( PCD.Landed_Value/PCD.Qty * L.Qty) ELSE 0 END AS ProfitPer " & _
    ...
    Private Sub ProcDebtorsOutstandingOverDue()
        ...
        Try
            ...
            mQry = "SELECT VRep.SubCode, Max(VRep.PartyName) AS PartyName, Max(VRep.TotalBal) AS TotalBal, Sum(VRep.NetBalAmount) AS BalAboveDays, '" & ReportFrm.FGetText(1) & "' AS OnOfDays " & _
                    ...
                    " FROM " & _
                    " ( " & _
                    " SELECT SG.SubCode AS SubCode, max(SG.Name) AS PartyName, IfNull(sum(H.AmtDr),0)- IfNull(sum(H.AmtCR),0) AS TotalBal " & _
                    " FROM Ledger H   " & _
                    " LEFT JOIN SubGroup SG  ON SG.SubCode = H.SubCode " & mCondStr & _
                    " GROUP BY SG.SubCode  " & _
                    " Having IfNull(sum(H.AmtDr),0)- IfNull(sum(H.AmtCR),0)  > 0 " & _
                    " ) As P " & _
                    " LEFT JOIN " & _
                    " ( " & _
                    ...
                    " FROM Ledger H  " & _
                    " LEFT JOIN SubGroup SG  ON SG.SubCode = H.SubCode " & mCondStr & _
                    " AND IfNull(H.AmtDr,0) <> 0 " & _
                    " )  s  " & _
                    " ) SD ON SD.SubCode = p.SubCode  AND IfNull(P.TotalBal,0)  > IfNull(SD.Sum_Dr,0)  - IfNull(SD.AmtDr,0) " & _
                    " ) VMain " & _
                    " ) VRep " & _
    ...
    Private Sub ProcPartyOutstandingReport()
        ...
        Try
            ...
            mQry = "Select DocId, V_SNo, Max(RecId) As RecId, Max(V_Type) As V_Type, " & _
                            ...
                            " Max(DateAdd(Day,IfNull(Sg.CreditDays,0),Tmp.V_Date)) As DueDate, " & _
                            ...
                            "       IfNull(Lg.AmtDr,0) As BillAmt,0 As Adjusted " & _
                            "       From Ledger LG " & _
                            "       LEFT JOIN SubGroup Sg On Lg.SubCode = Sg.SubCode " & mCondStr1 & _
                            "       And IfNull(Lg.AmtDr,0) > 0 " & _
                            " Union All " & _
                            ...
                            " Having (IfNull(Max(BillAmt),0)-IfNull(Sum(Adjusted),0))>0" & _
    ...
    Private Sub ProcVatAnexureReports()
        ...
        Try
            ...
            If AgL.StrCmp(ReportFrm.FGetText(2), "Annexure A") Then
                ...
                mCondStr = mCondStr & " AND H.V_Date Between '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
                mCondStr = mCondStr & " AND IfNull(H.Qty,0) > 0  "
            ...
            mQry = " SELECT 1 As Serial, 1 as GroupSr, '" & GroupHeaderTitle1 & "' As Title, H.DocId, Max(H.Sr) Sr, Max(H.V_Type) V_Type, Max(H.V_Date) V_Date, Max(H.SubCode) SubCode, Max(H.TINNo) TINNo, " & _
                    ...
                    " LEFT JOIN SubGroup Sg  ON H.SubCode = Sg.SubCode " & _
                    " LEFT JOIN Item I  ON H.Item = I.Code  " & _
                    " LEFT JOIN VatCommodityCode V  ON I.VatCommodityCode = V.Code " & _
...
#End Region
    ...
    Private Sub FWriteMainForm(ByVal xlWorkBook As Excel.Workbook)
        Dim TotalSalesAndAddSalesTaxOnPurchaseForLocal As Double = AgL.VNull(AgL.Dman_Execute("SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ", AgL.GCn).ExecuteScalar)

        mQry = "SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' "

        Dim TotalSalesAndAddSalesTaxOnSale$ = AgL.VNull(AgL.Dman_Execute("SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "'  ", AgL.GCn).ExecuteScalar)

        Dim PurchaseInOwnAcAgainstTaxInvoice$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Sales_Tax,0) <> 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NOT NULL ", AgL.GCn).ExecuteScalar)
        Dim PurchaseInOwnAcFromNonRegisteredDealer$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Sales_Tax,0) <> 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NULL ", AgL.GCn).ExecuteScalar)
        Dim PurchaseOfExcemptGoods$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  ", AgL.GCn).ExecuteScalar)

        Dim TotalSalesAndAddSalesTaxOnPurchaseForAll$ = AgL.VNull(AgL.Dman_Execute("SELECT Str(IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ", AgL.GCn).ExecuteScalar)
        Dim SaleInOwnAcAgainstTaxInvoice$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)) & "' And IfNull(H.Sales_Tax,0) <> 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' And H.TINNo IS NOT NULL And H.SalesTaxGroupParty = 'Local' ", AgL.GCn).ExecuteScalar)


        Dim NetTax$ = AgL.VNull(AgL.Dman_Execute(" Select " &
                " Str((SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "'  And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ) " &
                " - (SELECT IfNull(Sum(H.Sales_Tax),0) + IfNull(Sum(H.Additional_Sales_Tax),0) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "'  AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "'  And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "' ),15,2)   ", AgL.GCn).ExecuteScalar)
    ...
    Private Sub FWriteVatNonVat(ByVal xlWorkBook As Excel.Workbook)
        Dim VPurchaseInOwnAcAgainstTaxInvoice$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NOT NULL ", AgL.GCn).ExecuteScalar)
        Dim VPurchaseInOwnAcFromNonRegisteredDealer$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "') = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  And H.TINNo IS NULL ", AgL.GCn).ExecuteScalar)
        Dim VPurchaseOfExcemptGoods$ = AgL.VNull(AgL.Dman_Execute(" SELECT Str(IfNull(Sum(H.Sales_Tax_Taxable_Amt),0),15,2) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)) & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(H.Sales_Tax,0) = 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  ", AgL.GCn).ExecuteScalar)
        Dim VPurchaseOfExUp$ = AgL.VNull(AgL.Dman_Execute(" SELECT Sum(H.Sales_Tax_Taxable_Amt) FROM View_Vat H LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type LEFT JOIN SubGroup Sg ON H.SubCode = Sg.SubCode LEFT JOIN PostingGroupSalesTaxParty S On H.SalesTaxGroupParty = S.Description WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' AND H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' And IfNull(H.Qty,0) > 0 And IfNull(S.Nature,'" & ClsMain.SalesTaxGroupPartyNature.Local & "')  = '" & ClsMain.SalesTaxGroupPartyNature.Central & "' ", AgL.GCn).ExecuteScalar)
    ...
    Private Sub FWriteTaxDetailSale(ByVal xlWorkBook As Excel.Workbook)
        ...
        mSaleQry = " SELECT H.Sales_Tax_Per As Sales_Tax_Per, IfNull(VC.ManualCode,'') AS VatCommodityCode, " &
                " Sum(H.Sales_Tax_Taxable_Amt) AS Sales_Tax_Taxable_Amt, " &
                " Sum(H.Sales_Tax) AS Sales_Tax, 'V' As VatNonVat " &
                " FROM View_Vat H  " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                " LEFT JOIN Item I ON I.Code = H.Item " &
                " LEFT JOIN VatCommodityCode VC ON VC.Code = I.VatCommodityCode " &
                " WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' " &
                " And IfNull(H.Sales_Tax_Per,0) <> 0 " &
                " And H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' " &
                " GROUP BY H.Sales_Tax_Per , VC.ManualCode "

        mSaleQry += "UNION ALL "

        mSaleQry += " SELECT H.Additional_Sales_Tax_Per As Sales_Tax_Per, IfNull(VC.ManualCode,'') AS VatCommodityCode, " &
                " Sum(H.Sales_Tax_Taxable_Amt) AS Sales_Tax_Taxable_Amt, " &
                " Sum(H.Additional_Sales_Tax) AS Sales_Tax, 'AT' As VatNonVat  " &
                " FROM View_Vat H  " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                " LEFT JOIN Item I ON I.Code = H.Item " &
                " LEFT JOIN VatCommodityCode VC ON VC.Code = I.VatCommodityCode " &
                " WHERE Vt.Category = '" & ClsMain.Voucher_Category.Sale & "' " &
                " And IfNull(H.Additional_Sales_Tax,0) <> 0 " &
                " And H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' " &
    ...
    Private Sub FWriteTaxDetailPurchase(ByVal xlWorkBook As Excel.Workbook)
        ...
        mPurchaseQry = " SELECT H.Sales_Tax_Per As Sales_Tax_Per, IfNull(VC.ManualCode,'') AS VatCommodityCode, " &
                " Sum(H.Sales_Tax_Taxable_Amt) AS Sales_Tax_Taxable_Amt, " &
                " Sum(H.Sales_Tax) AS Sales_Tax, 'V' As VatNonVat " &
                " FROM View_Vat H  " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
                " LEFT JOIN Item I ON I.Code = H.Item " &
                " LEFT JOIN VatCommodityCode VC ON VC.Code = I.VatCommodityCode " &
                " WHERE Vt.Category = '" & ClsMain.Voucher_Category.Purchase & "' " &
                " And IfNull(H.Sales_Tax_Per,0) <> 0 " &
                " And H.SalesTaxGroupParty = 'Local' " &
                " And H.V_Date BETWEEN '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' " &
                " GROUP BY H.Sales_Tax_Per , VC.ManualCode "

        mPurchaseQry += "UNION ALL "

        mPurchaseQry += " SELECT H.Additional_Sales_Tax_Per As Sales_Tax_Per, IfNull(VC.ManualCode,'') AS VatCommodityCode, " &
                " Sum(H.Sales_Tax_Taxable_Amt) AS Sales_Tax_Taxable_Amt, " &
                " Sum(H.Additional_Sales_Tax) AS Sales_Tax, 'AT' As VatNonVat  " &
                " FROM View_Vat H  " &
                " LEFT JOIN Voucher_Type Vt ON H.V_Type = Vt.V_Type " &
    ...
    Private Sub FWriteForm24_Annexure_A(ByVal xlWorkBook As Excel.Workbook)
        ...
        mCondStr = mCondStr & " AND IfNull(H.Qty,0) > 0  "
        mCondStr = mCondStr & " AND H.TinNo Is Not Null "
        mCondStr = mCondStr & " AND IfNull(H.Sales_Tax_Per,0) <> 0 "
        mCondStr = mCondStr & " And S.Nature = '" & ClsMain.SalesTaxGroupPartyNature.Local & "'  "
    ...
    Private Sub FWriteForm24_Annexure_C(ByVal xlWorkBook As Excel.Workbook)
        ...
        mQry = " SELECT Max(H.RecId) AS InvoiceNo,  convert(NVARCHAR,Max(H.V_Date),103) AS V_Date, Max(VC.ManualCode) AS CommodityCode,  Sum(H.Qty) AS Qty, " & _
                ...
                " Max(H.Unit) As Unit, Max(SG.DispName) AS SellerName, IfNull(Max(SG.Add1),'') || ' ' || IfNull(Max(SG.Add2),'') || '-' || IfNull(Max(C.CityName),'') AS  SellerAddress, " & _
...
#Region "Weaving Order Ratio"
    Private Sub ProcWeavingOrderRatio()
        Try
            ...
            mSaleCondStr = mSaleCondStr & " AND H.V_Date Between '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
            ...
            mWeavingCondStr = " AND H.V_Date Between '" & CDate(ReportFrm.FGetText(0)).ToString("u") & "' And '" & CDate(ReportFrm.FGetText(1)).ToString("u") & "' "
