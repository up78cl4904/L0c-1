...
#End Region
    ...
    Private Sub FCreateHelpItem()
        ...
        mQry = "SELECT H.Code, H.Description, H.Unit, H.ItemType, H.SalesTaxPostingGroup, Measure, MeasureUnit , " & _
                " IfNull(H.IsDeleted ,0) AS IsDeleted, IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') AS Status, " & _
                " H.Div_Code, H.Unit, H.Prod_Measure As MeasurePerPcs, H.MeasureUnit " & _
                " FROM Item H " & _
                " Where IfNull(H.IsDeleted ,0)  = 0 " & _
                " And IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "')='" & AgTemplate.ClsMain.EntryStatus.Active & "' " & strCond
            ...
            Select Case sender.Name
                Case TxtGodown.Name
                    If TxtGodown.AgHelpDataSet Is Nothing Then
                        If e.KeyCode <> Keys.Enter Then
                            'mQry = "SELECT Code, Description, Site_Code, IfNull(IsDeleted,0) as IsDeleted, IfNull(Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') AS Status " & _
                            '        " FROM Godown " & _
    ...
    Private Sub FrmPhysicalStockEntry_BaseEvent_ApproveDeletion_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_ApproveDeletion_InTrans
    ...
    Private Sub FPostInStock(ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand)
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, " & _
                ...
                " H.FromGodown,   " & _
                " L.Item, " & _
                " Case When IfNull(L.DifferenceQty,0) > 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End As Qty_Iss, " & _
                " Case When IfNull(L.DifferenceQty,0) < 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End As Qty_Rec, " & _
                " L.Unit, L.MeasurePerPcs MeasurePerPcs, " & _
                " Case When IfNull(L.DifferenceMeasure,0) > 0 Then IfNull(Abs(L.DifferenceMeasure),0) Else 0 End As Measure_Iss, " & _
                " Case When IfNull(L.DifferenceMeasure,0) < 0 Then IfNull(Abs(L.DifferenceMeasure),0) Else 0 End As Measure_Rec, " & _
                " L.MeasureUnit, " & _
        ...
        Try
            ...
            If DtTemp.Rows.Count > 0 Then
                For I = 0 To DtTemp.Rows.Count - 1
                    ...
                    Dgl1.Item(Col1CurrentStock, I).Value = FRetItemStock(Dgl1.Item(Col1Item, I).Tag, TxtV_Date.Text, " And IfNull(ReferenceDocId,'') = '" & Dgl1.Item(Col1ReferenceDocId, I).Tag & "' And IfNull(ReferenceDocIdSr,'') = '" & Dgl1.Item(Col1ReferenceDocIdSr, I).Value & "'")
                Next
            End If
            Calculation()
        Catch ex As Exception
    ...
    Private Sub ChkAssumeStock0IfNotFound_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ChkAssumeStock0IfNotFound.Click
        ...
        Try
            ...
            If ChkAssumeStock0IfNotFound.Checked Then
                ...
                ComputerStockQry = " SELECT L.Item, L.Unit, Round(IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0),4) AS ComputerStock, 0 As PhysicalStock " & _
                           " FROM Stock L " & _
                           " LEFT JOIN Item I ON L.Item = I.Code " & _
                ...
                If PhysicalStockItemWiseQry <> "" Then
                    mQry = " Select VMain.Item, VMain.Unit, Max(I.Description) As ItemDesc, " & _
                            " IfNull(Sum(VMain.ComputerStock),0) As ComputerStock, IfNull(Sum(VMain.PhysicalStock),0) As PhysicalStock  " & _
                            " FROM " & _
                            " (" & ComputerStockQry & " " & _
                            " UNION ALL " & _
                ...
                Dim Cmd As New SqliteCommand(mQry, AgL.GCn)
