...
#End Region
    ...
    Private Sub TempPhysicalStockEntry_BaseEvent_Approve_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Approve_InTrans
    ...
    Private Sub FCreateHelpItem()
        ...
        If RbtForSaleChallan.Checked Then
            mQry = " SELECT Max(I.Code) AS Code, Max(I.Description) AS Description, " & _
                     " IfNull(Sum(L.Qty_Rec),0) - IfNull(Sum(L.Qty_Iss),0) AS [Bal.Qty], Max(I.Unit) As Unit, " & _
    ...
    'Private Sub FGroupOnItem(ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand)
    ...
    Private Sub FGroupOnItem(ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand)
        ...
        mQry = " INSERT INTO Stock (DocId, Sr, " & _
                ...
                " L.Item, " & _
                " Sum(Case When IfNull(L.DifferenceQty,0) > 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End) As Qty_Iss, " & _
                " Sum(Case When IfNull(L.DifferenceQty,0) < 0 Then  IfNull(Abs(L.DifferenceQty),0) Else 0 End) As Qty_Rec, " & _
                " Max(L.Unit) As Unit, Max(L.MeasurePerPcs) As MeasurePerPcs, " & _
                " Sum(Case When IfNull(L.DiffernceMeasure,0) > 0 Then  IfNull(Abs(L.DiffernceMeasure),0) Else 0 End) As Measure_Iss, " & _
                " Sum(Case When IfNull(L.DiffernceMeasure,0) < 0 Then  IfNull(Abs(L.DiffernceMeasure),0) Else 0 End) As Measure_Rec, " & _
