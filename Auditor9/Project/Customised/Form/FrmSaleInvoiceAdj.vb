Public Class FrmSaleInvoiceAdj
    ...
    Private Sub MoveRec()
        ...
        mQry = "SELECT S.DocID, S.Sr, S.RecId || '-' || S.V_Type  AS [Stock Out No], Sg.Name + (CASE WHEN C.CityName IS NULL THEN '' ELSE ', ' || C.CityName End) AS Party, I.Description AS Item, IfNull(S.Qty_Iss,0) - IfNull(SAdj.AdjQty,0) Qty, 0 AS [Adj Qty], 0 as [Bal Qty]  " & _
               "FROM Stock S " & _
               ...
               "LEFT JOIN City C ON Sg.CityCode = C.CityCode   " & _
               "WHERE IfNull(S.Qty_Iss, 0) - IfNull(SAdj.AdjQty, 0) > 0 " & _
        ...
        mQry = "SELECT L.DocId, L.Sr, H.ReferenceNo || '-'  + H.V_Type AS [Stock In No], Sg.Name + (CASE WHEN C.CityName IS NULL THEN '' ELSE ', ' || C.CityName End) AS Party, I.Description AS Item, L.Qty - IfNull(SI.SI_Qty,0) AS Qty, 0 AS [Adj Qty], 0 as [Bal Qty] " & _
               ...
               "WHERE L.Qty - IfNull(SI.SI_Qty, 0) > 0 " & _
               "Order by H.V_Date, H.DocID "

        mQry = "SELECT S.DocID, S.Sr, S.RecId || '-' || S.V_Type  AS [Stock In No], Sg.Name + (CASE WHEN C.CityName IS NULL THEN '' ELSE ', ' || C.CityName End) AS Party, I.Description AS Item, IfNull(S.Qty_Rec,0) - IfNull(SAdj.AdjQty,0) Qty, 0 AS [Adj Qty], 0 as [Bal Qty]  " & _
               "FROM Stock S " & _
               ...
               "LEFT JOIN SubGroup Sg ON S.SubCode = Sg.SubCode " & _
               "LEFT JOIN City C ON Sg.CityCode = C.CityCode   " & _
               "WHERE IfNull(S.Qty_Rec, 0) - IfNull(SAdj.AdjQty, 0) > 0  " & _
               "And S.Site_Code = '" & AgL.PubSiteCode & "' And S.Div_Code = '" & AgL.PubDivCode & "' " & _
               "Order By S.V_Date, S.DocID "
