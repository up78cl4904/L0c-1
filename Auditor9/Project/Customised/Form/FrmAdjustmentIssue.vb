...
Public Class FrmAdjustmentIssue
...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmAdjustmentIssue))
        ...
        Me.GrpDirectChallan.Size = New System.Drawing.Size(104, 27)
        ...
        Me.RbtForStock.Location = New System.Drawing.Point(6, 9)
        ...
        Me.BtnFillSaleChallan.Location = New System.Drawing.Point(353, 136)
        ...
        Me.RbtSaleReturn.Visible = False
        '
        'RbtDirect
        '
        ...
        Me.RbtDirect.Location = New System.Drawing.Point(417, 10)
        ...
        Me.RbtForSaleOrder.Location = New System.Drawing.Point(410, 9)
        ...
        Me.RbtForSaleOrder.Visible = False
        '
        'RbtForSaleChallan
        '
        ...
        Me.RbtForSaleChallan.Location = New System.Drawing.Point(422, 8)
        ...
        Me.RbtForSaleChallan.Visible = False
        '
        'FrmAdjustmentIssue
        '
...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        If bSelectionQry <> "" Then
            mQry = "Insert Into SaleInvoiceDetail(DocId, Sr, SaleOrder, SaleOrderSr, Item_Uid, Item, ItemInvoiced, Specification, SalesTaxGroupItem, " & _
                   ...
                   " SaleChallan, SaleChallanSr, SaleInvoice, SaleInvoiceSr, V_Nature) " & bSelectionQry
            AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
        End If
    ...
    Private Sub FrmSaleOrder_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        mQry = " Select H.*, Sg.Name || ',' || IfNull(C1.CityName,'') As SaleToPartyDesc, G.Description AS GodownDesc," & _
               " BillToParty.Name || ',' || IfNull(BillToPartyCity.CityName,'') As BillToPartyDesc, " & _
               " C.Description As CurrencyDesc, C1.CityName As SaleToPartyCityName, Agent.Name As AgentName, BillToParty.Nature " & _
               " From (Select * From SaleInvoice  Where DocID='" & SearchCode & "') H " & _
               " LEFT JOIN SubGroup Sg  ON H.SaleToParty = Sg.SubCode " & _
               " LEFT JOIN Godown G  ON H.Godown = G.Code " & _
               " LEFT JOIN SubGroup BillToParty  ON H.BillToParty = BillToParty.SubCode " & _
               " LEFT JOIN Currency C  ON H.Currency = C.Code " & _
               " LEFT JOIN City C1  On H.SaleToPartyCity = C1.CityCode " & _
               " LEFT JOIN City BillToPartyCity  On BillToParty.CityCode = BillToPartyCity.CityCode " & _
        ...
        With DsTemp.Tables(0)
            If .Rows.Count > 0 Then
                ...
                mQry = "Select L.*, I.Description As ItemDesc, II.Description As ItemInvoicedDesc , I.ManualCode, " & _
                        ...
                        " LEFT JOIN Item II  ON L.ItemInvoiced = II.Code " & _
                    ...
                    If TxtV_Date.Text <> "" And TxtSaleToParty.Text <> "" Then
                        ...
                        Else
                            mQry = " Select Mobile As SaleToPartyMobile, DispName As SaleToPartyName, " & _
                                    " IfNull(Add1,'') As SaleToPartyAdd1, IfNull(Add2,'') As SaleToPartyAdd2, " & _
    ...
    Private Sub FrmSaleOrder_BaseEvent_Topctrl_tbAdd() Handles Me.BaseEvent_Topctrl_tbAdd
        ...
        TxtCurrency.Tag = AgL.PubDtEnviro.Rows(0)("DefaultCurrency")
                ...
                If .Rows(I).Visible = True Then
                    If .Item(Col1Item, I).Value <> "" Then
                        ...
                        If AgL.StrCmp(Dgl1.Item(Col1V_Nature, I).Value, RbtForStock.Text) Then
                            mQry = " Select IfNull(Sum(Qty_Rec), 0) - IfNull(Sum(Qty_Iss), 0) " & _
                                          " FROM Stock " & _
                        ...
                        If AgL.StrCmp(Dgl1.Item(Col1V_Nature, I).Value, RbtSaleReturn.Text) Then
                            mQry = " Select IfNull(Sum(Qty), 0)  " & _
        ...
        Try
            ...
            Select Case sender.name
                Case TxtCurrency.Name
                    If CType(sender, AgControls.AgTextBox).AgHelpDataSet Is Nothing Then
                        If e.KeyCode <> Keys.Enter Then
                            mQry = "SELECT Code, Code AS Currency, IfNull(IsDeleted,0) AS IsDeleted " & _
                                    " FROM Currency " & _
                                    " ORDER BY Code "
                ...
                Case TxtAgent.Name
                    If CType(sender, AgControls.AgTextBox).AgHelpDataSet Is Nothing Then
                        If e.KeyCode <> Keys.Enter Then
                            mQry = "SELECT Sg.SubCode As Code, Sg.Name || ',' || IfNull(C.CityName,'') As Account_Name, Sg.Nature, Sg.UpLine " & _
                                    " FROM SubGroup Sg " & _
                                    " LEFT JOIN City C ON Sg.CityCode = C.CityCode  " & _
                ...
                Case TxtGodown.Name
                    If TxtGodown.AgHelpDataSet Is Nothing Then
                        mQry = "SELECT H.Code, H.Description " & _
                                ...
                                " And IfNull(H.Status,'" & AgTemplate.ClsMain.EntryStatus.Active & "') = '" & AgTemplate.ClsMain.EntryStatus.Active & "' " & _
    ...
    Private Function FFilterUsedItems() As String
        ...
        Try
            With Dgl1
                For I = 0 To .Rows.Count - 1
                    If .Item(Col1Item, I).Value <> "" Then
                        FFilterUsedItems += " And Code +  IfNull(StockInDocID,'') +  IfNull(StockInDocIDSr,'') <> '" & Dgl1.AgSelectedValue(Col1Item, I) & "' || '" & Dgl1.AgSelectedValue(Col1ReferenceDocId, I) & "' || '" & Dgl1.Item(Col1ReferenceDocIdSr, I).Value & "'"
                    End If
                Next
            End With
        Catch ex As Exception
    ...
    Private Sub FPostInSaleChallan(ByVal Conn As SqliteConnection, ByVal Cmd As SqliteCommand)
        ...
        If AgL.XNull(AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar) > 0 Then
            ...
            mQry = "Insert Into Stock(DocID, Sr, V_Type, V_Prefix, V_Date, V_No, RecID, Div_Code, Site_Code, " & _
                     ...
                     " Case When  IfNull(L.Qty,0) >= 0 Then L.Qty Else 0 End As Qty_Iss, " & _
                     " Case When  IfNull(L.Qty,0) < 0 Then L.Qty Else 0 End As Qty_Rec, " & _
                     " L.Unit, L.MeasurePerPcs, " & _
                     " Case When  IfNull(L.TotalMeasure,0) >= 0 Then L.TotalMeasure Else 0 End As Measure_Iss, " & _
                     " Case When  IfNull(L.TotalMeasure,0) < 0 Then L.TotalMeasure Else 0 End As Measure_Rec, " & _
                     " L.MeasureUnit,  " & _
                     " L.ReferenceDocId, L.ReferenceDocIdSr, " & _
            ...
            mQry = "INSERT INTO dbo.StockAdj(StockInDocID,StockInSr,StockOutDocID,StockOutSr,Site_Code,Div_Code,AdjQty) " & _
                   ...
                   "FROM SaleChallanDetail L " & _
                   "LEFT JOIN SaleChallan H ON L.DocId = H.DocID  " & _
                   "WHERE L.DocID  =  '" & mSearchCode & "' And L.ReferenceDocId IS NOT NULL "
            AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
        ...
        Try
            With Dgl1
                For I = 0 To .Rows.Count - 1
                    If .Item(Col1Item, I).Value <> "" Then
                        If mRow <> I Then
                            If AgL.StrCmp(AgL.XNull(.Item(Col1Item, I).Value) + "|" + AgL.XNull(.Item(Col1ReferenceDocId, I).Value) + "|" + AgL.XNull(.Item(Col1ReferenceDocIdSr, I).Value), AgL.XNull(.Item(Col1Item, mRow).Value) + "|" + AgL.XNull(.Item(Col1ReferenceDocId, mRow).Value) + "|" + AgL.XNull(.Item(Col1ReferenceDocIdSr, mRow).Value)) Then
    ...
    Private Function FHPGD_PendingSaleChallan() As String
        ...
        mQry = " SELECT 'o' As Tick, VMain.SaleChallan As SaleChallan, " & _
                ...
                " IfNull(Sum(VMain.Qty), 0) As [Qty]    " & _
    ...
    Private Function FRetFillItemWiseSaleChallanQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseSaleChallanQry = "SELECT Max(L.V_Nature) As V_Nature, Max(L.SaleOrder) As SaleOrder, Max(So.ReferenceNo) As SaleOrderNo, " & _
                        ...
                        " Sum(L.DocQty) - IfNull(Max(Cd.DocQty), 0) as DocQty,   " & _
                        " Sum(L.FreeQty) - IfNull(Max(Cd.FreeQty), 0) as FreeQty,   " & _
                        " Sum(L.Qty) - IfNull(Max(Cd.Qty), 0) as Qty,   " & _
                        ...
                        "    FROM SaleChallan  " & HeaderConStr & " " & _
                        " ) AS  H   " & _
                        " LEFT JOIN SaleChallanDetail L  ON H.DocID = L.SaleChallan    " & _
                        " Left Join Item I  On L.Item  = I.Code   " & _
                        " LEFT JOIN Voucher_Type Vt  ON H.V_Type = Vt.V_Type    " & _
                        " Left Join (   " & _
    ...
    Private Function FRetFillItemWiseSaleOrderQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseSaleOrderQry = "SELECT '" & RbtForSaleOrder.Text & "' As V_Nature, L.SaleOrder, " & _
                        ...
                        " FROM (  " & _
                        "    SELECT DocID, V_Type, ReferenceNo, V_Date   " & _
                        "    FROM SaleOrder  " & HeaderConStr & " " & _
                        " ) AS  H   " & _
                        " LEFT JOIN SaleOrderDetail L  ON H.DocID = L.SaleOrder " & _
                        " Left Join Item I  On L.Item  = I.Code   " & _
                        " LEFT JOIN Voucher_Type Vt  ON H.V_Type = Vt.V_Type    " & _
                        " Left Join (   " & _
    ...
    Private Function FRetFillItemWiseSaleInvoiceQry(ByVal HeaderConStr As String, ByVal LineConStr As String) As String
        FRetFillItemWiseSaleInvoiceQry = "SELECT '" & RbtSaleReturn.Text & "' As V_Nature, " & _
                        ...
                        " FROM (  " & _
                        "    SELECT DocID, V_Type, ReferenceNo, V_Date   " & _
                        "    FROM SaleInvoice  " & HeaderConStr & " " & _
                        " ) AS  H   " & _
                        " LEFT JOIN SaleInvoiceDetail L  ON H.DocID = L.SaleInvoice " & _
                        " Left Join Item I  On L.Item  = I.Code   " & _
                        " LEFT JOIN Voucher_Type Vt  ON H.V_Type = Vt.V_Type    " & _
                        " LEFT JOIN Unit U On L.Unit = U.Code   " & _
                        " LEFT JOIN Unit U1 On L.MeasureUnit = U1.Code   " & _
    ...
    Private Sub FCreateHelpSubgroup()
        ...
        mQry = "SELECT Sg.SubCode As Code, Sg.Name || ',' || IfNull(C.CityName,'') As Party, Sg.SalesTaxPostingGroup, " & _
    ...
    Private Sub FCreateHelpItem()
        ...
        mQry = " SELECT I.Code, I.Description, H.LotNo AS LotNo, " & _
                 ...
                 " Where IfNull(H.Qty_Rec, 0) - IfNull(SAdj.AdjQty, 0) > 0  And IfNull(H.Qty_Rec, 0)>0 And H.DocId <> '" & mSearchCode & "' And H.Site_Code ='" & TxtSite_Code.Tag & "' " & strCond
    ...
    Private Sub RbtDirect_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles RbtForStock.Click, RbtDirect.Click, RbtForSaleChallan.Click
        Try
            Select Case sender.Name
                Case RbtDirect.Name
                    BtnFillSaleChallan.Enabled = False
